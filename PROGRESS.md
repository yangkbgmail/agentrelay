...
### [세션 62 — PR 적체 3차 대량 통합(`overdue`+`tools` 계열 중복 47개 정리)] (2026-08-09, 무인 자율 세션, branch `claude/wizardly-pascal-qjxbmn`)
- **배경:** 세션 60·61이 타임존/히트맵·watch 중복을 정리했지만, 열린 PR이 **355개**까지 폭증해 있었다.
  근본 원인은 여전히 (a) 매 세션이 새 기능 PR을 병합 속도보다 빨리 열고, (b) 모든 PR이 PROGRESS.md/BACKLOG.md에
  append → 상호 머지 충돌 → 배치 병합 불가 → 같은 기능(`overdue`/`overdue --watch`, `tools`, `upcoming --watch`)이
  수십 개씩 재구현·적체. 새 기능을 추가하면 문제를 악화시키므로, CLAUDE.md·세션 60·61 지침("적체 시 통합 최우선")에
  따라 이번 세션은 **새 기능 0줄**, 오직 통합만 수행했다.
- **한 일 (병합 정책 COLLAB.md L41 "CI 초록이면 클로드 코드가 관리/병합" 근거):**
  - **`overdue` 단일 축 중복 34개를 대표 PR #450(base `overdue` + `--watch` 포함, 문서·테스트 완비)으로 통합(닫음)**:
    - 순수 `overdue --watch` 11개: #449 #447 #441 #433 #431 #430 #428 #427 #424 #422 #417
    - plain `overdue` 명령 23개: #360 #359 #358 #355 #354 #352 #351 #348 #347 #346 #344 #343 #339 #338 #336 #334 #333 #331 #330 #325 #324 #323 #322
    - 각 PR에 대표 #450을 가리키는 정리 코멘트 후 close(필요 시 재오픈 가능). `overdue --watch + tools`처럼 스코프가
      섞인 콤보(#425 #426 등)와 미묘히 다른 프로브(#327)는 보수적으로 보존.
  - **`tools` 단일 축 중복 13개를 대표 PR #459(base `tools`/`projects` + `--watch` 포함)로 통합(닫음)**:
    #384 #380 #370 #367 #365 #363 #356 #353 #350 #345 #335 #332 #312.
  - 순 효과: 열린 PR **355 → 약 308개**(중복 47 정리: overdue 34 + tools 13).
- **다음 할 일(다음 세션이 읽을 것):**
  1. 통합 최우선 기조 유지 — 남은 대형 중복 축(`tools`/`upcoming --watch`/`overdue+tools+projects` 콤보/파서)도
     각 축의 대표 1개만 남기고 정리하면 300 → 150대까지 줄일 수 있다.
  2. **구조적 근본 해결이 시급(🧭 코워크/소유자)**: 자율 빌더 루프가 병합보다 빠르게 PR을 양산 중.
     ① 기능 PR에서 PROGRESS/BACKLOG append 금지(로그는 병합 후 별도 커밋 또는 세션별 파일), ② 새 기능 시작 전
     열린 PR 중복 검사 강제, ③ 적체 임계(예: >50)를 넘으면 신규 생성 대신 통합 모드로 자동 전환 — 이 세 가지가
     없으면 통합 세션이 아무리 돌아도 적체가 재발한다.
