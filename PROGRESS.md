...
### [세션 53 — `agentrelay overdue --watch` 라이브 지연 타임라인] (2026-08-03, 무인 자율 세션, branch `claude/overdue-watch-live`)
- **배경:** 세션 시작 시 BACKLOG의 명시적 👷 항목은 전부 완료([x]), 남은 미완은 🧭 코워크 소유
  (README/ARCHITECTURE/경쟁조사/샘플수집/성능분석)뿐. 세션 49~52가 반복해 "다음 할 일"로 지목한
  인접 항목 — `upcoming`/`overdue`/`tools`/`projects`의 `--watch` 라이브 갱신(공용 `startWatchLoop`
  재사용) — 중 세션 52가 `upcoming --watch`를 끝냈고, 이번엔 그 진단용 거울인 `overdue --watch`를
  구현했다. `overdue`는 이미 due됐는데 재개 안 된 잡을 보여주는데, 지금까진 일회성 스냅샷뿐이라
  지연 span이 째깍째깍 커지는 걸 라이브로 볼 수 없었다.
- **한 일 (branch `claude/overdue-watch-live`):** `agentrelay overdue --watch [seconds]` — 세션 52의
  `startWatchLoop` 인프라 재사용.
  - CLI `overdue.ts`에 순수 `renderOverdueWatchFrame(report, storePath, intervalMs, now, scopeNote?)`
    신설: `status`/`upcoming`의 watch 프레임과 동일한 title/meta 블록(라이브 배너·타임스탬프·스토어
    경로) + 항상 컬러인 `renderOverdue` 본문. 순수 함수라 TTY/시계 없이 테스트 가능.
  - CLI `cli.ts`: 새 `runOverdueWatch`(공용 `startWatchLoop` 재사용, 매 프레임 스토어 재읽기·스코프/
    grace/limit 재적용·fresh `now`로 지연 span 재계산·화면 clear 후 프레임 출력). `upcoming --watch`와
    대칭 — 스토어가 재개되면 리스트가 자동으로 비고, 안 되면 span이 계속 커져 무음 실패 신호가 눈에
    보인다.
...
- **검증:** 로컬 `pnpm install`→`pnpm build` 클린(Next.js 포함)·`pnpm ci:lint`(Biome) **0 경고**·
  ...
  (mock 아님): 임시 스토어(지연 잡 1개)로 `overdue --watch 1`(화면 clear `\x1b[2J\x1b[H`·라이브 배너·
  지연 카운트다운 1h 30m·컬러 출력, timeout으로 종료), 일회성 `overdue` 불변, `--watch --json`(JSON
  우선), `--watch --limit 0`(watch 루프 전 exit 1), `--help`에 `-w, --watch` 노출 확인.
