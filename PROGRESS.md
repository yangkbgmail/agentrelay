...
### [세션 53 — `agentrelay overdue --watch` 라이브 지연 진단] (2026-08-05, 무인 자율 세션, branch `claude/wizardly-pascal-giqv0v`)
- **배경:** 세션 시작 시 BACKLOG의 명시적 👷 항목은 전부 완료([x]), 남은 미완은 🧭 코워크 소유
  (README/ARCHITECTURE/경쟁조사/샘플수집/성능분석)뿐. 세션 49~52가 반복해 "다음 할 일"로 지목한
  인접 항목 — `upcoming`/`overdue`/`tools`/`projects`의 `--watch` 확장 — 중 세션 52가 `upcoming --watch`를
  했고, 그 후속인 `overdue --watch`를 구현했다. `overdue`는 재개 루프(daemon/tick)가 죽었을 때 그 증상
  (리셋 시각이 지났는데 재개 안 된 잡)을 보여주는 진단 커맨드인데, 라이브 뷰가 없어 멈춘 잡의 지연 스팬이
  실시간으로 늘어나는 걸 지켜볼 수 없었다.
- **한 일 (branch `claude/wizardly-pascal-giqv0v`):** `agentrelay overdue --watch [seconds]` — 세션 52의
  공용 `startWatchLoop` 인프라 재사용.
  - CLI `overdue.ts`에 순수 `renderOverdueWatchFrame(report, storePath, intervalMs, now, scopeNote?)`
    신설: `status`/`upcoming` watch 프레임과 동일한 title/meta 블록(라이브 배너·타임스탬프·스토어 경로) +
    항상 컬러인 `renderOverdue` 본문. 순수 함수라 TTY/시계 없이 테스트 가능.
  - CLI `cli.ts`에 새 `runOverdueWatch`(매 프레임 스토어 재읽기·스코프 재적용·고정 `graceMs`/`--limit`로
    리포트 재구성·화면 clear 후 프레임 출력)가 세션 52의 공용 `startWatchLoop(intervalMs, draw)`를 재사용.
  - `overdue` 커맨드에 `-w, --watch [seconds]` 옵션 배선: limit/grace/scope 검증을 **먼저** 통과시켜
    ...
    일회성 기계 덤프). 인터벌 기본 2s, `--watch 5`처럼 초 지정 가능. completion은 라이브 프로그램에서
    파생되므로 `--watch`/`-w` 자동 포함. 새 파서/스케줄러/core 로직 0줄.
- **검증:** 로컬 `pnpm install`→`pnpm build` 클린(Next.js 포함)·`pnpm ci:lint`(Biome) **0 경고**·`pnpm test`
  **전 패키지 통과**(core + cli 285/1skip + dashboard; cli overdue 9→12, watch-frame 3케이스 신규).
  빌드된 실제 CLI e2e(mock 아님): 임시 스토어(overdue 잡 1개)로 `overdue --watch 1`(화면 clear
  `\x1b[2J\x1b[H`·라이브 배너 "live, every 1s"·타임스탬프·스토어 경로·overdue 스팬, 2초 동안 2프레임
  렌더, 파일 리다이렉트 시 stderr 없이 클린 종료), 일회성 `overdue` 불변, `--watch --json`(JSON 우선·
  인터벌 없이 즉시 종료), `--watch --limit 0`(watch 루프 전 exit 1), `--help`·completion에 `--watch` 노출 확인.
