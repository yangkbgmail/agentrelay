...
### [세션 54 — `agentrelay tools --watch` · `projects --watch` 라이브 인덱스] (2026-08-05, 무인 자율 세션, branch `claude/wizardly-pascal-bzpfmw`)
- **배경:** BACKLOG의 👷 항목은 사실상 소진(남은 미완은 전부 🧭 코워크 소유 문서/리서치). 세션 52·53이
  "다음 할 일"로 명시한 후속 — 세션 52가 추출한 공용 `startWatchLoop`을 재사용해 `tools`·`projects`
  두 인덱스 명령에도 `status`/`upcoming`/`overdue`와 동일한 라이브 `--watch` 뷰를 붙였다.
- **한 일:**
  - CLI `tools.ts`에 순수 `renderToolsWatchFrame(summary, storePath, intervalMs, now, scopeNote?)`,
    ...
    동일한 title/meta 블록(라이브 배너·타임스탬프·스토어 경로) + 항상 컬러인 본문(`renderTools`/
    `renderProjects`에 `color:true`·`now` 전달). 순수 함수라 TTY/시계 없이 테스트 가능.
  - CLI `cli.ts`: 세션 52의 공용 `startWatchLoop`을 재사용하는 `runToolsWatch`·`runProjectsWatch`
    (매 프레임 스토어 재읽기·스코프 재적용·summary를 fresh `now`로 재구성 → 리셋 카운트다운 live·
    화면 clear). 시간 창 경계는 시작 시 고정 epoch-ms.
  - `tools`·`projects` 커맨드에 `-w, --watch [seconds]` 옵션 배선: 스코프 검증을 **먼저** 통과시켜
    잘못된 값은 watch 루프 전에 exit 1, `--json`이 `--watch`보다 우선(일회성 기계 덤프), 인터벌 기본
    2s. completion은 라이브 프로그램에서 파생되므로 자동 포함. 새 파서/스케줄러/core 로직 0줄.
- **검증:** 로컬 `pnpm install`→`pnpm build` 클린(Next.js 포함)·`pnpm ci:lint`(Biome) **0 경고**·`pnpm test`
  **전 패키지 통과**(core 553 + cli 291/1skip + dashboard 7; cli tools 8→11, projects 8→11, watch-frame
  각 3케이스 신규). 빌드된 실제 CLI e2e(mock 아님): 임시 스토어(waiting+completed 2잡)로 `projects
  --watch 1`(화면 clear `\x1b[2J\x1b[H`·라이브 배너·컬러 테이블, timeout exit 124), `tools --watch --json`
  (JSON 우선), `tools --watch --status nope`(watch 루프 전 exit 1) 확인.
- **다음 할 일:** 이 브랜치로 main 대상 PR open(CI 초록 시 병합). 👷 백로그가 사실상 비었으니 후속은
  코워크(🧭) 문서 항목(README/ARCHITECTURE/ROADMAP)이나, 새 개선 아이디어 발굴이 필요.
