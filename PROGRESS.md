...
### [세션 49 — `agentrelay upcoming` 재개 타임라인 신규 구현] (2026-07-30, 무인 자율 세션, branch `claude/wizardly-pascal-tw6lzf`)
- **한 일:** BACKLOG의 👷 미완료 항목이 사실상 소진돼(§3 남은 것은 🧭 문서/공동 QA뿐) CLAUDE.md 지침대로
  스스로 신규 개선 항목을 발굴·구현. `agentrelay upcoming` — 재개 대기 잡들의 활주로(runway) 뷰. `next`는
  단 하나만, `status`는 큐 전체를 생성순으로 덤프하는데, "다음 것 뒤로 무엇이 언제 재개되나"를 보는 수단이
  없었다. core `upcoming.ts` 신설(순수): `buildUpcomingTimeline(jobs, now, limit?)`가 `waiting_for_reset`+
  파싱 가능 `resetAt` 잡을 `next`와 동일 tie-break(resetAt→createdAt→id)로 정렬, 각 행 `dueInMs`/`due`/
  `position` + `totalWaiting`/`hidden`/`dueNow`(limit로 잘라도 totals는 정직). CLI `upcoming.ts`에 순수
  `renderUpcoming`(표·공용 `formatCountdown` 재사용·scope note·"N more not shown" 푸터)·`renderUpcomingJson`.
  `agentrelay upcoming [--limit/-n][--tool][--project][--since][--until][--json]` — 공용 `buildScope` 재사용,
  completion 자동 포함. index.ts 재노출. 새 파서/시계 로직 0줄.
- **검증:** 로컬 `pnpm install`→`pnpm build` 클린(Next.js 포함)·`pnpm ci:lint`(Biome) **0 경고**·`pnpm test`
  **전 패키지 통과**(core 533 + cli 262/1skip + dashboard 7; core upcoming 10 + cli upcoming 9 신규 포함).
  빌드된 실제 CLI e2e(mock 아님): 4-잡 임시 스토어로 `upcoming`(정렬·`due now`·카운트다운), `--limit 2`
  (부분표시 + "1 more not shown"), `--project api-svc`(스코프 부분집합), `--json`(envelope), `--limit 0`
  (exit 1) 확인.
...
### [세션 52 — `agentrelay upcoming --watch` 라이브 타임라인] (2026-08-03, 무인 자율 세션, branch `claude/wizardly-pascal-1z6gb2`)
- **배경:** 세션 시작 시 BACKLOG의 명시적 👷 항목은 전부 완료([x]), 남은 미완은 🧭 코워크 소유
  (README/ARCHITECTURE/경쟁조사/샘플수집/성능분석)뿐. 세션 49·50·51이 반복해 "다음 할 일"로 지목한
  인접 항목 — `upcoming`/`overdue`/`tools`/`projects`의 `--watch` 라이브 갱신 — 을 발굴해 첫 항목인
  `upcoming --watch`를 구현했다. `status`엔 이미 `--watch`(세션 12)가 있지만, 재개 대기 타임라인
  (`upcoming`)엔 없어 카운트다운이 째깍째깍 줄어드는 라이브 뷰를 볼 수 없었다.
- **한 일 (branch `claude/wizardly-pascal-1z6gb2`):** `agentrelay upcoming --watch [seconds]` — `status --watch`
  인프라 재사용.
  - CLI `upcoming.ts`에 순수 `renderUpcomingWatchFrame(timeline, storePath, intervalMs, now, scopeNote?)`
    신설: `status`의 `renderWatchFrame`와 동일한 title/meta 블록(라이브 배너·타임스탬프·스토어 경로) +
    항상 컬러인 `renderUpcoming` 본문. 순수 함수라 TTY/시계 없이 테스트 가능.
  - CLI `cli.ts`: 기존 `runWatch`의 draw+setInterval+SIGINT/SIGTERM 정리 로직을 공용 `startWatchLoop
    (intervalMs, draw)`로 추출(리팩터링, 동작 불변) → `runWatch`가 이를 재사용. 새 `runUpcomingWatch`
    (매 프레임 스토어 재읽기·스코프 재적용·타임라인 재구성·화면 clear 후 프레임 출력)도 동일 헬퍼 사용.
  - `upcoming` 커맨드에 `-w, --watch [seconds]` 옵션 배선: limit/scope 검증을 **먼저** 통과시켜 잘못된
    값은 watch 루프를 돌리기 전에 exit 1, `--json`이 `--watch`보다 우선(라이브 TTY 뷰가 아닌 일회성
    기계 덤프). 인터벌 기본 2s, `--watch 5`처럼 초 지정 가능. completion은 라이브 프로그램에서 파생되므로
    `--watch`/`-w` 자동 포함. 새 파서/스케줄러/core 로직 0줄.
- **검증:** 로컬 `pnpm install`→`pnpm build` 클린(Next.js 포함)·`pnpm ci:lint`(Biome) **0 경고**·`pnpm test`
  **전 패키지 통과**(core 553 + cli 282/1skip + dashboard 7; cli upcoming 9→12, watch-frame 3케이스 신규).
  빌드된 실제 CLI e2e(mock 아님): 임시 스토어로 `upcoming --watch 1`(화면 clear `\x1b[2J\x1b[H`·라이브
  배너·카운트다운 1h 30m·컬러 출력, timeout으로 종료 exit 0), 일회성 `upcoming` 불변, `--watch --json`
  (JSON 우선·인터벌 없이 즉시 종료), `--watch --limit 0`(watch 루프 전 exit 1), `--help`·completion에
  `--watch` 노출 확인.
...
### [세션 53 — `agentrelay overdue --watch` 라이브 지연 리포트] (2026-08-05, 무인 자율 세션, branch `claude/wizardly-pascal-79vs8n`)
- **배경:** 세션 시작 시 BACKLOG의 명시적 👷 항목은 전부 완료([x]), 남은 미완은 🧭 코워크 소유
  (README/ARCHITECTURE/경쟁조사/샘플수집/성능분석)뿐. 세션 52가 "다음 할 일"로 명시한 후속 —
  같은 패턴으로 `overdue`/`tools`/`projects`의 `--watch` 확장(공용 `startWatchLoop` 재사용) — 의
  첫 항목인 `overdue --watch`를 구현했다. `overdue`는 "리셋 시각이 지났는데도 재개 안 된 잡"을
  진단하는데, 재개 루프가 죽었는지 실시간으로 지켜보려면 반복 실행해야 했다. 라이브 뷰로
  지연 스팬이 째깍째깍 자라는 걸 그대로 볼 수 있게 했다.
- **한 일 (branch `claude/wizardly-pascal-79vs8n`):** `agentrelay overdue --watch [seconds]` — 세션 12·52의
  `startWatchLoop`/watch-frame 인프라 재사용.
  - CLI `overdue.ts`에 순수 `renderOverdueWatchFrame(report, storePath, intervalMs, now, scopeNote?)`
    신설: `status`/`upcoming`의 watch-frame와 동일한 title/meta 블록(라이브 배너·타임스탬프·스토어
    경로) + 항상 컬러인 `renderOverdue` 본문. 순수 함수라 TTY/시계 없이 테스트 가능.
  - CLI `cli.ts`: 세션 52가 추출한 공용 `startWatchLoop`을 그대로 재사용하는 `runOverdueWatch`
    (매 프레임 스토어 재읽기·스코프 재적용·`buildOverdueReport`를 fresh `now`로 재구성 → 지연 스팬이
    live·화면 clear 후 프레임 출력). 시간 창 경계는 시작 시 고정 epoch-ms.
  - `overdue` 커맨드에 `-w, --watch [seconds]` 옵션 배선: limit/grace/scope 검증을 **먼저** 통과시켜
    잘못된 값은 watch 루프 전에 exit 1, `--json`이 `--watch`보다 우선(일회성 기계 덤프). 인터벌 기본
    2s. completion은 라이브 프로그램에서 파생되므로 `--watch`/`-w` 자동 포함. 새 파서/스케줄러/core
    로직 0줄.
- **검증:** 로컬 `pnpm install`→`pnpm build` 클린(Next.js 포함)·`pnpm ci:lint`(Biome) **0 경고**·`pnpm test`
  **전 패키지 통과**(core 553 + cli 285/1skip + dashboard 7; cli overdue 9→12, watch-frame 3케이스 신규).
  빌드된 실제 CLI e2e(mock 아님): 임시 스토어(overdue 잡 1개)로 `overdue --watch 1`(화면 clear
  `\x1b[2J\x1b[H`·라이브 배너·지연 스팬 1h 35m·컬러 출력, 파일 리다이렉트로 EPIPE 없이 2프레임·
  timeout exit 124), 일회성 `overdue` 불변, `--watch --json`(JSON 우선·1잡·worst-ms 에코),
  `--watch --limit 0`·`--watch --grace nope`(watch 루프 전 exit 1), `--help`·completion에 `--watch`
  노출 확인.
...
### [세션 54 — `agentrelay tools --watch` 라이브 툴 인덱스] (2026-08-05, 무인 자율 세션, branch `claude/wizardly-pascal-jn5lc0`)
- **배경:** 세션 시작 시 BACKLOG의 명시적 👷 항목은 전부 완료([x]), 남은 미완은 🧭 코워크 소유
  (README/ARCHITECTURE/경쟁조사/샘플수집/성능분석)뿐. 세션 52·53이 "다음 할 일"로 명시한 후속 —
  같은 패턴으로 `tools`/`projects`의 `--watch` 확장(공용 `startWatchLoop` 재사용) — 의 다음 항목인
  `tools --watch`를 구현했다. `tools`(세션 51)는 툴별 대기 잡과 다음 리셋을 보여주는데, 라이브 뷰가
  없어 리셋 카운트다운이 줄어드는 걸 보려면 반복 실행해야 했다.
- **한 일 (branch `claude/wizardly-pascal-jn5lc0`):** `agentrelay tools --watch [seconds]` — 세션 12·52의
  `startWatchLoop`/watch-frame 인프라 재사용.
  - CLI `tools.ts`에 순수 `renderToolsWatchFrame(summary, storePath, intervalMs, now, scopeNote?)`
    신설: `status`/`upcoming`/`overdue`의 watch-frame와 동일한 title/meta 블록(라이브 배너·타임스탬프·
    스토어 경로) + 항상 컬러인 `renderTools` 본문. 순수 함수라 TTY/시계 없이 테스트 가능.
  - CLI `cli.ts`: 세션 52가 추출한 공용 `startWatchLoop`을 그대로 재사용하는 `runToolsWatch`
    (매 프레임 스토어 재읽기·스코프 재적용·`summarizeTools`를 fresh `now`로 재구성 → 리셋 카운트다운
    live·화면 clear 후 프레임 출력). 시간 창 경계는 시작 시 고정 epoch-ms.
  - `tools` 커맨드에 `-w, --watch [seconds]` 옵션 배선: 스코프 검증을 **먼저** 통과시켜 잘못된 값은
    watch 루프 전에 exit 1, `--json`이 `--watch`보다 우선(일회성 기계 덤프). 인터벌 기본 2s.
    completion은 라이브 프로그램에서 파생되므로 `--watch`/`-w` 자동 포함. 새 파서/스케줄러/core 로직 0줄.
- **검증:** 로컬 `pnpm install`→`pnpm build` 클린(Next.js 포함)·`pnpm ci:lint`(Biome) **0 경고**·`pnpm test`
  **전 패키지 통과**(cli 288/1skip + dashboard 7; cli tools 8→11, watch-frame 3케이스 신규).
  빌드된 실제 CLI e2e(mock 아님): 2-잡 임시 스토어(claude-code 대기 1[리셋 1h 30m]/codex-cli 완료 1)로
  `tools --watch 1`(화면 clear `\x1b[2J\x1b[H`·라이브 배너·카운트다운 1h 30m·컬러 출력, timeout으로 2프레임·
  exit 124), 일회성 `tools` 불변, `--watch --json`(JSON 우선·즉시 종료 exit 0), `--watch --tool nope`
...
### [세션 59 — `agentrelay stats --weekday` 요일(day-of-week) 분포] (2026-08-07, 무인 자율 세션, branch `claude/wizardly-pascal-k411rs`)
- **배경:** BACKLOG의 명시적 👷 항목은 전부 완료([x]), 남은 미완은 🧭 코워크 소유(README/ARCHITECTURE/
  경쟁조사/샘플수집/성능분석)뿐. 세션 58이 "다음 할 일"로 명시한 후속을 구현했다 — `stats --hours`(세션 58)는
  하루 중 어느 시간(0–23)에 rate-limit이 몰리는지 시간-of-day 축을 주지만, "한 주 중 어느 요일에 몰리나"라는
  요일 축이 없었다. 이 뷰는 주간 사용 리듬 파악(예: "나는 주로 월요일에 throttle된다")에 유용하다.
- **한 일 (branch `claude/wizardly-pascal-k411rs`):** `agentrelay stats --weekday` — UTC 요일(Sun–Sat) 활동 분포 히스토그램.
  - core `stats.ts`: 순수 `computeWeekdayDistribution(jobs)` + `WeekdayActivity`(weekday 0–6·name Sun–Sat·count) +
    `WEEKDAY_NAMES` 상수 신설. 각 잡의 `createdAt`을 `Date.getUTCDay()`로 버킷팅해 **모든 주에 걸쳐** 집계,
    항상 정확히 7 슬롯(Sun→Sat) zero-fill, `createdAt` 누락/파싱불가는 스킵. `computeHourlyDistribution`과
    동일하게 창도 시계도 불필요 — day-of-week는 타임스탬프의 절대 속성이라 순수·`nowMs` 미주입.
  - CLI `stats.ts`: 순수 `renderWeekday(distribution, {color})`(`--hours`와 동일한 막대 스케일링 관례 —
    최다 요일에 스케일, 비영일 최소 1블록 보장, 0일은 dim 베이스라인 점, `Sun`…`Sat` 라벨 + 합계 푸터).
  ...
  - `cli.ts`: `stats`에 `--weekday` 플래그 배선. 일회성 뷰(hours 뒤에 append)·`--json`·`--watch`(fresh `now`로
    ...
    `--hours`/`--trend`와도 조합 가능. addHelpText 예시 1줄, completion은 라이브 프로그램 파생이라 `--weekday`
    자동 포함. 새 파서/스케줄러/core 스토어 로직 0줄.
- **검증:** `pnpm install`→`pnpm build` 클린(Next.js 포함)·`pnpm ci:lint`(Biome) **0 경고**·`pnpm test`
  전 패키지 통과(core 558→563, cli stats 32→37=304/1skip, dashboard 9). **실제 빌드 CLI e2e**(mock 아님):
  4-잡 임시 스토어(web 완료[Mon]·web 실패[Mon]·api 완료[Wed]·api 대기[Sat])로 `stats --weekday`가 Mon에
  풀바(2)·Wed·Sat 각 1·나머지 dim 점·"4 job(s) across 7 day(s), UTC" 푸터 렌더, `--weekday --json`은
  `weekday` 7개(Mon=2·Wed=1·Sat=1, name 포함), 기본 `--json`은 `weekday` 미포함, `--weekday --project web`은
  scope note + Mon만 2·나머지 0, `--hours --weekday`는 두 히스토그램 동시 렌더, `--help`·bash completion에
  `--weekday` 노출 확인.
...
### [세션 61 — PR 적체 2차 통합(중복 11 정리 + 고유 PR #543 병합)] (2026-08-09, 무인 자율 세션, branch `claude/wizardly-pascal-k8ftsn`)
- **배경:** 세션 60이 타임존/히트맵 중복은 정리했지만, 하위 번호대(#457–#509)에 **다른 축의 중복 클러스터**가
  그대로 남아 열린 PR이 **50개**까지 다시 쌓여 있었다. 세션 60의 재발방지 지침 2번("적체가 보이면 빌드가 아니라
  통합이 최우선")에 따라, 새 기능을 만들지 않고 통합을 수행했다.
- **한 일 (병합 정책 COLLAB.md L41 "CI 초록이면 클로드 코드가 병합" 근거):**
  1. **중복 클러스터 11개 정리(닫음)** — 각 클러스터의 가장 최근/정제된 PR을 생존자로 남기고 나머지를 코멘트 후 close:
     `next --watch`(#532 유지 · #478·#473·#465 닫음), `show --watch`(#535 유지 · #481·#472 닫음),
     `errors --watch`(#534 유지 · #477 닫음), `tools/projects --watch`(#459 유지 · #458·#457 닫음),
     `recent`(#475 유지 · #474 닫음), 파서 주(week) 단위(#536 유지 · #471 닫음),
     재시도 히스토그램(#542 `stats --attempts` 유지 · #489 `attempts` 닫음). 파서 tomorrow/today(#496·#527)는
     의미가 미묘히 달라 보수적으로 보존.
  2. **#543 병합** — `agentrelay stats` resolution-time 꼬리 지연 p95·p99(세션 60이 고유 후보로 지목한 순수
     stats 확장, +56/-3). 최신 main(#545) 위로 로컬 병합해 PROGRESS.md 문서 충돌만 수동 해소(양쪽 로그 보존,
     코드 파일은 자동 병합), `pnpm build`/`ci:lint`(0경고)/`test`(core 통과·cli 313/1skip·dashboard 9) 재검증 후 push→merge.
  - 순 효과: 열린 PR 50 → **약 38개**(병합 1, 중복 정리 11). 고유 기능 PR만 남아 다음 세션이 배치 병합하기 쉬워짐.
- **다음 할 일(다음 세션이 읽을 것):**
  1. 세션 60·61의 지침 유지 — **새 PR 전에 열린 PR부터 훑어 중복 확인**, 적체 시 통합 우선.
  2. 남은 고유 기능 PR을 계속 리베이스·병합해 파이프를 비운다: 생존자 `--watch` 계열(#532/#534/#535/#459),
     #536(파서 week), #537(exec 알림), #531(diff), #542(stats --attempts), #523/#483(대시보드), 파서 계열
...
### [세션 63 — 데모/QA 백로그 완료 병합(#548) + `upcoming --watch` 중복 통합] (2026-08-09, 무인 자율 세션, branch `claude/wizardly-pascal-bscnhs`)
- **배경:** BACKLOG의 순수 👷 항목은 전부 완료([x])이고 남은 미완은 🧭 코워크 소유(README/ARCHITECTURE/
  경쟁조사/샘플수집/성능분석)와 공동 항목뿐. 열린 PR을 스캔하니 세션 60·61·62가 정리했음에도 여전히
  ~300개 적체 상태였다. 세션 60~62의 재발방지 지침("적체 시 빌드가 아니라 통합이 최우선")에 따라 새 기능
  0줄로 통합만 수행했다.
- **한 일 (COLLAB.md L41 "CI 초록이면 클로드 코드가 병합" 근거):**
  1. **#548 병합** — `👷🧭 최종 QA + 재현 가능한 데모 스크립트`(BACKLOG §3 마지막 미완 항목)를 완성하는
     `scripts/demo.mjs`(run→감지→큐→`tick`→재개→`completed` 전 경로를 실제 rate-limit 없이 재현하는
     엔드투엔드 데모 겸 스모크 QA). CI 초록(run #622 success)·mergeable clean 확인 후 병합. 이로써 BACKLOG의
     MVP §3 잔여 👷 항목이 모두 소진됨(같은 PR에서 BACKLOG 라인 19가 [x]로 갱신됨).
  2. **`upcoming --watch` 순수 중복 클러스터 7개 정리(닫음)** — 가장 정제된 #398을 대표(생존)로 유지하고
     #383·#388·#389·#391·#394·#395·#397을 각각 대표를 가리키는 코멘트 후 close(필요 시 재오픈 가능).
     overdue/tools와 섞인 콤보(#386·#390)는 각 축의 기존 생존자(#450/#459)와 얽혀 있어 보수적으로 보존.
- **순 효과:** 실제 백로그 항목 1개 완료 병합 + 중복 7개 정리. `upcoming` 계열은 대표 #398로 수렴.
- **다음 할 일(다음 세션이 읽을 것):**
  1. 세션 60~62 기조 유지 — **새 PR 전에 열린 PR부터 훑어 중복 확인**, 적체 시 통합 우선.
  2. 남은 고유 기능 PR을 CI 초록 순으로 계속 병합해 파이프를 비운다: #550(eta)·#547(config get)·#542(stats
     --attempts)·#537(exec 알림)·#536(parser week)·#535/#534/#532(show/errors/next --watch)·#531(diff)·
     #523/#483(대시보드)·파서 계열(#462/#463/#498/#502/#504/#479/#487/#496/#527)·#494(자동 백업)·
     #501(--continue)·#460(ical) 등. 각 병합 후 후속 PR은 PROGRESS 충돌만 리베이스로 해소.
  3. 남은 대형 중복 축(콤보 `upcoming/overdue/tools/projects --watch`, 파서 계열)도 대표만 남기고 정리.
...
### [세션 75 — `agentrelay recover --far-future` (먼-미래 파킹 잡 재큐)] (2026-08-22, 무인 자율 세션, branch `claude/wizardly-pascal-hshkf2`)
- **항목 선정:** BACKLOG 미완료 👷 항목은 전부 소진(남은 미완료는 🧭 코워크 문서/리서치뿐). 세션 74 로그가
  "다음 할 일"로 명시한 후속 인접 후보 "recover가 먼-미래 파킹 잡을 자동 감지·재큐 대상으로 포함하는지 점검"을
  자기 발굴 항목으로 채택. 세션 72가 파서에 지평선 가드를, 73이 `doctor` reset-horizon 검사를, 74가 대시보드
  경고 카드를 넣어 "먼-미래로 misparse된 파킹 잡"을 *감지*하는 커버리지는 CLI·GUI 양쪽으로 넓혔지만, 정작
  그 잡을 **일괄 복구하는 수단은 없었다**(개별 `retry <id>`뿐). `recover`가 이미 고아 `resuming` 잡을
  복구하므로, 대칭적 두 번째 무음 실패인 먼-미래 파킹 잡도 같은 명령이 복구하도록 확장해 감지→복구 루프를 닫음.
- **한 일 (branch `claude/wizardly-pascal-hshkf2`, main에서 재시작):**
  - `packages/core/src/recover.ts`: 순수 `selectFarFutureParkedJobs(jobs, {nowMs, horizonMs})` +
    `FarFutureParkedReport`(total·parked·horizonMs·farFuture[]) 신설. `waiting_for_reset` 잡 중 `resetAt`이
    지평선(now+horizonMs)을 넘는 것만 선택(가장 먼 순 정렬). 파서 가드·`doctor`와 동일한 `isPlausibleReset`
    술어 재사용 → 드리프트 0. 비양수·비유한·null horizon=가드 off=빈 선택(off면 전부가 아닌 아무것도 복구).
    queued·resuming·종료 잡 제외.
  - `packages/cli/src/commands.ts`: `recoverJobs`에 `farFuture`/`horizonMs` 옵션. 먼-미래 잡은 `requeueNow`로
    재큐(attempts 0 리셋·lastError 클리어; 고아 재개는 `recoverResuming`으로 attempts 보존 — misparse 파킹은
    한 번도 안 돌았으므로 깨끗한 첫 시도가 맞음). scan↔write 사이 상태 변화는 라이브 재확인으로 스킵.
  - `packages/cli/src/recover.ts`: `renderRecover`를 orphaned/far-future 2섹션 분리 렌더로 재구성(라벨로 클래스
    구분), `renderRecoverJson`은 `farFuture` 블록을 요청 시에만 방출(기본 JSON shape 불변). `resetIn` 헬퍼 추가.
  - `packages/cli/src/cli.ts`: `recover --far-future` 플래그 배선(완성은 `maxResetHorizonMsFromEnv`로 지평선
    해소 — 파서·doctor·대시보드와 동일 env). completion은 커맨더 프로그램에서 자동 파생.
- **검증:** `pnpm install`→`pnpm build`(전 패키지 클린)→`pnpm ci:lint`(Biome 0에러; 포맷 2파일 `lint:fix`)→
  `pnpm test` 전 패키지 통과(**core 655(+6) · cli 365/1skip(+8) · dashboard 13**). 실제 빌드 CLI e2e로 기본
  recover는 파킹 잡 미변경(하위호환)·`--far-future --dry-run` 미리보기·`--far-future`는 100일 파킹 잡을
  due-now+attempts 0으로 재큐하고 근접 2h 잡은 보존·`AGENTRELAY_MAX_RESET_HORIZON=off`면 아무것도 복구 안 함
  +"guard is disabled" 노트·help/completion `--far-future` 노출 확인.
- **다음 할 일:** 이 브랜치로 main 대상 PR open(CI 초록 시 병합). 후속 인접 후보 — `recover --far-future`를
  대시보드에서 직접 트리거하는 액션 버튼(로컬 API route 필요), 또는 `cancel/retry --all`처럼 먼-미래 파킹 잡에
  대한 `bulkControl` 스코프 프리셋. README/ARCHITECTURE(🧭 코워크). PR 포화 항목(stats 분산/watch·summary --watch)은 지양.
