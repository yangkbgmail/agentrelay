...
### [세션 53 — `agentrelay overdue --watch` / `tools --watch` 라이브 갱신] (2026-08-03, 무인 자율 세션, branch `claude/wizardly-pascal-f0a1kn`)
- **배경:** 세션 시작 시 BACKLOG의 명시적 👷 항목은 전부 완료([x]), 남은 미완은 🧭 코워크 소유
  (README/ARCHITECTURE/경쟁조사/샘플수집/성능분석)뿐. 세션 49·50·51·52가 반복해 "다음 할 일"로 지목한
  인접 항목 — `overdue`/`tools`/`projects`의 `--watch` 라이브 갱신 — 중 앞 두 개를 구현했다. 세션 52가
  `upcoming --watch`와 공용 `startWatchLoop`를 만들어 둔 덕에 이번엔 core/파서 손대지 않고 순수 렌더 프레임
  + 워치 루프 배선만으로 끝났다.
- **한 일 (branch `claude/wizardly-pascal-f0a1kn`):** `agentrelay overdue --watch [seconds]` ·
  `agentrelay tools --watch [seconds]` — `upcoming --watch` 인프라 재사용.
  - CLI `overdue.ts`에 순수 `renderOverdueWatchFrame(report, storePath, intervalMs, now, scopeNote?)`,
    ...
    `upcoming`/`status`의 watch-frame과 동일한 title/meta 블록(라이브 배너·타임스탬프·스토어 경로) + 항상
    컬러인 본문(`renderOverdue`/`renderTools`). 순수 함수라 TTY/시계 없이 테스트 가능.
  - CLI `cli.ts`에 `runOverdueWatch`(매 프레임 스토어 재읽기·스코프 재적용·grace/limit로 리포트 재구성 →
    "overdue by" 스팬이 라이브로 자라남)·`runToolsWatch`(매 프레임 재요약 → next-reset 카운트다운이 째깍째깍)
    추가. 둘 다 세션 52의 공용 `startWatchLoop(intervalMs, draw)` + 화면 clear(`\x1b[2J\x1b[H`) 재사용.
  - `overdue`/`tools` 커맨드에 `-w, --watch [seconds]` 옵션 배선: limit/grace/scope 검증을 **먼저** 통과시켜
...
- **검증:** 로컬 `pnpm install`→`pnpm build` 클린(Next.js 포함)·`pnpm ci:lint`(Biome) **0 경고**·`pnpm test`
  **전 패키지 통과**(core 553 + cli 288/1skip + dashboard 7; cli overdue 9→12·tools 8→11, watch-frame 각 3케이스
  신규). 빌드된 실제 CLI e2e(mock 아님): 3-잡 임시 스토어(overdue 2 + 미래 대기 1)로 `overdue --watch 1`
  (화면 clear·라이브 배너·타임스탬프·overdue 스팬 1h30m/30m·most-overdue 정렬·footer 힌트), `tools --watch 1`
  (per-tool 카운트다운·툴 랭킹), `--watch --json`(JSON 우선·즉시 종료), `--watch --limit 0`·`--watch --tool nope`
  (watch 루프 전 exit 1), `overdue/tools --help`에 `--watch` 노출 확인.
- **다음 할 일:** 이 브랜치로 main 대상 PR open(CI 초록 시 병합). 후속: 마지막 남은 `projects --watch`도 같은
  패턴으로 확장(공용 `startWatchLoop` 재사용). README/ARCHITECTURE(🧭 코워크).
