...
### [세션 54 — `agentrelay tools --watch` / `projects --watch` 라이브 인덱스] (2026-08-05, 무인 자율 세션, branch `claude/wizardly-pascal-bp2ma3`)
- **배경:** 세션 시작 시 BACKLOG의 명시적 👷 항목은 전부 완료([x]), 남은 미완은 🧭 코워크 소유
  ...
  `tools`/`projects`는 툴·프로젝트별 대기 작업과 다음 리셋을 한눈에 보여주는데, 카운트다운이
  줄어드는 걸 실시간으로 지켜보려면 반복 실행해야 했다. 라이브 뷰로 리셋까지 남은 시간이
  째깍째깍 줄어드는 걸 그대로 볼 수 있게 했다.
- **한 일 (branch `claude/wizardly-pascal-bp2ma3`):** `agentrelay tools --watch [seconds]` /
  ...
  - CLI `tools.ts`·`projects.ts`에 순수 `renderToolsWatchFrame`/`renderProjectsWatchFrame` 신설:
    `status`/`upcoming`/`overdue`의 watch-frame와 동일한 title/meta 블록(라이브 배너·타임스탬프·스토어
    경로) + 항상 컬러인 `renderTools`/`renderProjects` 본문. `now` 주입으로 TTY/시계 없이 테스트 가능.
  - CLI `cli.ts`: 공용 `startWatchLoop`을 재사용하는 `runToolsWatch`/`runProjectsWatch`(매 프레임
    ...
    리셋 카운트다운 live·화면 clear 후 프레임 출력). 시간 창 경계는 시작 시 고정 epoch-ms.
  - 두 커맨드에 `-w, --watch [seconds]` 옵션 배선: 스코프 검증을 **먼저** 통과시켜 잘못된 필터는
    watch 루프 전에 exit 1, `--json`이 `--watch`보다 우선(일회성 기계 덤프). 인터벌 기본 2s.
    completion은 라이브 프로그램에서 파생되므로 `--watch`/`-w` 자동 포함. 새 파서/스케줄러/core 로직 0줄.
- **검증:** 로컬 `pnpm install`→`pnpm build` 클린(Next.js 포함)·`pnpm ci:lint`(Biome) **0 경고**·`pnpm test`
  **전 패키지 통과**(dashboard 7 + core + cli 291/1skip; cli tools 8→11·projects 8→11, watch-frame 각
  3케이스 신규). 빌드된 실제 CLI e2e(mock 아님): 임시 스토어(waiting 잡 1개+완료 잡 1개)로
  `tools --watch 1`·`projects --watch 1`(화면 clear `\x1b[2J\x1b[H`·라이브 배너·리셋 카운트다운 1h 30m·
  컬러 출력, 파일 리다이렉트로 EPIPE 없이 3프레임·timeout exit 124), `--watch --json`(JSON 우선·exit 0),
  `--watch --tool bogus`(watch 루프 전 exit 1), `--help`·completion에 `--watch` 노출 확인.
- **다음 할 일:** 이 브랜치로 main 대상 PR open(CI 초록 시 병합). 이로써 세 형제 발견 명령
  (`upcoming`/`overdue`/`tools`/`projects`)의 `--watch` 라이브 뷰가 모두 갖춰짐. 후속 아이디어:
  `stats --watch`(성공률/해결시간 라이브 대시보드) 또는 `errors --watch`. README/ARCHITECTURE(🧭 코워크).
