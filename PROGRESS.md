...
### [세션 53 — `agentrelay overdue --watch` 라이브 진단 타임라인] (2026-08-04, 무인 자율 세션, branch `claude/wizardly-pascal-3h573j`)
- **배경:** 세션 시작 시 BACKLOG의 명시적 👷 항목은 전부 완료([x]), 남은 미완은 🧭 코워크 소유
  (README/ARCHITECTURE/경쟁조사/샘플수집/성능분석)뿐. 세션 49~52가 반복해 "다음 할 일"로 지목한
  인접 항목 — `overdue`/`tools`/`projects`의 `--watch` 라이브 갱신 — 중 첫 항목인 `overdue --watch`를
  구현했다. `overdue`는 재개 루프가 죽었는지 진단하는 커맨드라 라이브 뷰의 가치가 가장 크다: 지연 잡의
  "OVERDUE BY"가 째깍째깍 늘어나는 걸 인시던트 중에 지켜볼 수 있다(`upcoming --watch`의 진단용 거울).
- **한 일 (branch `claude/wizardly-pascal-3h573j`):** `agentrelay overdue --watch [seconds]` — 세션 52가
  ...
  - CLI `cli.ts`: 새 `runOverdueWatch`(매 프레임 스토어 재읽기·스코프 재적용·`buildOverdueReport`로
    리포트 재구성·화면 clear 후 프레임 출력)가 공용 `startWatchLoop`를 재사용. `overdue` 커맨드에
    `-w, --watch [seconds]` 옵션 배선: limit/grace/scope 검증을 **먼저** 통과시켜 잘못된 값은 watch 루프를
    돌리기 전에 exit 1, `--json`이 `--watch`보다 우선(라이브 TTY 뷰가 아닌 일회성 기계 덤프). 인터벌
    기본 2s, `--watch 5`처럼 초 지정 가능. completion 자동 포함. 새 파서/스케줄러/core 로직 0줄.
- **검증:** 로컬 `pnpm install`→`pnpm build` 클린(Next.js 포함)·`pnpm ci:lint`(Biome) **0 경고**·`pnpm test`
  **전 패키지 통과**(core 553 + cli 285/1skip + dashboard 7; cli overdue 9→12, watch-frame 3케이스 신규).
  빌드된 실제 CLI e2e(mock 아님): 임시 스토어(90분 지연 1잡)로 `overdue --watch 1`(화면 clear
  `\x1b[2J\x1b[H`·라이브 배너·overdue 카운트업 `1h 30m`·컬러 출력, 2.5s 동안 3프레임 그린 뒤 SIGTERM
  종료·EPIPE 없음), 일회성 `overdue` 불변, `--watch --json`(JSON 우선), `--watch --limit 0`(watch 루프 전
