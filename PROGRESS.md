...
### [세션 53 — `agentrelay overdue --watch` 라이브 지연 진단] (2026-08-04, 무인 자율 세션, branch `claude/wizardly-pascal-s09wzq`)
- **배경:** 세션 시작 시 BACKLOG의 명시적 👷 항목은 전부 완료([x]), 남은 미완은 🧭 코워크 소유
  (README/ARCHITECTURE/경쟁조사/샘플수집/성능분석)뿐. 세션 52가 "다음 할 일"로 지목한 인접 항목 —
  `upcoming`의 `--watch`를 형제 명령으로 확장 — 중 `overdue --watch`를 구현했다. `overdue`는 리셋 시각이
  이미 지났는데도 재개 안 된 잡(재개 루프가 죽었다는 신호)을 보여주는 진단 커맨드인데, 정작 "루프가 정말
  멈췄나?"를 **앉아서 지켜보며** 지연 스팬이 계속 커지는지 확인하는 라이브 뷰가 없었다.
- **한 일 (branch `claude/wizardly-pascal-s09wzq`):** `agentrelay overdue --watch [seconds]` — 세션 52의
  공용 `startWatchLoop` 인프라 재사용, 새 core/스케줄러 로직 0줄.
  ...
  - CLI `cli.ts`: 새 `runOverdueWatch`(매 프레임 스토어 재읽기·스코프 재적용 + `--grace`·`--limit`
    재적용, 프레시 `now`로 리포트 재구성해 "overdue by" 스팬이 계속 커짐·화면 clear 후 프레임 출력)가
    세션 52의 공용 `startWatchLoop`를 재사용. 루프가 다시 살아나 잡이 재개되면 리스트가 라이브로 비워짐.
  - `overdue` 커맨드에 `-w, --watch [seconds]` 옵션 배선: limit/grace/scope 검증을 **먼저** 통과시켜
    ...
    `--watch 5`처럼 초 지정 가능. completion은 라이브 프로그램에서 파생되므로 `--watch`/`-w` 자동 포함.
- **검증:** 로컬 `pnpm install`→`pnpm build` 클린(Next.js 포함)·`pnpm ci:lint`(Biome) **0 경고**·`pnpm test`
  **전 패키지 통과**(core 29파일 + cli 17파일 + dashboard 1; cli overdue 9→12, watch-frame 3케이스 신규).
  빌드된 실제 CLI e2e(mock 아님): 2-job 임시 스토어(과거 리셋 web=overdue 2h + 미래 리셋 api)로
  `overdue --watch 1`(화면 clear `\x1b[2J\x1b[H`·라이브 배너·overdue 2h 0m·컬러 출력, timeout 종료),
  일회성 `overdue` 불변, `--watch --json`(JSON 우선·즉시 종료), `--watch --limit 0`(watch 루프 전 exit 1),
  `--tool codex-cli` 스코프(overdue 아닌 잡 제외→keeping-up), `--help`에 `--watch` 노출 확인.
- **다음 할 일:** 이 브랜치로 main 대상 PR open(CI 초록 시 병합). 후속: 남은 형제 `tools --watch`·
  `projects --watch`도 같은 패턴으로 확장(공용 `startWatchLoop` 재사용). README/ARCHITECTURE(🧭 코워크).
