...
### [세션 59 — `agentrelay stats --dow` 요일(day-of-week) 분포] (2026-08-07, 무인 자율 세션, branch `claude/wizardly-pascal-jsoctv`)
- **배경:** BACKLOG의 명시적 👷 항목은 전부 완료([x]), 남은 미완은 🧭 코워크 소유(README/ARCHITECTURE/
  경쟁조사/샘플수집/성능분석)뿐. 세션 58이 "다음 할 일"로 명시한 후속을 골랐다 — `stats --hours`(세션 58)는
  하루 중 시간(0–23) 분포를 주지만, "어느 요일에 rate-limit이 몰리나"라는 요일 축이 없었다. 주중/주말
  사용 패턴 파악(예: "나는 주로 평일에 throttle된다")에 유용하다.
- **한 일 (branch `claude/wizardly-pascal-jsoctv`):** `agentrelay stats --dow` — UTC 요일(일–토) 활동 분포 히스토그램.
  - core `stats.ts`: 순수 `computeWeekdayDistribution(jobs)` + `WeekdayActivity`(weekday 0–6, count) 신설.
    각 잡의 `createdAt`을 UTC 요일(`getUTCDay`, 0=일…6=토)로 버킷팅해 **모든 주에 걸쳐** 집계, 항상 정확히
    7 슬롯(일→토) zero-fill, `createdAt` 누락/파싱불가는 스킵. `computeHourlyDistribution`과 동일하게 창도
    시계도 불필요 — 요일은 타임스탬프의 절대 속성이라 순수·`nowMs` 미주입.
  - CLI `stats.ts`: 순수 `renderWeekday(distribution, {color})`(`--hours`와 동일한 막대 스케일링 관례 —
    최다 요일에 스케일, 비영 요일 최소 1블록 보장, 0요일은 dim 베이스라인 점, `Sun`~`Sat` 라벨 + 합계 푸터).
  ...
  - `cli.ts`: `stats`에 `--dow` 플래그 배선. 일회성 뷰(hours 뒤에 append)·`--json`·`--watch`(fresh `now`로
    본문 재조합) 세 뷰 모두 적용, 기존 스코프 필터(--status/--tool/--project/--since/--until)와 조합 가능
    (--trend/--hours와 함께도 렌더). addHelpText 예시 1줄, completion은 라이브 프로그램 파생이라 `--dow`
    자동 포함. 새 파서/스케줄러/core 스토어 로직 0줄.
- **검증:** `pnpm install`→`pnpm build` 클린(Next.js 포함)·`pnpm ci:lint`(Biome) **0 경고**·`pnpm test`
  전 패키지 통과(cli stats 32→40=307/1skip, dashboard·core 무변). **실제 빌드 CLI e2e**(mock 아님):
  4-잡 임시 스토어(web 완료[일]/실패[일]·api 완료[월]·대기[금])로 `stats --dow`가 Sun에 풀바(2)·Mon·Fri
  각 1·나머지 dim 점·"4 job(s) across 7 day(s), UTC" 푸터 렌더, `--dow --json`은 `weekdays` 7개
  (일=2/월=1/금=1), 기본 `--json`은 `weekdays` 미포함, `--dow --project api`는 scope note + Mon/Fri만 1,
  `--help`·bash completion에 `--dow` 노출 확인.
- **다음 할 일:** 이 브랜치로 main 대상 PR open(CI 초록 시 병합). 후속 인접 항목 발굴 후보 — 대시보드에
  시간대/요일/추이 히스토그램 노출, 또는 `stats --dow`를 요일×시간 히트맵으로 결합. README/ARCHITECTURE(🧭 코워크).
