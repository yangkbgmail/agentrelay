...
### [세션 53 — `agentrelay overdue --watch` 라이브 지연 재개 모니터] (2026-08-04, 무인 자율 세션, branch `claude/wizardly-pascal-nlgx38`)
- **배경:** 세션 시작 시 BACKLOG의 명시적 👷 항목은 전부 완료([x]), 남은 미완은 🧭 코워크 소유
  (README/ARCHITECTURE/경쟁조사/샘플수집/성능분석)뿐. 세션 52가 `upcoming --watch`를 만들며 "다음 할 일"로
  지목한 인접 항목 — `overdue`/`tools`/`projects`의 `--watch` 라이브 갱신(공용 `startWatchLoop` 재사용) —
  중 첫 항목인 `overdue --watch`를 구현했다. `overdue`는 리셋 시각이 지났는데도 재개 안 된 잡을 보여주는
  진단 커맨드라, 라이브로 띄워두면 "멈춰버린 재개 루프" 모니터로 가장 유용(overdue-by 스팬이 제자리에서 자라남).
- **한 일 (branch `claude/wizardly-pascal-nlgx38`):** `agentrelay overdue --watch [seconds]` —
  세션 52의 공용 `startWatchLoop` 인프라 재사용.
  - CLI `overdue.ts`에 순수 `renderOverdueWatchFrame(report, storePath, intervalMs, now, scopeNote?)`
    신설: `upcoming --watch`의 `renderUpcomingWatchFrame`와 동일한 title/meta 블록(라이브 배너·타임스탬프·
    스토어 경로) + 항상 컬러인 `renderOverdue` 본문. 순수 함수라 TTY/시계 없이 테스트 가능.
  - CLI `cli.ts`에 `runOverdueWatch(store, intervalMs, window, {graceMs, limit, scopeNote})` — 공용
    `startWatchLoop` 재사용. 매 프레임 스토어 재읽기·스코프 재적용·`now`로 리포트 재빌드(overdue 스팬이
    시간이 갈수록 자라남)·화면 clear 후 프레임 출력. window 경계는 명령 시작 시 고정 절대 epoch-ms라
    라이브 쓰기는 계속 반영(회복한 데몬이 리스트를 비우는 것도 자동 반영).
...
- **검증:** 로컬 `pnpm install`→`pnpm build` 클린(Next.js 포함)·`pnpm ci:lint`(Biome) **0 경고**·`pnpm test`
  **전 패키지 통과**(core 553 + cli 285/1skip + dashboard 7; cli overdue 9→12, watch-frame 3케이스 신규).
  빌드된 실제 CLI e2e(mock 아님): 임시 스토어(90분 지연 잡)로 `overdue --watch 1`(화면 clear
  `\x1b[2J\x1b[H`·라이브 배너·overdue 스팬 1h 30m·컬러 출력, timeout 종료 exit 0), 일회성 `overdue` 불변,
  `--watch --json`(JSON 우선·인터벌 없이 즉시 종료), `--watch --limit 0`(watch 루프 전 exit 1) 확인.
