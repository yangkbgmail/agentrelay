...
- [x] 👷 `agentrelay overdue --watch [seconds]` — 지연(overdue) 재개 잡을 라이브로 갱신(지연 시간이
      째깍째깍 늘어나는 진단 뷰). `upcoming --watch`(세션 52)를 그 진단용 거울인 `overdue`에 확장.
      (완료 — CLI `overdue.ts`에 순수 `renderOverdueWatchFrame(report, storePath, intervalMs, now,
      scopeNote?)` 신설(`status`/`upcoming`의 watch 프레임과 동일 title/meta 블록 + 항상 컬러인
      `renderOverdue` 본문). `cli.ts`의 세션 52 공용 `startWatchLoop(intervalMs, draw)`를 재사용하는 새
      `runOverdueWatch`(매 프레임 스토어 재읽기·스코프 재적용·`graceMs`/`limit` 재적용·보고서 재구성·화면
      clear). `overdue`에 `-w, --watch [seconds]` 배선: limit/grace/scope 검증을 먼저 통과시켜 잘못된 값은
      watch 전에 exit 1, `--json`이 `--watch`보다 우선. 인터벌 기본 2s. completion 자동 포함. 새 파서/
      스케줄러/core 로직 0줄. cli overdue watch-frame 3케이스 신규, 실제 빌드 CLI e2e로 화면 clear·라이브
      배너·overdue by 카운트업·--json 우선·--limit 0 exit 1·completion 검증. branch
      `claude/wizardly-pascal-vr8b9m`)
