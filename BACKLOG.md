...
- [x] 👷 `agentrelay overdue --watch [seconds]` — 지연(overdue) 리포트를 라이브로 갱신("overdue by"가
      째깍째깍 커지는 뷰). 재개 루프 무음 실패를 실시간 감시. `upcoming --watch`(세션 52)의 진단용 거울.
      (완료 — CLI `overdue.ts`에 순수 `renderOverdueWatchFrame(report, storePath, intervalMs, now, scopeNote?)`
      신설(`upcoming`/`status` watch 프레임과 동일 title/meta 블록 + 항상 컬러인 `renderOverdue` 본문).
      `cli.ts`에 새 `runOverdueWatch`가 세션 52의 공용 `startWatchLoop`를 재사용(매 프레임 스토어 재읽기·
      스코프/grace/limit 재적용·리포트 재구성·화면 clear). `overdue`에 `-w, --watch [seconds]` 배선:
      limit/grace/scope 검증을 먼저 통과시켜 잘못된 값은 watch 전에 exit 1, `--json`이 `--watch`보다 우선,
      인터벌 기본 2s. completion 자동 포함. 새 파서/스케줄러/core 로직 0줄. cli overdue watch-frame 3케이스
      신규(9→12), 실제 빌드 CLI e2e로 화면 clear·라이브 배너·"overdue by 1h 30m"·--json 우선·--limit 0/
      --grace 오류 exit 1·completion 검증. branch `claude/wizardly-pascal-gwnpuo`)
