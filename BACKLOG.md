...
- [x] 👷 `agentrelay overdue/tools/projects --watch [seconds]` — `--watch` 라이브 뷰를 세 형제 명령으로
      확장(`status`·`upcoming`에만 있던 라이브 갱신을 지연 진단·툴/프로젝트 인덱스에도). 세션 52가 "다음
      할 일"로 지목한 인접 항목.
      (완료 — 세션 52의 공용 `startWatchLoop` 재사용. CLI `overdue.ts`/`tools.ts`/`projects.ts`에 각각 순수
      `renderOverdueWatchFrame`/`renderToolsWatchFrame`/`renderProjectsWatchFrame`(storePath, intervalMs, now,
      scopeNote?) 신설 — `renderUpcomingWatchFrame`와 동일 title/meta 블록[라이브 배너·타임스탬프·스토어
      경로] + 항상 컬러인 본문. `cli.ts`에 `runOverdueWatch`/`runToolsWatch`/`runProjectsWatch`(매 프레임
      스토어 재읽기·스코프 재적용·리포트/서머리 재구성·화면 clear, `overdue`는 graceMs/limit도 재적용)
      + 세 명령에 `-w, --watch [seconds]` 배선: limit/grace/scope 검증을 **먼저** 통과시켜 잘못된 값은
      watch 전에 exit 1, `--json`이 `--watch`보다 우선(일회성 기계 덤프), 인터벌 기본 2s. `--since`/`--until`
      경계는 시작 시 고정 epoch-ms. completion 자동 포함. 새 파서/스케줄러/core 로직 0줄. cli overdue/tools/
      projects 각 watch-frame 3케이스(총 9) 신규, 실제 빌드 CLI e2e로 네 watch 명령 라이브 렌더·clean
      timeout-kill(stderr 0)·`--json` 우선·`--watch --limit 0`/`--tool nope` exit 1·`--help` 노출 검증.
      branch `claude/wizardly-pascal-ouqihu`)
