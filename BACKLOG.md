...
- [x] 👷 `agentrelay overdue --watch [seconds]` — 지연(overdue) 진단 테이블을 라이브로 갱신(정체된
      재개 루프를 실시간 모니터링). `upcoming --watch`(세션 52)의 진단 거울.
      (완료 — `overdue`는 재개 루프가 죽었을 때 늘어나는 지연 잡을 보여주지만 일회성 스냅샷뿐이라,
      데몬/tick 복구를 지켜보려면 매번 다시 실행해야 했다. CLI `overdue.ts`에 순수
      `renderOverdueWatchFrame(report, storePath, intervalMs, now, scopeNote?)` 신설: `status`/`upcoming`
      watch와 동일한 title/meta 블록(라이브 배너·타임스탬프·스토어 경로) + 항상 컬러인 `renderOverdue`
      본문. `upcoming`이 카운트다운을 아래로 세는 거울이라면 이건 "overdue by" 스팬이 위로 자라는 뷰.
      `cli.ts`에 `runOverdueWatch`(세션 52의 공용 `startWatchLoop` 재사용 — 매 프레임 스토어 재읽기·
      스코프 재적용·`graceMs`/`limit` 재적용·리포트 재구성·화면 clear) + `overdue`에 `-w, --watch [seconds]`
      배선: limit/grace/scope 검증을 **먼저** 통과시켜 잘못된 값은 watch 전에 exit 1, `--json`이
      `--watch`보다 우선(일회성 기계 덤프), 인터벌 기본 2s. completion 자동 포함. 새 파서/스케줄러/core
      로직 0줄. cli overdue watch-frame 3케이스 신규(10→13), 실제 빌드 CLI e2e로 화면 clear·라이브 배너·
      overdue 스팬·--json 우선·--limit 0 exit 1·--help 노출 검증. branch `claude/wizardly-pascal-x1dhb4`)
