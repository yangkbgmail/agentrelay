...
- [x] 👷 `agentrelay overdue --watch [seconds]` — 지연(overdue) 진단 리포트를 라이브로 갱신(지연 시간이
      째깍째깍 늘어나는 뷰). 세션 49(`upcoming --watch`)의 후속으로 공용 `startWatchLoop` 재사용, 재개
      루프가 막혔을 때 백로그가 쌓이는 걸 실시간으로 지켜보는 모니터링 뷰.
      (완료 — CLI `overdue.ts`에 순수 `renderOverdueWatchFrame(report, storePath, intervalMs, now, scopeNote?)`
      신설(`upcoming`의 `renderUpcomingWatchFrame`와 동일 title/meta 블록 + 항상 컬러인 `renderOverdue`
      본문). `cli.ts`에 `runOverdueWatch`(매 프레임 스토어 재읽기·스코프 재적용·`graceMs` 유지·fresh `now`로
      리포트 재구성·화면 clear)가 공용 `startWatchLoop` 재사용. `overdue`에 `-w, --watch [seconds]` 배선:
      limit/grace/scope 검증을 **먼저** 통과시켜 잘못된 값은 watch 전에 exit 1, `--json`이 `--watch`보다
      우선(일회성 기계 덤프), 인터벌 기본 2s. completion 자동 포함. 새 core/파서/스케줄러 로직 0줄.
      cli overdue watch-frame 3케이스 신규, 실제 빌드 CLI e2e로 화면 clear·라이브 배너·카운트다운·
      --json 우선·--watch --limit 0 exit 1·help/completion 노출 검증. branch `claude/wizardly-pascal-9x44qk`)
