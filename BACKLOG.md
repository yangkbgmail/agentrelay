...
- [x] 👷 `agentrelay stats --dow` — 어느 UTC 요일(Sun–Sat)에 rate-limit이 몰리는지 요일 분포 히스토그램.
      (완료 — 세션 58의 `--hours`(시간-of-day)의 자매 뷰. "나는 주로 월요일에 throttle된다" 같은 주간 패턴
      파악용. core `stats.ts`에 순수 `computeWeekdayDistribution(jobs)` + `WeekdayActivity`(weekday 0–6, count)
      신설 — 각 잡의 `createdAt`을 `getUTCDay()`(0=Sun..6=Sat)로 버킷팅해 **모든 주에 걸쳐** 집계, 항상 정확히
      7 슬롯 zero-fill, `createdAt` 누락/파싱불가는 스킵. `--hours`와 동일하게 창도 시계도 불필요(요일은
      절대 속성). CLI `stats.ts`에 순수 `renderDow`(7행 ASCII 막대, 최다 요일에 스케일, 0요일은 dim 점,
      `Sun`/`Mon`… 3글자 라벨 + 합계 푸터)·`renderStatsJson`에 옵셔널 `dow` 필드(요청 시에만 방출, 기존 JSON
      shape 불변). `cli.ts` `stats`에 `--dow` 배선 — 일회성·`--json`·`--watch` 세 뷰 모두 적용, 기존 스코프
      필터 및 `--hours`/`--trend`와 조합 가능. 새 파서/스케줄러 로직 0줄. core stats +5 + cli stats +5(=37)
      신규 테스트, 실제 빌드 CLI e2e로 요일 버킷팅·최다요일 풀바·스코프 부분집합·`--json` dow 필드·기본 JSON
      미포함·`--hours --dow` 병용·help/completion `--dow` 노출 검증. branch `claude/wizardly-pascal-p55s50`)
