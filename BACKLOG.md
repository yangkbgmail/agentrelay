...
- [x] 👷 `agentrelay stats --weekday` — 한 주 중 어느 UTC 요일에 rate-limit이 몰리는지 요일(Mon–Sun) 분포 히스토그램.
      (완료 — 세션 58의 `--hours`(시간-of-day)와 세션 58이 "다음 할 일"로 명시한 후속. `--hours`가 하루 중
      시간 축을 준다면 `--weekday`는 한 주 중 요일 축을 준다("나는 주로 평일에 throttle된다"). core `stats.ts`에
      순수 `computeWeekdayDistribution(jobs)` + `WeekdayActivity`(weekday 0=Mon…6=Sun ISO 순서, name, count) +
      `WEEKDAY_NAMES` 상수 신설 — 각 잡의 `createdAt`을 UTC 요일로 버킷팅해 **모든 주에 걸쳐** 집계, 항상 정확히
      7 슬롯(Mon→Sun) zero-fill, `createdAt` 누락/파싱불가는 스킵. `getUTCDay()`(0=일)를 `(d+6)%7`로 ISO 순서
      (0=월)로 시프트해 Mon→Sun로 읽힘. `--hours`처럼 창도 시계도 불필요(요일은 타임스탬프의 절대 속성). CLI
      `stats.ts`에 순수 `renderWeekday`(7행 ASCII 막대, 최다 요일에 스케일, 0요일은 dim 베이스라인 점, `Mon`…`Sun`
      라벨 + 합계 푸터)·`renderStatsJson`에 옵셔널 `weekday` 필드(요청 시에만 방출, 기존 JSON shape 불변). `cli.ts`
      `stats`에 `--weekday` 배선 — 일회성·`--json`·`--watch` 세 뷰 모두 적용, 기존 스코프 필터(--status/--tool/
      --project/--since/--until)·`--trend`·`--hours`와 조합 가능. 새 파서/스케줄러 로직 0줄. core stats +6 + cli
      stats +5(=37) 신규 테스트, 실제 빌드 CLI e2e로 요일 버킷팅(Mon=2/Sat=1/Sun=1)·최다요일 풀바·스코프 부분집합·
      `--json` weekday 필드·기본 JSON 미포함·help/completion `--weekday` 노출 검증. branch `claude/wizardly-pascal-z0vmv6`)
