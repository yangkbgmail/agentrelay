...
- [x] 👷 `agentrelay tools --watch [seconds]` · `agentrelay projects --watch [seconds]` — 툴/프로젝트
      인덱스를 라이브로 갱신(다음 리셋 카운트다운이 째깍째깍). 세션 52·53의 `--watch` 확장을 마지막
      두 인덱스 커맨드까지 완성해 status/upcoming/overdue와 동일 라이브 뷰 제공.
      ...
      `renderProjects` 본문). `cli.ts`에 세션 52가 추출한 공용 `startWatchLoop` 재사용 `runToolsWatch`/
      `runProjectsWatch`(매 프레임 스토어 재읽기·스코프 재적용·`summarizeTools`/`summarizeProjects`를
      fresh `now`로 재계산 → 리셋 카운트다운 live·화면 clear). 두 커맨드에 `-w, --watch [seconds]` 배선:
      scope 검증을 먼저 통과시켜 잘못된 값은 watch 전에 exit 1, `--json`이 `--watch`보다 우선, 인터벌
      기본 2s, completion 자동 포함. 새 파서/스케줄러/core 로직 0줄. cli tools/projects watch-frame 각
      3케이스 신규(8→11), 실제 빌드 CLI e2e로 화면 clear·라이브 배너·카운트다운·--json 우선·bad scope
      exit 1 검증. branch `claude/wizardly-pascal-8cr33j`)
