...
- [x] 👷 `agentrelay overdue --watch [seconds]` / `agentrelay tools --watch [seconds]` — `upcoming --watch`
      (세션 52)를 지연 진단 리포트와 툴 인덱스로 확장, 공용 `startWatchLoop` 재사용.
      (완료 — 세션 49·50·51·52가 "다음 할 일"로 반복 지목한 인접 항목. CLI `overdue.ts`에 순수
      `renderOverdueWatchFrame(report, storePath, intervalMs, now, scopeNote?)`, `tools.ts`에
      `renderToolsWatchFrame(summary, ...)` 신설 — 둘 다 `status`/`upcoming` watch와 동일 title/meta
      블록(라이브 배너·타임스탬프·스토어 경로) + 항상 컬러인 본문. `cli.ts`에 `runOverdueWatch`
      (매 프레임 스토어 재읽기·스코프+grace+limit 재적용·리포트 재구성·화면 clear)·`runToolsWatch`
      (요약 재롤·화면 clear)를 세션 52의 공용 `startWatchLoop`로 배선. `overdue`·`tools` 커맨드에
      `-w, --watch [seconds]` 옵션 추가: limit/grace/scope 검증을 **먼저** 통과시켜 잘못된 값은 watch
      전에 exit 1, `--json`이 `--watch`보다 우선(일회성 기계 덤프), 인터벌 기본 2s, completion 자동
      포함. 새 파서/스케줄러/core 로직 0줄. cli overdue watch-frame 3 + tools watch-frame 3 신규
      테스트, 실제 빌드 CLI e2e로 화면 clear·라이브 배너·카운트다운·--json 우선·--limit 0/--tool
      nope→exit 1·completion 검증. branch `claude/wizardly-pascal-y9a1ei`)
