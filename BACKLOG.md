...
- [x] 👷 `agentrelay overdue --watch [seconds]` — 지연(overdue) 리포트를 라이브로 갱신(overdue 스팬이
      커지고 새 잡이 마감을 넘는 걸 실시간으로 감시). 세션 52 `upcoming --watch`의 공용 `startWatchLoop`
      재사용.
      (완료 — CLI `overdue.ts`에 순수 `renderOverdueWatchFrame(report, storePath, intervalMs, now,
      scopeNote?)` 신설(`upcoming`/`status` watch와 동일 title/meta 블록 + 항상 컬러인 `renderOverdue`
      본문). `cli.ts`에 `runOverdueWatch`(매 프레임 스토어 재읽기·스코프 재적용·리포트 재계산·화면 clear)
      추가, 세션 52의 공용 `startWatchLoop` 재사용. `overdue`에 `-w, --watch [seconds]` 배선: limit/grace/
      scope 검증을 먼저 통과시켜 잘못된 값은 watch 전에 exit 1, `--json`이 `--watch`보다 우선, 인터벌 기본
      2s. 스코프 창 경계는 시작 시 고정 epoch-ms지만 라이브 쓰기·새 overdue 잡은 계속 반영. completion
      자동 포함. 새 파서/스케줄러/core 로직 0줄. cli overdue watch-frame 3케이스 신규, 실제 빌드 CLI e2e로
      화면 clear·라이브 배너·커지는 스팬·--json 우선·--limit 0 exit 1·help/completion 노출 검증.
      branch `claude/wizardly-pascal-nryfhc`)
