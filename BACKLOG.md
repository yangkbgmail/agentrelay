...
- [x] 👷 `agentrelay overdue|tools|projects --watch [seconds]` — 세션 52 `upcoming --watch`의 후속:
      같은 라이브 갱신 뷰를 지연 진단(`overdue`)·툴 인덱스(`tools`)·프로젝트 인덱스(`projects`)로 확장.
      (완료 — CLI `overdue.ts`/`tools.ts`/`projects.ts`에 순수 `render*WatchFrame(…, storePath, intervalMs,
      now, scopeNote?)` 신설(`renderUpcomingWatchFrame`와 동일 title/meta 블록 + 항상 컬러 본문). `cli.ts`에
      `runOverdueWatch`/`runToolsWatch`/`runProjectsWatch`(세션 52 공용 `startWatchLoop` 재사용): 매 프레임
      스토어 재읽기·스코프 재적용·리포트/서머리 재구성·화면 clear, overdue는 grace/limit도 재적용, 윈도우
      경계는 시작 시 고정 epoch-ms. 세 커맨드에 `-w, --watch [seconds]` 배선: 검증(limit/grace/scope) 먼저
      통과 → 잘못된 값은 watch 전 exit 1, `--json`이 `--watch`보다 우선, 인터벌 기본 2s, completion 자동
      포함. 새 파서/스케줄러/core 로직 0줄. overdue 9→12·tools 8→11·projects 8→11(watch-frame 각 3케이스
      신규), 실제 빌드 CLI e2e로 화면 clear·라이브 배너·카운트다운/overdue-by·--json 우선·에러 exit 1·
      completion 검증. branch `claude/wizardly-pascal-qarp9b`)
