...
describe("renderTools", () => {
  ...
  it("shows the no-match message for an empty scoped subset", () => {
    const out = renderTools(summarizeTools([]), { scopeNote: "status=queued" });
  ...
  it("lists the distinct binaries spawned under a tool", () => {
    const out = renderTools(
      summarizeTools([
    ...
    expect(out).toContain("binaries: aider, cursor");
  });
  ...
  it("shows (idle) for a tool with only terminal jobs and no reset", () => {
    const out = renderTools(summarizeTools([job({ tool: "claude-code", status: "completed" })]), { now: NOW });
    expect(out).toContain("(idle)");
  });

  it("echoes an active scope note once at the top", () => {
    const out = renderTools(summarizeTools([job()]), { scopeNote: "status=completed", now: NOW });
...
describe("renderToolsJson", () => {
  it("wraps the summary in a stable envelope", () => {
    const summary = summarizeTools([job({ tool: "codex-cli", status: "queued", command: ["codex", "run"] })]);
    ...
    expect(parsed.summary.tools[0].tool).toBe("codex-cli");
    expect(parsed.summary.tools[0].active).toBe(1);
    expect(parsed.summary.tools[0].binaries).toEqual(["codex"]);
  });
