...
describe("renderToolsWatchFrame", () => {
  it("prepends a live title with the store path, timestamp, and interval", () => {
    const summary = summarizeTools([
      job({ tool: "claude-code", status: "waiting_for_reset", resetAt: "2026-07-12T13:00:00.000Z" }),
    ]);
  ...
  it("embeds the colored index with a live reset countdown", () => {
    const summary = summarizeTools([
      job({ tool: "claude-code", status: "waiting_for_reset", resetAt: "2026-07-12T13:30:00.000Z" }),
    ]);
