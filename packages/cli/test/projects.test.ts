...
describe("renderProjectsWatchFrame", () => {
  it("prepends a live title with the store path, timestamp, and interval", () => {
    const summary = summarizeProjects([job({ project: "web", status: "waiting_for_reset", resetAt: at(60) })]);
  ...
  it("embeds the colored body with the live reset countdown", () => {
    const summary = summarizeProjects([job({ project: "web", status: "waiting_for_reset", resetAt: at(90 * 60) })]);
