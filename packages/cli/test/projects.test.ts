...
describe("renderProjectsWatchFrame", () => {
  it("prepends a live title with the store path, timestamp, and interval", () => {
    const out = renderProjectsWatchFrame(summarizeProjects([job({ project: "web" })]), "/tmp/jobs.json", 5000, NOW);
    ...
    // Then a blank line, then the summary body.
    expect(lines[2]).toBe("");
    expect(out).toContain("project(s)");
  });

  it("embeds the colored summary body with a live reset countdown", () => {
