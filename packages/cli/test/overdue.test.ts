...
describe("renderOverdueWatchFrame", () => {
  it("prepends a live title with the store path, timestamp, and interval", () => {
    const r = report([job({ resetAt: at(-90 * 60_000) })]);
    const out = renderOverdueWatchFrame(r, "/tmp/jobs.json", 5000, NOW);
    const lines = out.split("\n");
  ...
  it("embeds the colored overdue body with its aging span", () => {
    const r = report([job({ resetAt: at(-90 * 60_000) })]);
    const out = renderOverdueWatchFrame(r, "/tmp/jobs.json", 2000, NOW);
