...
describe("renderOverdue", () => {
  it("shows the all-clear message when nothing is overdue", () => {
    expect(renderOverdue(report([]))).toBe(NO_OVERDUE_MESSAGE);
  });

  it("appends the scope note to the all-clear message when scoped", () => {
  ...
  it("renders one row per overdue job, worst first, with positions", () => {
    const out = renderOverdue(
      report([
        job({ id: "less0000", project: "less-app", resetAt: at(-30 * 60_000) }),
        job({ id: "worst000", project: "worst-app", resetAt: at(-3 * 3_600_000) }),
      ])
    );
    expect(out).toContain("PROJECT");
    expect(out.indexOf("worst-app")).toBeLessThan(out.indexOf("less-app"));
    expect(out).toContain("1  ");
    expect(out).toContain("2  ");
  });

  it("reuses the shared duration format for the overdue amount", () => {
    const out = renderOverdue(report([job({ resetAt: at(-90 * 60_000) })]));
    expect(out).toContain("1h 30m");
  });

  it("footer reports totals, the grace window, and hidden rows under a limit", () => {
    const jobs = [
  ...
  it("uses the singular 'job' in the footer for exactly one", () => {
...
describe("renderOverdueJson", () => {
  it("emits the store path, report, and honest totals", () => {
    ...
    const parsed = JSON.parse(
      renderOverdueJson({ storePath: "/tmp/jobs.json", generatedAt: "2026-07-30T13:00:00.000Z", report: r })
    );
    expect(parsed.storePath).toBe("/tmp/jobs.json");
    expect(parsed.generatedAt).toBe("2026-07-30T13:00:00.000Z");
    ...
    expect(typeof parsed.report.graceMs).toBe("number");
  });
