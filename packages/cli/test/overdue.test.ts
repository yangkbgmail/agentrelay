...
describe("renderOverdueWatchFrame", () => {
  it("prepends a live title with the store path, timestamp, and interval", () => {
    const out = renderOverdueWatchFrame(report([job({ resetAt: at(-2 * 3_600_000) })]), "/tmp/jobs.json", 5000, NOW);
    const lines = out.split("\n");
  ...
  it("embeds the colored overdue body with the overdue span", () => {
    const out = renderOverdueWatchFrame(report([job({ resetAt: at(-2 * 3_600_000) })]), "/tmp/jobs.json", 2000, NOW);
  ...
  it("carries the scope note into the frame body when nothing is overdue", () => {
