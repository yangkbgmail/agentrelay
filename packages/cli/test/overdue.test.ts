...
import { formatOverdueAge, NO_OVERDUE_MESSAGE, renderOverdue, renderOverdueJson } from "../src/overdue.js";
...
describe("formatOverdueAge", () => {
  it("formats sub-minute ages in seconds", () => {
    expect(formatOverdueAge(0)).toBe("0s");
    expect(formatOverdueAge(10_000)).toBe("10s");
    expect(formatOverdueAge(59_000)).toBe("59s");
  });

  it("formats minutes, hours, and days like the countdown", () => {
    expect(formatOverdueAge(5 * 60_000)).toBe("5m");
    expect(formatOverdueAge(90 * 60_000)).toBe("1h 30m");
    expect(formatOverdueAge(26 * 3_600_000)).toBe("1d 2h");
  });
});
...
describe("renderOverdue", () => {
  ...
  it("mentions the grace window on the empty message when one is set", () => {
    const out = renderOverdue(report([job({ resetAt: at(-10_000) })], { graceMs: 60_000 }));
    expect(out).toContain(NO_OVERDUE_MESSAGE);
  ...
  it("renders one row per overdue job, worst first, with positions", () => {
    ...
    expect(out.indexOf("worst-app")).toBeLessThan(out.indexOf("recent-app"));
    expect(out).toContain("1  ");
    expect(out).toContain("2  ");
  });

  it("shows how long a job has been overdue", () => {
    const out = renderOverdue(report([job({ resetAt: at(-90 * 60_000) })]));
    expect(out).toContain("1h 30m");
  });

  it("footer reports totals, the worst age, and hidden rows under a limit", () => {
    const jobs = [
      job({ id: "j1", resetAt: at(-5 * 3_600_000) }),
    ...
    expect(out).toContain("worst 5h 0m late");
