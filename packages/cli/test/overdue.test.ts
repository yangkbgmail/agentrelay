...
function report(jobs: RelayJob[], options?: { minOverdueMs?: number; limit?: number }): OverdueReport {
  ...
  it("renders one row per overdue job, most overdue first, with positions", () => {
    const out = renderOverdue(
      report([
        ...
        job({ id: "stale000", project: "stale-app", resetAt: at(-3 * 3_600_000) }),
      ])
    );
    const lines = out.split("\n");
    expect(lines[0]).toContain("OVERDUE BY");
    expect(out.indexOf("stale-app")).toBeLessThan(out.indexOf("recent-app"));
