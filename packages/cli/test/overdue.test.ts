...
describe("renderOverdue", () => {
  ...
  it("renders one row per overdue job, worst first, with positions", () => {
    const out = renderOverdue(
      report([
        job({ id: "recent00", project: "recent-app", resetAt: at(-3 * 60_000) }),
        job({ id: "worst000", project: "worst-app", resetAt: at(-2 * 3_600_000) }),
      ]),
      { now: NOW }
    );
    const lines = out.split("\n");
    expect(lines[0]).toContain("OVERDUE BY");
    // worst-app (2h past due) ranks above recent-app (3m).
    expect(out.indexOf("worst-app")).toBeLessThan(out.indexOf("recent-app"));
    expect(out).toContain("1  ");
    expect(out).toContain("2  ");
  });

  it("formats the overdue duration", () => {
    const out = renderOverdue(report([job({ resetAt: at(-90 * 60_000) })]), { now: NOW });
    expect(out).toContain("1h 30m");
  });

  it("prints the daemon hint when jobs are overdue", () => {
    const out = renderOverdue(report([job({ resetAt: at(-30 * 60_000) })]), { now: NOW });
    expect(out).toContain("agentrelay daemon");
  });

  it("footer reports totals, worst lag, and hidden rows under a limit", () => {
    const jobs = [
    ...
    expect(out).toContain("worst 4h 0m past due");
...
describe("renderOverdueJson", () => {
  it("emits the store path, report, and honest totals including graceMs", () => {
    const r = report([job({ id: "j1", resetAt: at(-2 * 3_600_000) })], { limit: 1, graceMs: 60_000 });
