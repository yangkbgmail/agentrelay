...
function report(jobs: RelayJob[], opts?: { minOverdueMs?: number; limit?: number }): OverdueReport {
...
describe("renderOverdue", () => {
  ...
  it("notes the grace period on the empty message", () => {
    const out = renderOverdue(report([job({ resetAt: at(-60_000) })], { minOverdueMs: 10 * 60_000 }));
    expect(out).toContain(NO_OVERDUE_MESSAGE);
  ...
  it("renders one row per overdue job, most-overdue first, with positions", () => {
    const out = renderOverdue(
      report([
        ...
        job({ id: "stuck000", project: "stuck-app", resetAt: at(-5 * 3_600_000) }),
      ])
    );
  ...
  it("footer reports the total and hidden rows under a limit", () => {
    const jobs = [
