...
function report(jobs: RelayJob[], opts?: { limit?: number; graceMs?: number }): OverdueReport {
...
describe("renderOverdue", () => {
  ...
  it("renders one row per overdue job, most overdue first, with ranks", () => {
    const out = renderOverdue(
      report([
        job({ id: "barely00", project: "barely-app", resetAt: at(-30 * 60_000) }),
        job({ id: "worst000", project: "worst-app", resetAt: at(-5 * 60 * 60_000) }),
      ])
    );
    const lines = out.split("\n");
    expect(lines[0]).toContain("PROJECT");
    expect(out.indexOf("worst-app")).toBeLessThan(out.indexOf("barely-app"));
    expect(out).toContain("1  ");
    expect(out).toContain("2  ");
  });

  it("shows the lateness as a compact duration", () => {
  ...
  it("footer reports totals, worst lateness, and hidden rows under a limit", () => {
    const jobs = [
      job({ id: "j1", resetAt: at(-5 * 60 * 60_000) }),
  ...
  it("uses the singular form for exactly one overdue job", () => {
    const out = renderOverdue(report([job({ resetAt: at(-60 * 60_000) })]));
