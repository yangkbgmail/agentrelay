...
function job(overrides: Partial<RelayJob> = {}): RelayJob {
  return {
    ...
    resetAt: at(-60 * 60_000), // one hour overdue by default
...
function report(jobs: RelayJob[], options?: { limit?: number; graceMs?: number }): OverdueReport {
...
describe("renderOverdue", () => {
  ...
  it("shows the grace note when a grace window is applied", () => {
    const out = renderOverdue(report([job({ resetAt: at(-60 * 60_000) })], { graceMs: 5 * 60_000 }));
    expect(out).toContain("grace: 5m");
  });

  it("footer reports totals, the worst span, and hidden rows under a limit", () => {
    const jobs = [
    ...
    expect(out).toContain("worst 3h 0m");
...
describe("renderOverdueJson", () => {
  it("emits the store path, report, and honest totals", () => {
    ...
    expect(parsed.report.maxOverdueMs).toBe(60 * 60_000);
  });
