...
describe("renderOverdueWatchFrame", () => {
  it("includes the live title, interval, store path, timestamp and the table", () => {
    const frame = renderOverdueWatchFrame(report([job({ id: "j1", project: "web" })]), "/tmp/store.json", 5000, NOW);
  ...
  it("passes the scope note through to the empty (keeping-up) frame", () => {
    const frame = renderOverdueWatchFrame(report([]), "/tmp/store.json", 2000, NOW, "project=ghost");
