...
function report(jobs: RelayJob[], options: { thresholdMs?: number; limit?: number } = {}): OverdueReport {
...
describe("renderOverdue", () => {
  ...
  it("appends scope and threshold notes to the empty message", () => {
    const out = renderOverdue(report([], { thresholdMs: 15 * 60_000 }), { scopeNote: "project=ghost" });
    expect(out).toContain(NO_OVERDUE_MESSAGE);
    expect(out).toContain("scope: project=ghost");
    expect(out).toContain("threshold 15m 0s");
  });
  ...
  it("renders one row per overdue job, most-overdue first, with ranks", () => {
    const out = renderOverdue(
      report([
        ...
        job({ id: "stuck000", project: "stuck-app", resetAt: at(-4 * 3_600_000) }),
      ])
    );
  ...
  it("footer reports totals, the worst overdue span, and hidden rows under a limit", () => {
    const jobs = [
...
describe("renderOverdueJson", () => {
  it("emits the store path, report, and honest totals", () => {
    ...
    expect(parsed.report.maxOverdueMs).toBe(2 * 3_600_000);
  });
