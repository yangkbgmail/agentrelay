...
describe("renderOverdueWatchFrame", () => {
  it("prepends a live title with the store path, timestamp, and interval", () => {
    const out = renderOverdueWatchFrame(
      report([job({ resetAt: at(-60 * 60_000) })]),
      "/tmp/jobs.json",
      5000,
      undefined,
      NOW
  ...
  it("embeds the colored report body with the aging span", () => {
    const out = renderOverdueWatchFrame(
      report([job({ resetAt: at(-90 * 60_000) })]),
      "/tmp/jobs.json",
      2000,
      undefined,
      NOW
  ...
  it("carries the scope note into the frame body", () => {
    const out = renderOverdueWatchFrame(report([]), "/tmp/jobs.json", 2000, "project=ghost", NOW);
    expect(out).toContain(NO_OVERDUE_MESSAGE);
    expect(out).toContain("scope: project=ghost");
  });
});
