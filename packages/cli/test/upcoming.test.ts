...
describe("renderUpcomingWatchFrame", () => {
  it("prepends a live title/meta block, then the colored timeline", () => {
    const out = renderUpcomingWatchFrame(
      timeline([job({ id: "j1", resetAt: at(90 * 60_000) })]),
      "/tmp/jobs.json",
      2000,
      NOW
    );
  ...
  it("passes the scope note through to the framed table", () => {
    const out = renderUpcomingWatchFrame(timeline([]), "/tmp/jobs.json", 2000, NOW, "project=demo");
    expect(out).toContain(NO_UPCOMING_MESSAGE);
    expect(out).toContain("scope: project=demo");
  });

  it("emits ANSI escapes (the live view is always colored)", () => {
    const out = renderUpcomingWatchFrame(timeline([job()]), "/tmp/jobs.json", 2000, NOW);
