...
describe("renderUpcomingWatchFrame", () => {
  it("wraps the timeline table with a live banner, timestamp, and store path", () => {
    const out = renderUpcomingWatchFrame(
      timeline([job({ project: "demo", resetAt: at(90 * 60_000) })]),
      "/s/jobs.json",
      5000,
      {
        now: NOW,
    ...
    // Timestamp line carries the injected now and the store path.
    expect(lines[1]).toContain("2026-07-30 10:00:00");
    expect(lines[1]).toContain("/s/jobs.json");
    // The table body is still present (with its live countdown).
    expect(out).toContain("demo");
    expect(out).toContain("1h 30m");
  });

  it("passes the scope note through into the frame", () => {
    const out = renderUpcomingWatchFrame(timeline([]), "/s/jobs.json", 2000, {
      now: NOW,
