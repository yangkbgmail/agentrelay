...
// already knows the snapshot/outcome); this keeps the JSON shape consistent with
// `wait --json` / `eta --json` and unit-testable without a store or a clock.
 ...
 * code a script should branch on, and the last snapshot seen (final counts on a
 * clean drain, still-active counts on a timeout).
 */
