...
export async function runCommand(options: RunOptions): Promise<RunResult> {
  ...
  queue.markWaitingForReset(
    ...
    // Persist the output that triggered the limit so `agentrelay show`/`tail`
    // have context on this job *before* any resume has run and captured its own.
    boundOutputTail(output)
  );
  queue.close();
