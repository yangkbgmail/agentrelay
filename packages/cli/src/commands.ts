...
export function runDoctor(options: DoctorOptions = {}): DiagnosticReport {
  ...
  // --- reset-horizon facts. A job queued while the guard was off (or imported)
  // can carry a reset time days/years out and wait silently. Flag any active job
  // whose resetAt is beyond the effective horizon so the user can cancel it.
  const nowMs = options.nowMs ?? Date.now();
