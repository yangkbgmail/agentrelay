...
export function runDoctor(options: DoctorOptions = {}): DiagnosticReport {
  ...
  // --- reset-horizon facts. A waiting job whose reset lands implausibly far in
  // the future (a mis-parse, or a leftover from before the horizon guard) waits
  // silently for days/years. Resolve the same horizon the parser uses and flag
  // any parked job beyond it.
  const nowMs = options.nowMs ?? Date.now();
  const horizonMs = maxResetHorizonMsFromEnv(env);
  const { farFuture, checkedCount } = selectFarFutureResets(jobs, { nowMs, horizonMs });
  ...
  return runDiagnostics({
    ...
    reset: { horizonMs, farFuture, checkedCount },
  });
}
