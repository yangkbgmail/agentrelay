 ...
 * (matching the shape of `status`/`tools`/`summary --watch`) plus the colored
 * one-liner. Separated out so the watch loop in cli.ts only has to clear the
 * screen and print this. The countdown ticks down — and flips to "due now" the
 * instant the reset passes — because the loop re-reads the store and reselects
 * the next resume with a fresh `now` each pass.
 */
