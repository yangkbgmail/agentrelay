 ...
 * shape of `status --watch`, `upcoming --watch`, and `overdue --watch`) plus the
 * colored one-liner. Separated out so the watch loop only has to clear the
 * screen and print this. The countdown ticks down in place because the loop
 * re-reads the store and re-selects the next resume with a fresh `now` each pass.
 */
