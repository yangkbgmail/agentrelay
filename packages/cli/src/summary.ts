 ...
 * (matching the shape of `status`/`tools`/`stats --watch`) plus the compact
 ...
 * When a scope filter is active, an optional `scopeNote` line records it so the
 * live view shows which subset the counts cover (mirrors the other watch views).
 * Pure: `now` is injected, never read from an ambient clock here.
 */
