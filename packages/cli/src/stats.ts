 ...
 */
export function renderWeekdays(days: WeekdayActivity[], options: { color?: boolean } = {}): string {
  const color = options.color ?? false;
  const b = (s: string) => (color ? `${BOLD}${s}${RESET}` : s);
  const d = (s: string) => (color ? `${DIM}${s}${RESET}` : s);
  ...
  const max = days.reduce((m, w) => Math.max(m, w.count), 0);
  const total = days.reduce((sum, w) => sum + w.count, 0);
  for (const { label, count } of days) {
