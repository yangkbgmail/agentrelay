...
export function renderTrend(trend: DailyActivity[], options: { color?: boolean; tzLabel?: string } = {}): string {
  ...
  const lines: string[] = [b("activity") + d(` (jobs created per day, ${tzLabel})`)];
  if (trend.length === 0) {
    lines.push("  none");
    return lines.join("\n");
  }
...
export function renderStatsJson(
  ...
  // `tz` names the offset the histograms are bucketed in; omit it for plain UTC
  // so the default shape is unchanged and it only appears when it actually
  // matters (a non-UTC --tz was applied to a histogram).
  const tz = options.tz && options.tz !== "UTC" ? options.tz : undefined;
  return JSON.stringify({ storePath, generatedAt, scope, tz, trend, hours, weekday, stats }, null, 2);
}
