...
export function renderStatsJson(
  ...
  // `tz` names the timezone the histograms are bucketed in; emit it only when a
  // histogram is present so the default JSON shape is unchanged for consumers
  // that don't ask for one.
  const tz = trend !== undefined || hours !== undefined || weekday !== undefined ? options.tz : undefined;
