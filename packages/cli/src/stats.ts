...
/**
 * Renders a UTC hour-of-day activity histogram (jobs created per hour, summed
 * across all days) as a compact ASCII bar chart — 24 rows, "00".."23". Bars are
 * scaled to the busiest hour so the daily rhythm is readable regardless of
 * absolute volume; a zero hour shows a dim baseline dot. Pure: no I/O, no clock.
...
export function renderHourly(hourly: HourlyActivity[], options: { color?: boolean } = {}): string {
  ...
  const lines: string[] = [b("activity by hour") + d(" (jobs created per hour of day, UTC)")];
  if (hourly.length === 0) {
    lines.push("  none");
    return lines.join("\n");
  }
