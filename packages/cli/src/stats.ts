...
/**
 * Renders an hour-of-day activity histogram (jobs created per UTC hour, 0–23) as
 * a compact ASCII bar chart, folding every day onto a single 24-hour clock so
 * the daily rhythm of rate-limits is visible. Bars are scaled to the busiest
 * hour; a zero hour shows a dim baseline dot. Pure: no I/O, no clock. Callers
...
export function renderHourly(hourly: HourlyActivity[], options: { color?: boolean } = {}): string {
  ...
  // The peak hour(s) get a marker so the busiest time of day pops out at a glance.
  for (const { hour, count } of hourly) {
    ...
    const peak = count > 0 && count === max ? ` ${d("← peak")}` : "";
...
export function renderStatsJson(
  ...
  // entirely otherwise so the default JSON shape is unchanged for existing
  // consumers.
