...
export function renderStatsJson(
  ...
  // consumers. `timeZone` rides along only when `--tz` bucketed the histograms,
  // so scripts can tell UTC output apart from a zoned run.
  ...
  return JSON.stringify({ storePath, generatedAt, scope, timeZone, trend, hours, weekday, stats }, null, 2);
}
