...
export function renderStatsJson(
  ...
  // Echo the zone label only when a histogram it applies to is present, so the
  // reader knows which zone `hours`/`weekday` buckets are in (omitted for UTC
  // and when neither histogram was requested — default shape stays unchanged).
  const tz = (hours || weekday) && options.tz ? options.tz : undefined;
  return JSON.stringify({ storePath, generatedAt, scope, trend, hours, weekday, tz, stats }, null, 2);
}
