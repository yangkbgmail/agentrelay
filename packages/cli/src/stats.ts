...
/**
 * Renders an hour-of-day activity histogram (jobs created per UTC hour, every
 * day collapsed onto one 24-hour clock) as a compact ASCII bar chart. Same
 * scaling rules as {@link renderTrend}: bars scale to the busiest hour and any
 * non-zero hour keeps at least one block, a zero hour shows a dim baseline dot.
 * Pure: no I/O, no clock. Callers pass the already-computed buckets.
 */
export function renderHourlyTrend(byHour: HourlyActivity[], options: { color?: boolean } = {}): string {
  const color = options.color ?? false;
  const b = (s: string) => (color ? `${BOLD}${s}${RESET}` : s);
  const d = (s: string) => (color ? `${DIM}${s}${RESET}` : s);
  ...
  const max = byHour.reduce((m, slot) => Math.max(m, slot.count), 0);
  const total = byHour.reduce((sum, slot) => sum + slot.count, 0);
  for (const { hour, count } of byHour) {
    const filled = max === 0 || count === 0 ? 0 : Math.max(1, Math.round((count / max) * TREND_BAR_WIDTH));
    const plain = count === 0 ? "·" : "█".repeat(filled);
    const padded = plain.padEnd(TREND_BAR_WIDTH);
    const shown = count === 0 && color ? padded.replace("·", d("·")) : padded;
    // Zero-pad the hour label ("00".."23") so the bar column stays aligned.
    lines.push(`  ${String(hour).padStart(2, "0")}h  ${shown} ${count}`);
  }
  lines.push(d(`  ${total} job(s) across 24 hour(s)`));
  return lines.join("\n");
}
...
export function renderStatsJson(
  ...
  // entirely otherwise so the default JSON shape is unchanged for consumers.
