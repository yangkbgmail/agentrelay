...
const HOUR_BAR_WIDTH = 24;

/**
 * Renders a UTC hour-of-day activity histogram (jobs created per hour, collapsed
 * across all days) as a compact ASCII bar chart. Bars are scaled to the busiest
 * hour so the shape is readable regardless of absolute volume; a zero hour shows
 * a dim baseline dot. Pure: no I/O, no clock. Callers pass the already-computed
 * activity so it stays testable.
 */
export function renderHourlyActivity(hourly: HourlyActivity[], options: { color?: boolean } = {}): string {
  const color = options.color ?? false;
  const b = (s: string) => (color ? `${BOLD}${s}${RESET}` : s);
  const d = (s: string) => (color ? `${DIM}${s}${RESET}` : s);
  ...
  for (const { hour, count } of hourly) {
    ...
    const filled = max === 0 || count === 0 ? 0 : Math.max(1, Math.round((count / max) * HOUR_BAR_WIDTH));
    const plain = count === 0 ? "·" : "█".repeat(filled);
    const padded = plain.padEnd(HOUR_BAR_WIDTH);
    const shown = count === 0 && color ? padded.replace("·", d("·")) : padded;
    const label = `${String(hour).padStart(2, "0")}:00`;
    lines.push(`  ${label}  ${shown} ${count}`);
  }
  lines.push(d(`  ${total} job(s) across 24h`));
  return lines.join("\n");
}
