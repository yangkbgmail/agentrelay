 ...
 * as a compact ASCII bar chart. Bars scale to the busiest hour so the daily
 * shape reads regardless of absolute volume; a zero hour shows a dim baseline
 * dot. Pure: no I/O, no clock. Callers pass the already-computed hourly buckets.
 */
export function renderHourly(hourly: HourlyActivity[], options: { color?: boolean } = {}): string {
  ...
  for (const { hour, count } of hourly) {
    ...
    const padded = plain.padEnd(TREND_BAR_WIDTH);
    const shown = count === 0 && color ? padded.replace("·", d("·")) : padded;
    // Two-digit zero-padded hour with a trailing "h" reads unambiguously (03h).
