 ...
 * across all days) as a compact ASCII bar chart with a "HHh" label per row.
 * Bars are scaled to the busiest hour so the shape is readable regardless of
...
export function renderHourly(hourly: HourlyActivity[], options: { color?: boolean } = {}): string {
  ...
  for (const { hour, count } of hourly) {
    ...
    const padded = plain.padEnd(TREND_BAR_WIDTH);
    const shown = count === 0 && color ? padded.replace("·", d("·")) : padded;
    // Zero-pad the hour to two digits so the label column stays aligned.
...
export function renderStatsJson(
  ...
  // Only emit `trend`/`hourly` when the corresponding flag was requested; omit
