...
/** Max width (chars) of a full-scale bar in the by-hour histogram. */
const HOUR_BAR_WIDTH = 24;

/**
 * Renders an hour-of-day activity histogram (jobs created per UTC hour,
 * aggregated across all days) as a compact ASCII bar chart. Bars scale to the
 * busiest hour so the daily rhythm is readable regardless of volume; a quiet
 * hour shows a dim baseline dot. Pure: no I/O, no clock. Callers pass the
...
export function renderHourly(hourly: HourlyActivity[], options: { color?: boolean } = {}): string {
  ...
  for (const { hour, count } of hourly) {
    ...
    const shown = count === 0 && color ? padded.replace("·", d("·")) : padded;
    // "00:00".."23:00" — a fixed-width label keeps the bar column aligned.
