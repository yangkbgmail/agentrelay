...
/**
 * ASCII bar chart of the 24-hour job-creation distribution (UTC). Bars are
 * scaled to the busiest hour so the shape is readable regardless of absolute
 * volume; a zero hour shows a dim baseline dot. Pure: no I/O, no clock. Callers
 * pass the already-computed distribution so it stays testable.
 */
export function renderHourly(hours: HourlyActivity[], options: { color?: boolean } = {}): string {
  const color = options.color ?? false;
  const b = (s: string) => (color ? `${BOLD}${s}${RESET}` : s);
  const d = (s: string) => (color ? `${DIM}${s}${RESET}` : s);

  const lines: string[] = [b("hourly") + d(" (jobs created per hour of day, UTC)")];
