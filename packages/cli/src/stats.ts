...
/** Short weekday labels, Monday-first — index matches `WeekdayActivity.weekday`. */
...
/**
 * Renders a day-of-week activity histogram (jobs created per UTC weekday, Mon–Sun,
 * aggregated across all weeks) as a compact ASCII bar chart. The calendar-day
 * companion to {@link renderHours}: same bar-scaling convention (scaled to the
 * busiest day, at least one block for any non-zero day, a dim baseline dot for a
 * zero day). Pure: no I/O, no clock. Callers pass the already-computed
 * distribution so it stays testable.
 */
export function renderWeekday(weekdays: WeekdayActivity[], options: { color?: boolean } = {}): string {
  ...
  for (const { weekday, count } of weekdays) {
    ...
    const shown = count === 0 && color ? padded.replace("·", d("·")) : padded;
    const label = WEEKDAY_NAMES[weekday] ?? String(weekday);
