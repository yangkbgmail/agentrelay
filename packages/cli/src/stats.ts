...
/** Three-letter labels for weekday 0 (Sunday) through 6 (Saturday). */
const WEEKDAY_LABELS = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];

/**
 * Renders a day-of-week activity histogram (jobs created per UTC weekday,
 * Sunday through Saturday, aggregated across all weeks) as a compact ASCII bar
 * chart. Mirrors {@link renderHours}: bars scale to the busiest weekday so the
 * shape reads regardless of absolute volume; a zero weekday shows a dim baseline
 * dot. Pure: no I/O, no clock. Callers pass the already-computed distribution so
 * it stays testable.
 */
export function renderWeekday(weekday: WeekdayActivity[], options: { color?: boolean } = {}): string {
  ...
  const max = weekday.reduce((m, w) => Math.max(m, w.count), 0);
  const total = weekday.reduce((sum, w) => sum + w.count, 0);
  for (const { weekday: day, count } of weekday) {
    // Scale each bar to the busiest weekday; guarantee at least one block for any
    // non-zero weekday so small counts don't vanish next to a spike.
    const filled = max === 0 || count === 0 ? 0 : Math.max(1, Math.round((count / max) * WEEKDAY_BAR_WIDTH));
    // Pad the plain bar to a fixed width so the count column stays aligned; a
    // zero weekday shows a single baseline dot (dimmed only when color is on).
    const plain = count === 0 ? "·" : "█".repeat(filled);
    const padded = plain.padEnd(WEEKDAY_BAR_WIDTH);
    const shown = count === 0 && color ? padded.replace("·", d("·")) : padded;
    lines.push(`  ${WEEKDAY_LABELS[day] ?? String(day)}  ${shown} ${count}`);
  }
