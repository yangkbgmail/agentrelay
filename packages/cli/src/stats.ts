...
/**
 * Renders an hour-of-day activity histogram (jobs created per UTC hour, summed
 * across all days) as a compact ASCII bar chart — the daily-rhythm companion to
 * {@link renderTrend}. Bars are scaled to the busiest hour so the shape reads
 * regardless of absolute volume; a zero hour shows a dim baseline dot. Pure: no
 * I/O, no clock. Callers pass the already-computed distribution so it stays
 * testable.
 */
export function renderHourly(hourly: HourlyActivity[], options: { color?: boolean } = {}): string {
  ...
  for (const { hour, count } of hourly) {
    ...
    const padded = plain.padEnd(TREND_BAR_WIDTH);
    const shown = count === 0 && color ? padded.replace("·", d("·")) : padded;
    // Two-digit UTC hour label, e.g. "05:00".
...
export function renderStatsJson(
  ...
  // Only emit `trend`/`hourly` when the flag was requested; omit them entirely
