...
/**
 * Renders a UTC hour-of-day activity histogram (jobs created per hour, 0–23) as
 * a compact ASCII bar chart — the clock-axis complement to {@link renderTrend}.
 * Bars scale to the busiest hour so the shape reads regardless of absolute
 * volume; a quiet hour shows a dim baseline dot. Pure: no I/O, no clock. Callers
 * pass the already-computed 24-slot histogram so it stays testable.
 */
