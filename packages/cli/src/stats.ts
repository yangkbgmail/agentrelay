...
/** Three-letter labels for weekday 0 (Sunday) … 6 (Saturday), matching `getUTCDay`. */
const WEEKDAY_LABELS = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"] as const;

/**
 * Renders a day-of-week activity histogram (jobs created per UTC weekday,
 * Sunday–Saturday, aggregated across all weeks) as a compact ASCII bar chart.
 * Bars scale to the busiest weekday so the shape reads regardless of absolute
...
export function renderStatsJson(
  ...
  options: {
    ...
    weekdays?: WeekdayActivity[] | null;
  } = {}
): string {
  const generatedAt = options.generatedAt ?? new Date().toISOString();
  const scope = options.scope && isJobScopeActive(options.scope) ? options.scope : undefined;
  // Only emit `trend`/`hours`/`weekdays` when the matching flag was requested;
  ...
  const weekdays = options.weekdays ?? undefined;
  return JSON.stringify({ storePath, generatedAt, scope, trend, hours, weekdays, stats }, null, 2);
}
