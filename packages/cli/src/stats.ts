 ...
 * day collapsed onto a single 24-hour clock) as a compact ASCII bar chart. Bars
 * are scaled to the busiest hour so the shape is readable regardless of absolute
 * volume; a zero hour shows a dim baseline dot. The busiest hour is flagged with
 * a `←` marker so the peak reads at a glance. Pure: no I/O, no clock. Callers
...
export function renderHourly(hourly: HourlyActivity[], options: { color?: boolean } = {}): string {
  ...
  const max = hourly.reduce((m, slot) => Math.max(m, slot.count), 0);
  const total = hourly.reduce((sum, slot) => sum + slot.count, 0);
  for (const { hour, count } of hourly) {
    ...
    const shown = count === 0 && color ? padded.replace("·", d("·")) : padded;
    // Flag the peak hour(s) so the busiest window reads without scanning counts.
    const peak = count > 0 && count === max ? ` ${d("←")}` : "";
    const label = `${String(hour).padStart(2, "0")}:00`;
    lines.push(`  ${label}  ${shown} ${count}${peak}`);
  }
  lines.push(d(`  ${total} job(s) over 24 hour(s)`));
  return lines.join("\n");
}
