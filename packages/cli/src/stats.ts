...
export function renderStatsJson(
  ...
  // Echo the histogram offset only when non-zero (a shifted clock is in play),
  // so the default UTC shape is untouched for existing consumers.
  const utcOffsetMinutes =
    options.utcOffsetMinutes !== undefined && options.utcOffsetMinutes !== 0 ? options.utcOffsetMinutes : undefined;
  return JSON.stringify({ storePath, generatedAt, scope, utcOffsetMinutes, trend, hours, weekday, stats }, null, 2);
}
