 ...
 * the shape of `status --watch`, `upcoming --watch`, and `overdue --watch`) plus
 * the colored project index. Separated out so the watch loop only has to clear
 * the screen and print this. The reset countdowns tick down in place because the
 * loop re-reads the store and re-renders with a fresh `now` each pass.
 */
