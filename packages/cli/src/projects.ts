 ...
 * the shape of `status`/`upcoming`/`overdue --watch`) plus the colored
 * per-project table. Separated out so the watch loop only has to clear the
 * screen and print this. The reset countdowns tick down in place because the
 * loop re-reads the store and rebuilds the summary with a fresh `now` each pass.
 */
