...
/**
 * One frame of the live `agentrelay projects --watch` view: a title/header block
 * (matching the shape of `status`/`upcoming`/`overdue`/`tools --watch`) plus the
 * colored project index. Separated out so the watch loop only has to clear the
 * screen and print this. The reset countdowns tick down in place because the loop
