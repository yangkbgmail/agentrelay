 ...
 *   distinct (`show`/`get` load the file themselves to attribute each value).
...
/**
 * The bare value line `config get <key>` prints to stdout — the scriptable
 * payload, so no labels or color. An unset/default value renders as an empty
 * string (a script can test emptiness), a secret is masked unless `showSecrets`.
 */
export function renderConfigGetValue(result: ConfigGetResult, options: { showSecrets?: boolean } = {}): string {
  const resolved = result.resolved;
  if (!resolved || resolved.value === undefined) return "";
  if (resolved.secret && !(options.showSecrets ?? false)) return maskSecret(resolved.value);
