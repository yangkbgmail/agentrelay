...
/**
 * Renders a single `config get` value for shell scripting: the bare effective
 * value on its own line and nothing else, so `STORE=$(agentrelay config get
 * store)` captures exactly the value. An unset key (built-in default applies)
 * prints an empty line. Secrets are masked unless `showSecrets` is set — a
 * script that needs the raw webhook URL opts in explicitly. Pure: no I/O.
 */
export function renderConfigValue(result: ConfigGetResult, options: { showSecrets?: boolean } = {}): string {
  const entry = result.entry;
  if (!entry || entry.value === undefined) return "";
  if (entry.secret && !options.showSecrets) return maskSecret(entry.value);
  return entry.value;
}

/** Machine-readable snapshot for `config get --json` (value + source + provenance). */
export function renderConfigValueJson(
  ...
  const entry = result.entry;
  const value =
    entry && entry.value !== undefined && entry.secret && !options.showSecrets ? maskSecret(entry.value) : entry?.value;
