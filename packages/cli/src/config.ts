 ...
 *   (they load the file themselves to attribute each value).
...
/**
 * Renders a single resolved setting for `config get <key>` as plain text — just
 * the value, so it can be captured with `$(agentrelay config get store)`. A
 * built-in default (nothing set in env or file) prints as an empty string;
 * secret values (webhook URLs/tokens) are masked unless `showSecrets` is set.
 * Pure: no I/O.
 */
export function renderConfigValue(result: ConfigGetResult, options: { showSecrets?: boolean } = {}): string {
  ...
  if (resolved.secret && !options.showSecrets) return maskSecret(resolved.value);
  return resolved.value;
}

/** Machine-readable snapshot for `config get <key> --json` (scripts, jq, tooling). */
export function renderConfigValueJson(result: ConfigGetResult, generatedAt: string = new Date().toISOString()): string {
