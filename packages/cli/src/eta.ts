 ...
 * shape of `status`/`upcoming --watch`) plus the colored catch-up line. Separated
 * out so the watch loop only has to clear the screen and print this. The countdown
 * ticks down in place because the loop recomputes the ETA with a fresh `now` each
 * pass. Unlike the other watch surfaces `eta` has no scope filters, so there is no
 * scope note to thread through.
 */
