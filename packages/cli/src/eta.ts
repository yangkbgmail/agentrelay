 ...
 * shape of `status`/`upcoming --watch`) plus the always-colored catch-up line.
 * Separated out so the watch loop only has to clear the screen and print this.
 * The countdown ticks down in place because the loop re-reads the store and
 * recomputes the ETA with a fresh `now` each pass.
 */
