 ...
 * a running daemon's writes and jobs draining show up automatically) and the
 * `--status`/`--tool`/`--project`/`--since`/`--until` scope is re-applied every
 * frame. The summary is rebuilt with a fresh `now` each pass so the countdowns
 ...
 * Clears the screen and re-renders the per-tool index on an interval so the
 * reset countdowns tick down in place. `listStatus` re-reads the JSON store each
 * pass and the `--status`/`--tool`/`--project`/`--since`/`--until` scope is
 * re-applied every frame; the summary is rebuilt with a fresh `now` each pass.
 * The time-window boundaries stay fixed (absolute epoch-ms from command start).
...
export function buildCli(): Command {
  ...
  program
    ...
    .action((opts: ScopeOpts & { json?: boolean; watch?: string | boolean }) => {
      ...
      // Live view: scope is validated above, so a bad filter still exits 1 rather
      // than spinning a broken watch loop. --json takes precedence over --watch
      // (a one-shot machine dump, not a live TTY view).
      if (opts.watch !== undefined && !opts.json) {
        const parsed = typeof opts.watch === "string" ? Number.parseFloat(opts.watch) : NaN;
        const intervalMs = Number.isFinite(parsed) && parsed > 0 ? Math.round(parsed * 1000) : 2000;
