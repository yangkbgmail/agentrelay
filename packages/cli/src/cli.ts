 ...
 * ETA on an interval so the "queue caught up in …" countdown ticks down in place.
 * Like the other watch loops, `listStatus` re-reads the JSON store each pass (so
 * a running daemon's writes surface automatically — jobs dropping off as they
 * resume, the latest reset moving as new rate-limits are detected) and
 * `computeQueueEta` is recomputed with a fresh `now` each pass so the countdown
 * stays live. Unlike `upcoming`/`overdue`, `eta` has no scope filters, so there
 * is nothing to re-apply. Runs until the process is interrupted (Ctrl-C).
 */
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the catch-up countdown in place (Ctrl-C to exit)")
    .option("--json", "Print as JSON (machine-readable, for scripts/jq)")
    .option(
      "--exit-code",
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # live countdown to catch-up, refreshing every 2s\n" +
    ...
    .action((opts: { json?: boolean; exitCode?: boolean; watch?: string | boolean }) => {
      ...
      // not a live TTY view). --exit-code is meaningless for an endless loop, so
      // it is simply ignored when watching.
