 ...
 * overdue table on an interval so the "overdue by" spans grow in place — the
 * diagnostic mirror of the upcoming watch loop. `listStatus` re-reads the JSON
 * store each pass (a resume that finally fires drops off the list automatically)
 * and the `--tool`/`--project`/`--since`/`--until` scope plus `--grace`/`--limit`
 * are re-applied every frame. The window boundaries stay fixed (absolute
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the table so overdue spans grow live (Ctrl-C to exit)")
