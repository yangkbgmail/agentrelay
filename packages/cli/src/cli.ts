  ...
  startWatchLoop(intervalMs, () => {
    ...
    if (groupBy !== undefined) {
      ...
      if (byHour && stats.total > 0) {
        body += `\n\n${renderHourly(computeHourlyActivity(jobs), { color: true })}`;
      }
    }
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("--by-hour", "Also show an hour-of-day activity histogram (jobs per UTC hour, across all days)")
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # when in the day do rate-limits cluster?\n" +
    ...
    .action(
      (opts: {
        ...
        if (opts.watch !== undefined && !opts.json) {
          const parsed = typeof opts.watch === "string" ? Number.parseFloat(opts.watch) : NaN;
          const intervalMs = Number.isFinite(parsed) && parsed > 0 ? Math.round(parsed * 1000) : 2000;
          runStatsWatch(store, intervalMs, scope, scopeNote, groupBy, trendDays, byHour);
          return; // setInterval keeps the process alive.
        }
        ...
        const hourly = byHour ? computeHourlyActivity(jobs) : null;
