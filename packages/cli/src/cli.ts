 ...
 * overdue report on an interval so the backlog ages in place. Like the other
 * watch loops, `listStatus` re-reads the JSON store each pass (a running
 * daemon's writes show up automatically) and the `--tool`/`--project`/`--since`/
 * `--until` scope, `--limit`, and `--grace` are re-applied every frame against a
 * fresh `now`. A stuck resume loop shows the overdue spans growing; when the
 * relay catches up the frame drops back to the empty message. Runs until the
...
function runOverdueWatch(
  ...
  startWatchLoop(intervalMs, () => {
    ...
    const frame = renderOverdueWatchFrame(report, store, intervalMs, scopeNote, now);
    process.stdout.write(`\x1b[2J\x1b[H${frame}\n`);
  });
}
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the report so the backlog ages in place (Ctrl-C to exit)")
