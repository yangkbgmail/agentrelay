 ...
 * the other watch loops, `listStatus` re-reads the JSON store each pass (so a
 * recovering resume loop draining the backlog shows up automatically) and the
 * `--grace`, `--limit`, and `--tool`/`--project`/`--since`/`--until` scope are
 * re-applied every frame. The window boundaries stay fixed (absolute epoch-ms
 * from when the command started) while live writes still surface. Runs until the
 ...
 * place. `listStatus` re-reads the JSON store each pass and the
 * `--status`/`--tool`/`--project`/`--since`/`--until` scope is re-applied every
 * frame. Runs until the process is interrupted (Ctrl-C).
 */
 ...
 * `runProjectsWatch` — `listStatus` re-reads the store each pass and the same
 * scope is re-applied every frame. Runs until the process is interrupted (Ctrl-C).
 */
...
/**
 * Resolve the refresh interval for a `--watch [seconds]` flag. Commander gives
 * the option value as a string when a number was supplied (`--watch 5`) or
 * `true` when the flag was bare (`--watch`). A positive finite number wins;
 * anything else (bare flag, non-number, non-positive) falls back to 2s. Shared
 * by every live watch loop so the parsing is identical across commands.
 */
function parseWatchIntervalMs(watch: string | boolean): number {
...
export function buildCli(): Command {
  ...
  program
    ...
    .action((opts: ScopeOpts & { limit?: string; json?: boolean; watch?: string | boolean }) => {
      ...
      if (opts.watch !== undefined && !opts.json) {
        runUpcomingWatch(store, parseWatchIntervalMs(opts.watch), built.scope, limit, scopeNote);
        return; // setInterval keeps the process alive.
      }
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the report with live spans (Ctrl-C to exit)")
    ...
    .action((opts: ScopeOpts & { limit?: string; grace?: string; json?: boolean; watch?: string | boolean }) => {
      ...
      if (opts.watch !== undefined && !opts.json) {
        runOverdueWatch(store, parseWatchIntervalMs(opts.watch), built.scope, graceMs, limit, scopeNote);
        return; // setInterval keeps the process alive.
      }
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the index with live countdowns (Ctrl-C to exit)")
    .option("--json", "Print the summary as JSON (machine-readable, for scripts/CI)")
    .option("-s, --status <statuses>", "Only count jobs with these comma-separated statuses (e.g. waiting_for_reset)")
    .option("-t, --tool <tools>", `Only count jobs run with these comma-separated tools: ${ALL_TOOLS.join(", ")}`)
    .option("-p, --project <projects>", "Only count jobs from these comma-separated project names (exact match)")
    ...
    .action((opts: ScopeOpts & { json?: boolean; watch?: string | boolean }) => {
      ...
      // Live view: validate scope above first so a bad value still exits 1
      // instead of spinning a broken watch loop. --json takes precedence over
      // --watch (a one-shot machine dump, not a live TTY view).
      if (opts.watch !== undefined && !opts.json) {
        runProjectsWatch(store, parseWatchIntervalMs(opts.watch), built.scope, scopeNote);
        return; // setInterval keeps the process alive.
      }
  ...
  program
    ...
    .action((opts: ScopeOpts & { json?: boolean; watch?: string | boolean }) => {
      ...
      // --watch (a one-shot machine dump, not a live TTY view).
      if (opts.watch !== undefined && !opts.json) {
        runToolsWatch(store, parseWatchIntervalMs(opts.watch), built.scope, scopeNote);
        return; // setInterval keeps the process alive.
      }
