 ...
 * watch loops, `listStatus` re-reads the JSON store each pass (a running
 * daemon's writes show up automatically) and the scope plus `--grace`/`--limit`
 * are re-applied every frame with a fresh `now` (the `--since`/`--until` window
 * boundaries stay fixed as absolute epoch-ms from command start). Runs until the
...
/**
 * Live `agentrelay tools --watch`: clears the screen and rebuilds the per-tool
 ...
 * re-applied every frame with a fresh `now`. Runs until interrupted (Ctrl-C).
 */
 ...
 * the scope is re-applied every frame with a fresh `now`. Runs until interrupted
 * (Ctrl-C).
 */
