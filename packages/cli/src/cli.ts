 ...
 * stuck resume loop shows its backlog aging in real time. Like the other watch
 * loops, `listStatus` re-reads the JSON store each pass (a daemon that recovers
 * and drains the backlog shows up automatically) and the scope plus `--limit`
 * and `--grace` are re-applied every frame with a fresh `now`. Runs until the
  ...
  program
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # watch the backlog live while a stalled daemon recovers\n" +
