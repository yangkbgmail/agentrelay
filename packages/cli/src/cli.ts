...
function runStatsWatch(
  ...
  startWatchLoop(intervalMs, () => {
    ...
    if (groupBy !== undefined) {
      ...
      if (weekdays && stats.total > 0) {
        body += `\n\n${renderWeekdays(computeWeekdayDistribution(jobs), { color: true })}`;
      }
    }
  ...
  program
    ...
    .action(
      (opts: {
        ...
        const weekdays = opts.dow ? computeWeekdayDistribution(jobs) : null;

        if (opts.json) {
          console.log(renderStatsJson(stats, store, { scope, trend, hours, weekdays }));
          return;
        }
        ...
        if (weekdays !== null && stats.total > 0) {
          console.log("");
          console.log(renderWeekdays(weekdays, { color: Boolean(process.stdout.isTTY) }));
        }
      }
    );
