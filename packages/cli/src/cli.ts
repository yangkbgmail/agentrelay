...
export function buildCli(): Command {
  ...
  program
    ...
    .option(
      "--tool <tool>",
      "Agent tool adapter to use (claude-code | codex-cli | aider | generic). Inferred from the command when omitted."
    )
    .option(
      "-p, --project <name>",
  ...
  program
    .command("tools")
    .description(
      "List the agent tools in play (claude-code/codex-cli/aider/generic) with per-tool job counts and next reset"
    )
    .option("-w, --watch [seconds]", "Continuously refresh the index with live reset countdowns (Ctrl-C to exit)")
    .option("--json", "Print the summary as JSON (machine-readable, for scripts/CI)")
    .option("-s, --status <statuses>", "Only count jobs with these comma-separated statuses (e.g. waiting_for_reset)")
