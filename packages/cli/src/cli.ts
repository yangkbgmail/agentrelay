...
export function buildCli(): Command {
  ...
  program
    ...
    .option("--by-hour", "Also show a UTC hour-of-day (0–23) activity histogram — when in the day limits hit")
    ...
    .action(
      (opts: {
        ...
        const byHour = opts.byHour ? computeHourlyDistribution(jobs) : null;
        ...
        if (byHour !== null && stats.total > 0) {
          console.log("");
          console.log(renderHourly(byHour, { color: Boolean(process.stdout.isTTY) }));
        }
      }
    );
