 ...
 * overdue report on an interval so the "overdue by" durations grow in place. Like
 * the other watch loops, `listStatus` re-reads the JSON store each pass and the
 * scope filters plus `--limit`/`--grace` are re-applied every frame. The window
 * boundaries stay fixed (absolute epoch-ms from when the command started) while
 * live writes still surface. Runs until interrupted (Ctrl-C).
 */
 ...
 * rollup on an interval so the next-reset countdowns tick down in place. Re-reads
 * the store and re-applies the scope each pass, like the other watch loops.
 */
 ...
 * place. Re-reads the store and re-applies the scope each pass, like the other
 * watch loops.
 */
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the report with live durations (Ctrl-C to exit)")
    ...
    .action((opts: ScopeOpts & { limit?: string; grace?: string; json?: boolean; watch?: string | boolean }) => {
      ...
      // Live view: validate flags (limit/grace/scope) above first so a bad value
