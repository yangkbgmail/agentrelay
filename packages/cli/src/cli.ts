 ...
 * daemon's writes show up automatically), the `--tool`/`--project`/`--since`/
 * `--until` scope plus `--limit`/`--grace` are re-applied every frame, and the
 * window boundaries stay fixed (absolute epoch-ms from when the command
 * started). Runs until the process is interrupted (Ctrl-C).
 */
...
/**
 * Live `agentrelay tools --watch`: clears the screen and re-rolls the per-tool
 * index on an interval so the reset countdowns tick down in place. Same shape as
 * the other watch loops — re-reads the store each pass and re-applies the scope
 * (fixed window boundaries) every frame. Runs until interrupted (Ctrl-C).
 */
...
export function buildCli(): Command {
  ...
  program
    ...
    .action((opts: ScopeOpts & { json?: boolean; watch?: string | boolean }) => {
      ...
      // Live view: validate the scope first so a bad value still exits 1 instead
