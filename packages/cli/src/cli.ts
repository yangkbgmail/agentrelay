...
function runStatsWatch(
  ...
  const tzLabel = formatUtcOffset(offsetMinutes);
  startWatchLoop(intervalMs, () => {
    const now = Date.now();
    const all = listStatus(store);
...
export function buildCli(): Command {
  ...
  program
    ...
    .option(
      "--utc-offset <offset>",
      "Bucket the --hours/--weekday/--trend histograms in this UTC offset instead of UTC (e.g. +9, -05:30)"
    )
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # the hour histogram in your local clock (e.g. KST)\n" +
        "  agentrelay stats --hours --utc-offset +9"
    )
    .action(
      (opts: {
        status?: string;
        ...
        // --utc-offset shifts only the histogram bucketing (hours/weekday/trend)
        // into a local clock; it never filters, so it's not part of the scope.
        let offsetMinutes = 0;
        if (opts.utcOffset !== undefined) {
          const parsed = parseUtcOffset(opts.utcOffset);
          if (parsed === null) {
            console.error(
              `Invalid --utc-offset: "${opts.utcOffset}". Use e.g. +9, -5, +09:30, -05:30, or UTC (range -12:00…+14:00).`
            );
            process.exitCode = 1;
            return;
          }
