 ...
 * overdue report on an interval so the "overdue by" spans climb in place while
 * a resume loop is stuck. Mirrors `runUpcomingWatch`: `listStatus` re-reads the
 * JSON store each pass (a recovering daemon's writes make the list shrink), the
 * `--tool`/`--project`/`--since`/`--until` scope plus `--limit` are re-applied,
 * and the report is rebuilt with the same `graceMs` and a fresh `now` each frame.
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the report as overdue spans climb (Ctrl-C to exit)")
    ...
    .action((opts: ScopeOpts & { limit?: string; grace?: string; json?: boolean; watch?: string | boolean }) => {
      ...
      // Live view: the limit/grace/scope flags above are validated first so a bad
