...
export function buildCli(): Command {
  ...
  program
    .command("overdue")
    .description("List jobs whose reset time has passed but are still waiting — the resume loop is behind")
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  agentrelay overdue\n" +
        "  # gate a monitor on it (non-empty means resumes are stuck)\n" +
        "  test \"$(agentrelay overdue --json | jq '.report.totalOverdue')\" -eq 0\n" +
        "  # scope to one project\n" +
        "  agentrelay overdue --project my-app"
    )
    .action((opts: ScopeOpts & { limit?: string; json?: boolean }) => {
      const { store } = program.opts();
      const now = Date.now();
