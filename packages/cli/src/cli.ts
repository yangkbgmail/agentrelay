 ...
 * the other watch loops, `listStatus` re-reads the JSON store each pass (a
 * running daemon's writes show up automatically) and the `--grace`, scope, and
 * `--limit` are re-applied every frame. Runs until interrupted (Ctrl-C).
 */
 ...
 * index on an interval so the "next reset" countdowns tick down in place. The
 * store is re-read and the scope re-applied every frame. Runs until interrupted.
 */
 ...
 * place. The store is re-read and the scope re-applied every frame. Runs until
 * interrupted.
 */
 ...
 */
/**
 * Turns a `--watch [seconds]` option value into a poll interval in ms. Commander
 * gives `true` for a bare `--watch` (no seconds) and the string for `--watch 5`;
 * anything non-positive or non-numeric falls back to the 2s default so a live
 * view never spins at zero delay.
 */
function watchIntervalMs(watch: string | boolean): number {
  const parsed = typeof watch === "string" ? Number.parseFloat(watch) : NaN;
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the report as jobs fall further behind (Ctrl-C to exit)")
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # watch the backlog grow live, refreshing every 2s\n" +
    ...
    .action((opts: ScopeOpts & { limit?: string; grace?: string; json?: boolean; watch?: string | boolean }) => {
      ...
      // Live view: validate flags (limit/grace/scope) first so a bad value still
      // exits 1 instead of spinning a broken watch loop. --json takes precedence
      // over --watch (a one-shot machine dump, not a live TTY view).
      if (opts.watch !== undefined && !opts.json) {
  ...
  program
    ...
    .action((opts: ScopeOpts & { json?: boolean; watch?: string | boolean }) => {
      ...
      // Live view: validate scope first; --json wins over --watch.
      if (opts.watch !== undefined && !opts.json) {
