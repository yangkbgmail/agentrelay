...
export function buildCli(): Command {
  ...
  program
    .command("overdue")
    .description("Show waiting jobs whose reset time has passed but that nothing resumed (stuck relay diagnostic)")
    .option("--grace <duration>", "Ignore jobs due within the last <duration> (e.g. 2m, 30s, 5m); default 2m")
    .option("-n, --limit <n>", "Show at most N rows (the totals still count all overdue jobs)")
    .option("-t, --tool <tools>", `Only include jobs run with these comma-separated tools: ${ALL_TOOLS.join(", ")}`)
    .option("-p, --project <projects>", "Only include jobs from these comma-separated project names (exact match)")
    .option("--since <duration>", "Only include jobs created within the last <duration> (e.g. 24h, 7d, 30m)")
    .option("--until <duration>", "Only include jobs created more than <duration> ago (e.g. 1d) — window's older edge")
    .option("--exit-code", "Exit 5 when any job is overdue (0 otherwise), for cron/monitoring without jq")
    .option("--json", "Print the report as JSON (machine-readable, for scripts/jq)")
    .addHelpText(
      "after",
      "\nExamples:\n" +
        "  # are any resumes stuck past due? (empty means the relay is keeping up)\n" +
        "  agentrelay overdue\n" +
        "  # tighter grace, one project\n" +
        "  agentrelay overdue --grace 30s --project my-app\n" +
        "  # alert from cron/CI: nonzero exit when anything is stuck\n" +
        "  agentrelay overdue --exit-code || notify-send 'agentrelay stuck'"
    )
    .action((opts: ScopeOpts & { grace?: string; limit?: string; exitCode?: boolean; json?: boolean }) => {
      const { store } = program.opts();
      const now = Date.now();
      ...
      if (opts.grace !== undefined) {
        ...
        if (ms === null || ms < 0) {
          console.error(`Invalid --grace duration: "${opts.grace}". Use e.g. 2m, 30s, 5m.`);
          process.exitCode = 1;
          return;
        }
        graceMs = ms;
      ...
      // Opt-in exit code lets a cron/monitor branch on "is the relay stuck?"
      // without parsing output: 5 = something overdue, 0 = all clear.
