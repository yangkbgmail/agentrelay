...
import { renderWaitAllJson, renderWaitJson } from "./wait.js";
 ...
 * timeline on an interval so the countdowns tick down in place. Like the status
 * watch loop, `listStatus` re-reads the JSON store each pass (a running daemon's
 * writes show up automatically) and the `--tool`/`--project`/`--since`/`--until`
 * scope plus `--limit` are re-applied every frame. The window boundaries stay
 * fixed (absolute epoch-ms from when the command started) while live writes
 * still surface. Runs until the process is interrupted (Ctrl-C).
 */
...
/**
 * Live `agentrelay overdue --watch`: clears the screen and re-renders the stuck-
 * jobs report on an interval so the overdue spans grow in place. Like the other
 * watch loops, `listStatus` re-reads the JSON store each pass (so a resume loop
 * catching up shows jobs dropping off the list) and the `--tool`/`--project`/
 * `--since`/`--until` scope, the `--grace` window, plus `--limit` are re-applied
 * every frame. `buildOverdueReport` is rebuilt with a fresh `now` each pass so
 * the spans stay live; the time-window boundaries stay fixed (absolute epoch-ms
...
/**
 * Live `agentrelay tools --watch`: clears the screen and re-renders the per-tool
 * index on an interval so the reset countdowns tick down in place. Like the other
 * watch loops, `listStatus` re-reads the JSON store each pass (so a running
 * daemon's writes and newly-queued tools surface automatically) and the
 * `--status`/`--tool`/`--project`/`--since`/`--until` scope is re-applied every
 * frame. `summarizeTools` is rebuilt with a fresh `now` each pass so the
 ...
 * watch loops, `listStatus` re-reads the JSON store each pass and the
 * `--status`/`--tool`/`--project`/`--since`/`--until` scope is re-applied every
 * frame (time-window boundaries stay fixed at the absolute epoch-ms captured when
 * the command started). Unlike them, the `stats` body can be a `--group-by`
 * breakdown and/or a `--trend` histogram, so the body is composed here (with a
 * fresh `now` each pass) and wrapped in the live banner. Runs until interrupted.
 */
 ...
 * caller returns right after invoking this.
 */
...
export function buildCli(): Command {
  ...
  program
    .command("summary")
    .description("Show a one-glance queue overview: total jobs, per-status counts, and the next reset countdown")
  ...
  program
    ...
    .addHelpText(
      "after",
      "\nExamples:\n" +
        "  # what resumes next, and what's queued behind it?\n" +
        "  agentrelay upcoming\n" +
    ...
    .action((opts: ScopeOpts & { limit?: string; json?: boolean; watch?: string | boolean }) => {
      ...
      if (opts.watch !== undefined && !opts.json) {
        const parsed = typeof opts.watch === "string" ? Number.parseFloat(opts.watch) : NaN;
        const intervalMs = Number.isFinite(parsed) && parsed > 0 ? Math.round(parsed * 1000) : 2000;
        runUpcomingWatch(store, intervalMs, built.scope, limit, scopeNote);
        return; // setInterval keeps the process alive.
      }
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the report with live-growing overdue spans (Ctrl-C to exit)")
    .option("-n, --limit <n>", "Show at most N rows (the totals still count all overdue jobs)")
    .option(
      "--grace <duration>",
      "Ignore jobs due within the last <duration> so freshly-due jobs aren't flagged (default 60s)",
      "60s"
    )
    .option("-t, --tool <tools>", `Only include jobs run with these comma-separated tools: ${ALL_TOOLS.join(", ")}`)
    .option("-p, --project <projects>", "Only include jobs from these comma-separated project names (exact match)")
    ...
    .addHelpText(
      "after",
      "\nExamples:\n" +
        "  # what should already be running but isn't?\n" +
        "  agentrelay overdue\n" +
  ...
  program
    .command("wait")
    .description(
      "Block until a job finishes (or --all: until the whole queue drains), then exit with a code reflecting the outcome (0 completed/drained, 1 failed, 2 cancelled, 124 timeout)"
    )
    .argument("[id]", "Job id or a short id prefix (see `agentrelay status`); omit when using --all")
    .option("--all", "Wait until the whole queue drains (no active jobs remain) instead of a single job")
    ...
    .option("-s, --status <statuses>", "With --all: only count jobs with these comma-separated statuses")
    .option("-t, --tool <tools>", "With --all: only count jobs for these comma-separated tools")
    .option("-p, --project <projects>", "With --all: only count jobs for these comma-separated projects")
    .option("--since <duration>", "With --all: only count jobs created within this window (e.g. 24h, 7d)")
    .option("--until <duration>", "With --all: only count jobs created up to this long ago (e.g. 1h)")
    ...
    .addHelpText(
      "after",
      "\nExamples:\n" +
        "  # block until one job finishes, then deploy only if it completed\n" +
        "  agentrelay wait 1a2b3c4d --timeout 6h && ./deploy.sh\n" +
        "  # start the relay, then block until every job has settled\n" +
        "  agentrelay daemon & ; agentrelay wait --all --timeout 8h\n" +
        "  # drain only one project's jobs\n" +
        "  agentrelay wait --all --project my-app\n"
    )
    .action(
      async (
        id: string | undefined,
        ...
        if (opts.all) {
          if (id !== undefined) {
            console.error("[agentrelay] Provide a job id or --all, not both.");
            process.exitCode = 1;
            return;
          }
          ...
          if (!opts.quiet && !opts.json) {
            const suffix = built.active ? ` (scope: ${built.note})` : "";
            console.error(`[agentrelay] waiting for the queue to drain${suffix}… (Ctrl-C to stop)`);
          }

          const result = await waitForAllJobs({ storePath: store, intervalMs, timeoutMs, scope: built.scope });
        ...
        if (id === undefined) {
          console.error("[agentrelay] Provide a job id, or use --all to wait for the whole queue.");
          process.exitCode = 1;
          return;
        }
        if (scopeGiven) {
          console.error(
            "[agentrelay] Scope filters (--status/--tool/--project/--since/--until) only apply with --all."
          );
          process.exitCode = 1;
          return;
        }
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the metrics with a live countdown (Ctrl-C to exit)")
    ...
    .addHelpText(
      "after",
      "\nExamples:\n" +
        "  # aggregate relay metrics at a glance\n" +
        "  agentrelay stats\n" +
    ...
    .action(
      (opts: {
        ...
        // Zone for the --hours/--weekday histograms: UTC by default, or this
        // machine's local wall clock with --local. getTimezoneOffset() returns
        // minutes to add to local to reach UTC, so negate it to get minutes
        // ahead of UTC (e.g. KST → +540). The label carries the offset so the
        // histogram header/footer say which clock the buckets are in.
        const zone = opts.local
          ? (() => {
              const offsetMinutes = -new Date().getTimezoneOffset();
              return { offsetMinutes, label: `local (${formatUtcOffsetLabel(offsetMinutes)})` };
            })()
          : { offsetMinutes: 0, label: "UTC" };

        // Live view: all validation (scope/group-by/trend) has run above, so a
        // bad value still exits 1 instead of spinning a broken watch loop.
        // --json takes precedence over --watch (a one-shot machine dump, not a
        // live TTY view). The loop re-reads the store and rebuilds the metrics
        // with a fresh `now` each pass, so the countdown ticks down in place.
        if (opts.watch !== undefined && !opts.json) {
          const parsed = typeof opts.watch === "string" ? Number.parseFloat(opts.watch) : NaN;
          const intervalMs = Number.isFinite(parsed) && parsed > 0 ? Math.round(parsed * 1000) : 2000;
          runStatsWatch(
