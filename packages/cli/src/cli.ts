  ...
  startWatchLoop(intervalMs, () => {
    ...
    if (groupBy !== undefined) {
      ...
      if (trendDays !== null && stats.total > 0) {
        const trend = computeDailyTrend(jobs, { nowMs: now, days: trendDays, offsetMinutes });
...
export function buildCli(): Command {
  ...
  program
    ...
    .option(
      "--tz <offset>",
      "Bucket the --hours/--weekday/--trend histograms in this UTC offset instead of UTC (e.g. +09:00, -5, local)"
    )
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # the same, but in your own timezone\n" +
    ...
    .action(
      (opts: {
        ...
        // --tz shifts the activity histograms (--hours/--weekday/--trend) into a
        // fixed UTC offset. "local" resolves to the host's current offset;
        // anything else is parsed as an explicit offset. Default is UTC (0).
        let offsetMinutes = 0;
        if (opts.tz !== undefined) {
          if (opts.tz.trim().toLowerCase() === "local") {
            // getTimezoneOffset() is minutes behind UTC, so negate to get
            // minutes east of UTC (UTC+09:00 → -540 → 540).
            offsetMinutes = -new Date().getTimezoneOffset();
          } else {
            const parsed = parseUtcOffsetMinutes(opts.tz);
            if (parsed === null) {
              console.error(`Invalid --tz value: "${opts.tz}". Use e.g. +09:00, -5, UTC, or local.`);
              process.exitCode = 1;
              return;
            }
        ...
        const stats = computeStats(jobs);
        const trend =
          trendDays !== null ? computeDailyTrend(jobs, { nowMs: now, days: trendDays, offsetMinutes }) : null;
