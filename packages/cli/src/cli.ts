 ...
 * pass (so a running daemon's writes surface automatically) and `computeQueueEta`
 * is recomputed with a fresh `now` each frame. `eta` acts on the whole queue (no
 * scope filters), so there is no window to re-apply. Runs until interrupted.
 */
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the ETA with a live countdown (Ctrl-C to exit)")
    .addHelpText(
      "after",
      "\nExamples:\n" +
    ...
    .action((opts: { json?: boolean; exitCode?: boolean; watch?: string | boolean }) => {
      ...
      // live TTY view). A live countdown replaces --exit-code, which only makes
      // sense for a one-shot poll.
