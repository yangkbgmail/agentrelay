 ...
 * recovering resume loop clears the list automatically) and the
 * `--tool`/`--project`/`--since`/`--until` scope, `--grace`, and `--limit` are
 * re-applied every frame. Window boundaries stay fixed (absolute epoch-ms from
 * when the command started). Runs until the process is interrupted (Ctrl-C).
 */
 ...
 * frame. Window boundaries stay fixed (absolute epoch-ms from when the command
