 ...
 * report on an interval so the overdue spans grow in place. Like the other watch
 * loops, `listStatus` re-reads the JSON store each pass and the
 * `--tool`/`--project`/`--since`/`--until` scope plus `--grace`/`--limit` are
 * re-applied every frame with a fresh `now` (so a job crossing the grace edge
 * shows up automatically). Runs until the process is interrupted (Ctrl-C).
 */
 ...
 * index on an interval so the reset countdowns tick down in place. `listStatus`
 * re-reads the JSON store each pass and the `--status`/`--tool`/`--project`/
 * `--since`/`--until` scope is re-applied every frame with a fresh `now`. Runs
 ...
 * `listStatus` re-reads the JSON store each pass and the
 * `--status`/`--tool`/`--project`/`--since`/`--until` scope is re-applied every
 * frame with a fresh `now`. Runs until the process is interrupted (Ctrl-C).
 */
 ...
 */
/**
 * Resolves the `-w, --watch [seconds]` flag value to a refresh interval in ms.
 * A bare `--watch` (boolean true) or a non-positive/unparsable value falls back
 * to the shared 2s default; `--watch 5` means every 5 seconds. Shared by every
 * `--watch` command so the cadence and default match.
 */
function parseWatchInterval(watch: string | boolean): number {
...
export function buildCli(): Command {
  ...
  program
    ...
    .action((opts: ScopeOpts & { limit?: string; json?: boolean; watch?: string | boolean }) => {
      ...
      if (opts.watch !== undefined && !opts.json) {
        runUpcomingWatch(store, parseWatchInterval(opts.watch), built.scope, limit, scopeNote);
        return; // setInterval keeps the process alive.
      }
  ...
  program
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # live watch, refreshing every 2s\n" +
    ...
    .action((opts: ScopeOpts & { limit?: string; grace?: string; json?: boolean; watch?: string | boolean }) => {
      ...
      if (opts.watch !== undefined && !opts.json) {
        runOverdueWatch(store, parseWatchInterval(opts.watch), built.scope, graceMs, limit, scopeNote);
        return; // setInterval keeps the process alive.
      }
  ...
  program
    ...
    .action((opts: ScopeOpts & { json?: boolean; watch?: string | boolean }) => {
      ...
      if (opts.watch !== undefined && !opts.json) {
        runProjectsWatch(store, parseWatchInterval(opts.watch), built.scope, scopeNote);
        return; // setInterval keeps the process alive.
      }
  ...
  program
    ...
    .action((opts: ScopeOpts & { json?: boolean; watch?: string | boolean }) => {
      ...
      // --watch (a one-shot machine dump, not a live TTY view).
      if (opts.watch !== undefined && !opts.json) {
        runToolsWatch(store, parseWatchInterval(opts.watch), built.scope, scopeNote);
        return; // setInterval keeps the process alive.
      }
