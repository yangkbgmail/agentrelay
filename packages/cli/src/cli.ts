...
/**
 * Live `agentrelay overdue --watch`: the diagnostic twin of the upcoming watch
 * loop. Clears the screen and rebuilds the overdue report on an interval so the
 * "overdue by" spans grow in place — a live view of a resume loop falling
 * behind. Like the other watch loops, `listStatus` re-reads the JSON store each
 * pass and the `--tool`/`--project`/`--since`/`--until` scope plus `--limit` are
 * re-applied every frame; `graceMs` (fixed at command start) is re-measured
 * against a fresh `now` each pass. The window boundaries stay fixed (absolute
 * epoch-ms) while live writes still surface. Runs until interrupted (Ctrl-C).
 */
  ...
  program
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # live view while a stuck queue drains\n" +
