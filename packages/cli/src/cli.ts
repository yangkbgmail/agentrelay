 ...
 * running daemon's writes show up automatically) and the
 * `--status`/`--tool`/`--project`/`--since`/`--until` scope is re-applied every
 * frame. `summarizeProjects` is rebuilt with a fresh `now` each pass so the
...
/**
 * Live `agentrelay tools --watch`: the per-tool mirror of `runProjectsWatch`.
 * Clears the screen and re-renders the tool index on an interval, re-reading the
 * store and re-summarizing with a fresh `now` each pass so the reset countdowns
 * tick down in place. The scope is re-applied every frame; the time-window
 * boundaries stay fixed. Runs until interrupted (Ctrl-C).
 */
...
export function buildCli(): Command {
  ...
  program
    ...
    .action((opts: ScopeOpts & { json?: boolean; watch?: string | boolean }) => {
      ...
      // Live view: validate scope above first so a bad value still exits 1 instead
      // of spinning a broken watch loop. --json takes precedence over --watch (a
      // one-shot machine dump, not a live TTY view).
      if (opts.watch !== undefined && !opts.json) {
        const parsed = typeof opts.watch === "string" ? Number.parseFloat(opts.watch) : NaN;
        const intervalMs = Number.isFinite(parsed) && parsed > 0 ? Math.round(parsed * 1000) : 2000;
