 ...
 * overdue report on an interval so the "overdue by" spans grow in place — a
 * stuck-resume-loop monitor you can leave running. Like the other watch loops,
 * `listStatus` re-reads the JSON store each pass (a recovering daemon's writes
 * clear the list automatically) and the `--tool`/`--project`/`--since`/`--until`
 * scope, `--limit`, and `--grace` are re-applied every frame. The window
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the report so overdue spans grow in place (Ctrl-C to exit)")
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # leave a live stuck-loop monitor running, refreshing every 5s\n" +
