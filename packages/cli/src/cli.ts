 ...
 * the resume loop stays stuck. Like the other watch loops, `listStatus`
 * re-reads the JSON store each pass (a recovering daemon draining the backlog
 * shows up automatically) and the `--tool`/`--project`/`--since`/`--until`
 * scope, `--grace`, and `--limit` are re-applied every frame. The window
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the report as the overdue spans climb (Ctrl-C to exit)")
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # live view while the backlog is stuck\n" +
