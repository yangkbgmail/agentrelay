 ...
 * overdue report on an interval so a stuck resume loop shows the "overdue by"
 * spans widening in place. Like the other watch loops, `listStatus` re-reads the
 * JSON store each pass (so a daemon catching up makes rows disappear) and the
 * `--grace`, `--tool`/`--project`/`--since`/`--until` scope plus `--limit` are
 * re-applied every frame. The scope window boundaries stay fixed (absolute
 * epoch-ms from when the command started) while live writes still surface. Runs
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the report so overdue spans widen in place (Ctrl-C to exit)")
