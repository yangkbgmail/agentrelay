...
/**
 * Live `agentrelay summary --watch`: clears the screen and re-renders the compact
 * queue overview on an interval so the next-reset countdown ticks down in place.
 * Like the other watch loops, `listStatus` re-reads the JSON store each pass (so a
 * running daemon's writes surface automatically) and the `--status`/`--tool`/
 * `--project`/`--since`/`--until` scope is re-applied every frame. `summarizeJobs`
 * is rebuilt with a fresh `now` each pass so the countdown stays live; the
 * time-window boundaries stay fixed (absolute epoch-ms from when the command
 * started). Runs until interrupted (Ctrl-C).
 */
function runSummaryWatch(store: string, intervalMs: number, window: JobScope, scopeNote?: string): void {
  ...
  startWatchLoop(intervalMs, () => {
    ...
    const frame = renderSummaryWatchFrame(summary, store, intervalMs, now, scopeNote || undefined);
    process.stdout.write(`\x1b[2J\x1b[H${frame}\n`);
  });
}
  ...
  program
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  agentrelay summary\n" +
        "  # keep a live overview open on a second screen\n" +
    ...
    .action((opts: ScopeOpts & { json?: boolean; watch?: string | boolean }) => {
      ...
      if (opts.watch !== undefined) {
        const parsed = typeof opts.watch === "string" ? Number.parseFloat(opts.watch) : NaN;
        const intervalMs = Number.isFinite(parsed) && parsed > 0 ? Math.round(parsed * 1000) : 2000;
        runSummaryWatch(store, intervalMs, built.scope, built.note);
        return; // setInterval keeps the process alive.
      }
      ...
      const summary = summarizeJobs(jobs);
      console.log(
        renderSummary(summary, { color: Boolean(process.stdout.isTTY), now, scopeNote: built.note || undefined })
      );
    });
