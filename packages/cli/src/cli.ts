 ...
 * next-resume one-liner on an interval so the countdown ticks down in place.
 * Like the other watch loops, `listStatus` re-reads the JSON store each pass (so
 * a running daemon's writes — a job resuming, a new one arriving — show up
 * automatically) and `selectNextResume` is re-evaluated with a fresh `now` every
...
export function buildCli(): Command {
  ...
  program
    ...
    .addHelpText(
      "after",
      "\nExamples:\n" +
        "  # which job resumes next, and in how long?\n" +
        "  agentrelay next\n" +
    ...
    .action((opts: { json?: boolean; exitCode?: boolean; watch?: string | boolean }) => {
      ...
      // not a live TTY view). --exit-code is meaningless in a loop that never
      // returns, so --watch simply supersedes it.
