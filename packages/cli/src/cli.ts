...
export function buildCli(): Command {
  ...
  program
    .command("reschedule")
    .description("Change when a pending job resumes (adjusts its reset time; keeps its attempt count)")
    .argument("<id>", "Job id or a short id prefix (see `agentrelay status`)")
    .argument(
      "<when>",
      'When to resume: "now", a duration (e.g. 30m, 2h, 90s), or an absolute time (e.g. 2026-07-25T18:00:00Z)'
    )
