 ...
 * report on an interval so a stuck job's "overdue by" span grows in place — the
 * diagnostic mirror of the `upcoming` watch (whose countdowns tick down). Like
 * the other watch loops, `listStatus` re-reads the JSON store each pass (so a
 * daemon that finally resumes a job makes it drop off the list live) and the
 * `--tool`/`--project`/`--since`/`--until` scope plus `--limit`/`--grace` are
...
export function buildCli(): Command {
  ...
  program
    ...
    .option(
      "-w, --watch [seconds]",
      "Continuously refresh the report; a stuck job's overdue span grows live (Ctrl-C to exit)"
    )
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # live view: watch a stuck job's overdue span grow (or drop off when it resumes)\n" +
