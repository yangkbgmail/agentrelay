...
export function buildCli(): Command {
  ...
  program
    ...
    .option("--local", "Bucket --hours/--weekday in your local timezone instead of UTC (uses your current UTC offset)")
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # the same, but in your local timezone\n" +
    ...
    .action(
      (opts: {
        ...
        const now = Date.now();
        // --local shifts the --hours/--weekday buckets into the host's current
        // wall-clock offset. getTimezoneOffset() is minutes *behind* UTC (west =
        // positive), so negate it to get a conventional UTC offset (UTC+9 → 540).
        ...
        const tzLabel = formatUtcOffsetLabel(utcOffsetMinutes);
        // Surface the offset in JSON only when --local actually shifted a
        // histogram, so the default (UTC) JSON shape is unchanged.
        const jsonOffset = opts.local && (hours !== null || weekday !== null) ? utcOffsetMinutes : null;

        if (opts.json) {
          console.log(renderStatsJson(stats, store, { scope, trend, hours, weekday, utcOffsetMinutes: jsonOffset }));
          return;
        }
