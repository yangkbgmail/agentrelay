 ...
 * pass (so a running daemon's writes — a job dropping off as it resumes, or the
 * queue reaching "caught up" — surface automatically) and `computeQueueEta` is
 * rebuilt with a fresh `now` each pass so the ETA stays live. `eta` has no scope
 * filters, so there is nothing to re-apply. Runs until interrupted (Ctrl-C).
 */
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the catch-up ETA with a live countdown (Ctrl-C to exit)")
    .option("--json", "Print as JSON (machine-readable, for scripts/jq)")
    .option(
      "--exit-code",
    ...
    .action((opts: { json?: boolean; exitCode?: boolean; watch?: string | boolean }) => {
      ...
      // returns, so --watch simply ignores it.
