...
export function buildCli(): Command {
  ...
  program
    ...
    .option(
      "--far-future",
      "Also reclaim jobs parked with an implausibly far-future reset (uses the AGENTRELAY_MAX_RESET_HORIZON guard)"
    )
    ...
    .action((opts: { olderThan?: string; farFuture?: boolean; dryRun?: boolean; json?: boolean }) => {
      ...
      const { report, recovered, farFutureReport, farFutureRecovered, dryRun } = recoverJobs({
        storePath: store,
        stuckAfterMs,
      ...
      const result: RecoverResult = { report, recovered, farFutureReport, farFutureRecovered, dryRun };
