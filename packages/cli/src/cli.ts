 ...
 * running daemon's writes show up automatically) and the `--status`/`--tool`/
 * `--project`/`--since`/`--until` scope is re-applied every frame.
 * `summarizeTools` is recomputed with a fresh `now` each pass so the countdowns
 ...
 * place. The mirror of `runToolsWatch`: `listStatus` re-reads the JSON store
 * each pass, the `--status`/`--tool`/`--project`/`--since`/`--until` scope is
 * re-applied every frame, and `summarizeProjects` is recomputed with a fresh
 * `now` so the countdowns stay live (time-window boundaries stay fixed). Runs
