 ...
 * index on an interval so the reset countdowns tick down in place. Like the
 * other watch loops, `listStatus` re-reads the JSON store each pass (a running
 * daemon's writes show up automatically) and the `--status`/`--tool`/`--project`/
 ...
 * `summarizeProjects` is rebuilt with a fresh `now` each pass. The time-window
