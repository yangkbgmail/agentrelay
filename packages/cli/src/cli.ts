 ...
 * most-imminent resume on an interval so its countdown ticks down in place and a
 * daemon's writes (a job resuming, a new rate-limit landing) are picked up. Like
 * the other watch loops, `listStatus` re-reads the JSON store each pass and
 * `selectNextResume` recomputes the pick with a fresh `now`. The one-line body
 * makes it a natural status-bar/tmux widget. Runs until interrupted.
 */
...
export function buildCli(): Command {
  ...
  program
    ...
    .option(
      "-w, --watch [seconds]",
      "Continuously refresh the one-line countdown (status-bar friendly; Ctrl-C to exit)"
    )
    ...
    .addHelpText(
      "after",
      "\nExamples:\n" +
        "  # what resumes next, and how long until it's due?\n" +
        "  agentrelay next\n" +
        "  # poke the relay only when something is actually due (no jq)\n" +
        "  agentrelay next --exit-code && agentrelay tick\n" +
        "  # live one-line countdown for a status bar / tmux, refreshing every 2s\n" +
        "  agentrelay next --watch"
    )
    .action((opts: { watch?: string | boolean; json?: boolean; exitCode?: boolean }) => {
      const { store } = program.opts();

      // Live view: --json (a one-shot machine dump) and --exit-code (a one-shot
      // branch for scripts) both take precedence over --watch, which never exits
      // on its own. A bare --watch spins the loop; setInterval keeps us alive.
      if (opts.watch !== undefined && !opts.json && !opts.exitCode) {
        const parsed = typeof opts.watch === "string" ? Number.parseFloat(opts.watch) : NaN;
        const intervalMs = Number.isFinite(parsed) && parsed > 0 ? Math.round(parsed * 1000) : 2000;
