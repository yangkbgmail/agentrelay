...
export function buildCli(): Command {
  ...
  program
    .command("wait")
    .description(
      "Block until a job finishes (or --all: every active job), then exit with a code reflecting the outcome (0 completed, 1 failed, 2 cancelled, 124 timeout)"
    )
    .argument("[id]", "Job id or a short id prefix (see `agentrelay status`); omit when using --all")
    .option("--all", "Wait for every currently-active job to finish (queue-drain gate); scope with the filters below")
    .option("-s, --status <statuses>", "With --all: only wait on jobs with these statuses (comma-separated)")
    .option("-t, --tool <tools>", "With --all: only wait on jobs run with these tools (comma-separated)")
    .option("-p, --project <projects>", "With --all: only wait on jobs in these projects (comma-separated)")
    .option("--since <duration>", "With --all: only wait on jobs created within this long ago (e.g. 24h, 7d)")
    .option("--until <duration>", "With --all: only wait on jobs created before this long ago (e.g. 1d)")
    ...
    .action(
      async (
        ...
        if (opts.all) {
          ...
          if (opts.json) {
            console.log(renderWaitAllJson(result, store, built.note));
          } else if (!opts.quiet) {
            console.log(`[agentrelay] ${result.message}`);
          }
          process.exitCode = result.exitCode;
        ...
        if (!id) {
          console.error("Provide a job id, or --all to wait for every active job.");
          process.exitCode = 1;
          return;
        }
