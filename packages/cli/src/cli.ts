 ...
 * scope plus `--grace`/`--limit` are re-applied every frame (`buildOverdueReport`
 * recomputes against a fresh `now`, so jobs cross the grace threshold live).
 * Runs until the process is interrupted (Ctrl-C).
 */
 ...
 * place. `listStatus` re-reads the store each pass and the scope is re-applied
 * every frame. Runs until the process is interrupted (Ctrl-C).
 */
 ...
 * rollup on an interval so the "next reset" countdowns tick down in place.
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the report as jobs slip further overdue (Ctrl-C to exit)")
    ...
    .action((opts: ScopeOpts & { limit?: string; grace?: string; json?: boolean; watch?: string | boolean }) => {
      ...
      // Live view: validate limit/grace/scope first so a bad value still exits 1
      // instead of spinning a broken watch loop. --json wins over --watch.
      if (opts.watch !== undefined && !opts.json) {
        const parsed = typeof opts.watch === "string" ? Number.parseFloat(opts.watch) : NaN;
        const intervalMs = Number.isFinite(parsed) && parsed > 0 ? Math.round(parsed * 1000) : 2000;
  ...
  program
    ...
    .action((opts: ScopeOpts & { json?: boolean; watch?: string | boolean }) => {
      ...
      // Live view: validate scope first so a bad value exits 1 before the loop.
      // --json wins over --watch (a one-shot machine dump, not a live TTY view).
      if (opts.watch !== undefined && !opts.json) {
        const parsed = typeof opts.watch === "string" ? Number.parseFloat(opts.watch) : NaN;
        const intervalMs = Number.isFinite(parsed) && parsed > 0 ? Math.round(parsed * 1000) : 2000;
