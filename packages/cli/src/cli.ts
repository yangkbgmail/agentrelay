...
export function buildCli(): Command {
  ...
  program
    .command("overdue")
    .description("List jobs whose reset has already passed but are still waiting — the resume loop is behind")
    .option("-n, --limit <n>", "Show at most N rows (the totals still count all overdue jobs)")
    .option("--grace <duration>", "Only flag jobs more than <duration> past due (e.g. 30s, 5m) — absorbs tick lag")
    .option("-t, --tool <tools>", `Only include jobs run with these comma-separated tools: ${ALL_TOOLS.join(", ")}`)
    .option("-p, --project <projects>", "Only include jobs from these comma-separated project names (exact match)")
    .option("--since <duration>", "Only include jobs created within the last <duration> (e.g. 24h, 7d, 30m)")
    .option("--until <duration>", "Only include jobs created more than <duration> ago (e.g. 1d) — window's older edge")
    .option("--exit-code", "Exit 1 when any job is overdue (for monitors/cron: `agentrelay overdue --exit-code`)")
    .option("--json", "Print the report as JSON (machine-readable, for scripts/monitors)")
    .addHelpText(
      "after",
      "\nExamples:\n" +
        "  # is the relay actually resuming? (empty = healthy)\n" +
        "  agentrelay overdue\n" +
        "  # ignore jobs only a little late, then fail a monitor if any remain\n" +
        "  agentrelay overdue --grace 1m --exit-code\n" +
        "  # feed the report to a monitor\n" +
        "  agentrelay overdue --json | jq '.report.totalOverdue'"
    )
    .action((opts: ScopeOpts & { limit?: string; grace?: string; exitCode?: boolean; json?: boolean }) => {
      ...
      const jobs = built.active ? scopeJobs(allJobs, built.scope) : allJobs;
      const scopeNote = built.active ? built.note : undefined;
      const report = buildOverdueReport(jobs, now, { limit, graceMs });
