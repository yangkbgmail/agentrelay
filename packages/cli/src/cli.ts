...
/**
 * Live `agentrelay overdue --watch`: clears the screen and re-renders the report
 * on an interval so a stuck job's "overdue by" span grows in place. Like the
 * other watch loops, `listStatus` re-reads the JSON store each pass (so a resume
 * loop that finally catches up makes rows disappear) and the fixed `graceMs`,
 * `--limit`, and `--tool`/`--project`/`--since`/`--until` scope are re-applied
 * every frame with a fresh `now`. The window boundaries stay fixed (absolute
  ...
  program
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # live view, watching stuck spans grow, refreshing every 2s\n" +
