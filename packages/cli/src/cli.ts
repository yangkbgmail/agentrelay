 ...
 * report on an interval so the "overdue by" spans grow in place — the at-a-glance
 * monitor for a stuck resume loop. Like the other watch loops, `listStatus`
 * re-reads the JSON store each pass, and the `--tool`/`--project`/`--since`/
 * `--until` scope plus `--grace`/`--limit` are re-applied every frame. The window
 * boundaries stay fixed (absolute epoch-ms from when the command started) while
 * live writes still surface. Runs until the process is interrupted (Ctrl-C).
 */
function runOverdueWatch(
  store: string,
  intervalMs: number,
  window: JobScope,
  options: { graceMs: number; limit?: number; scopeNote?: string }
): void {
  const active = isJobScopeActive(window);
  startWatchLoop(intervalMs, () => {
    const now = Date.now();
    const all = listStatus(store);
    const jobs = active ? scopeJobs(all, window) : all;
    const report = buildOverdueReport(jobs, now, { graceMs: options.graceMs, limit: options.limit });
    const frame = renderOverdueWatchFrame(report, store, intervalMs, now, options.scopeNote);
    process.stdout.write(`\x1b[2J\x1b[H${frame}\n`);
  });
}

/**
 * Live `agentrelay projects --watch`: clears the screen and rebuilds the
 * per-project index on an interval so counts and reset countdowns stay current.
 * Like the other watch loops, `listStatus` re-reads the JSON store each pass and
 * the `--status`/`--tool`/`--project`/`--since`/`--until` scope is re-applied
 * every frame (window boundaries fixed at command start). Runs until interrupted.
 */
...
/**
 * Live `agentrelay tools --watch`: the tool-axis mirror of `runProjectsWatch`.
 * Rebuilds the per-tool index on an interval, re-reading the store and
 * re-applying the same scope each frame. Runs until interrupted (Ctrl-C).
 */
 ...
 */
/**
 * Interpret a `--watch [seconds]` flag value into a refresh interval in ms.
 * A bare `--watch` (boolean true) or a non-positive/non-numeric value falls back
 * to the shared 2s default, matching `status`/`upcoming`.
 */
function watchIntervalMs(watch: string | boolean): number {
...
export function buildCli(): Command {
  ...
  program
    ...
    .action((opts: ScopeOpts & { limit?: string; json?: boolean; watch?: string | boolean }) => {
      ...
      if (opts.watch !== undefined && !opts.json) {
        runUpcomingWatch(store, watchIntervalMs(opts.watch), built.scope, limit, scopeNote);
        return; // setInterval keeps the process alive.
      }
  ...
  program
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # live monitor, refreshing every 5s\n" +
        "  agentrelay overdue --watch 5"
    )
    .action((opts: ScopeOpts & { limit?: string; grace?: string; json?: boolean; watch?: string | boolean }) => {
      ...
      if (opts.watch !== undefined && !opts.json) {
        runOverdueWatch(store, watchIntervalMs(opts.watch), built.scope, { graceMs, limit, scopeNote });
        return; // setInterval keeps the process alive.
      }
  ...
  program
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # live dashboard, refreshing every 2s\n" +
    ...
    .action((opts: ScopeOpts & { json?: boolean; watch?: string | boolean }) => {
      ...
      if (opts.watch !== undefined && !opts.json) {
        runProjectsWatch(store, watchIntervalMs(opts.watch), built.scope, scopeNote);
        return; // setInterval keeps the process alive.
      }
  ...
  program
    ...
    .action((opts: ScopeOpts & { json?: boolean; watch?: string | boolean }) => {
      ...
      // spinning a broken watch loop. --json takes precedence over --watch.
      if (opts.watch !== undefined && !opts.json) {
        runToolsWatch(store, watchIntervalMs(opts.watch), built.scope, scopeNote);
        return; // setInterval keeps the process alive.
      }
