 ...
 * overdue report on an interval so the "overdue by" spans grow in place — the
 * diagnostic mirror of the `upcoming` watch. Like the other watch loops,
 * `listStatus` re-reads the JSON store each pass (so a resume loop draining the
 * backlog shows up automatically), and the `--grace`, `--tool`/`--project`/
 * `--since`/`--until` scope, plus `--limit` are re-applied every frame. The
