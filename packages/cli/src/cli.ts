...
export function buildCli(): Command {
  ...
  program
    .command("overdue")
    .description("Show jobs whose reset has passed but are still waiting to resume (a stalled-relay diagnostic)")
    .option("-n, --limit <n>", "Show at most N rows (the totals still count all overdue jobs)")
    .option("--grace <duration>", "Only flag jobs overdue by more than this (e.g. 5m, 1h) — filters barely-late jobs")
    .option("-t, --tool <tools>", `Only include jobs run with these comma-separated tools: ${ALL_TOOLS.join(", ")}`)
    .option("-p, --project <projects>", "Only include jobs from these comma-separated project names (exact match)")
    .option("--since <duration>", "Only include jobs created within the last <duration> (e.g. 24h, 7d, 30m)")
    .option("--until <duration>", "Only include jobs created more than <duration> ago (e.g. 1d) — window's older edge")
    ...
    .addHelpText(
      "after",
      "\nExamples:\n" +
        "  # anything the relay should have resumed by now but hasn't?\n" +
        "  agentrelay overdue\n" +
        "  # only jobs stuck for more than 10 minutes (ignore transient lag)\n" +
        "  agentrelay overdue --grace 10m\n" +
        "  # exit non-zero from a monitor when a backlog exists\n" +
        "  test \"$(agentrelay overdue --json | jq '.report.totalOverdue')\" -eq 0"
    )
    .action((opts: ScopeOpts & { limit?: string; grace?: string; json?: boolean }) => {
      ...
      if (opts.grace !== undefined) {
        ...
        if (ms === null || ms < 0) {
          console.error(`Invalid --grace value "${opts.grace}". Use a duration like 5m, 1h, 30s.`);
          process.exitCode = 1;
          return;
        }
        graceMs = ms;
