 ...
 * report on an interval so the "overdue by" spans grow in place. Like the other
 * watch loops, `listStatus` re-reads the JSON store each pass and the scope plus
 * `--grace`/`--limit` are re-applied every frame (the report is rebuilt with a
 * fresh `now`, so newly-overdue jobs appear as they cross their reset time).
 * Runs until the process is interrupted (Ctrl-C).
 */
 ...
 * rollup on an interval so the next-reset countdowns tick down in place. Reuses
 * the same store re-read + scope re-apply as the other watch loops; the summary
 * is recomputed with a fresh `now` each frame. Runs until interrupted (Ctrl-C).
 */
 ...
 * per-project rollup on an interval so the next-reset countdowns tick down in
 * place. Reuses the same store re-read + scope re-apply as the other watch
 * loops; the summary is recomputed with a fresh `now` each frame. Runs until
 * interrupted (Ctrl-C).
 */
 ...
 */
/**
 * Parse a `--watch [seconds]` value into a refresh interval in ms. A bare
 * `--watch` (boolean true) or any non-positive/garbage value falls back to the
 * default 2s, matching the behavior across `status`/`upcoming`/etc.
 */
...
export function buildCli(): Command {
  ...
  program
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # live monitor, refreshing every 2s\n" +
    ...
    .action((opts: ScopeOpts & { limit?: string; grace?: string; json?: boolean; watch?: string | boolean }) => {
      ...
      // Live view: flags above (limit/grace/scope) are validated first so a bad
      // value still exits 1 instead of spinning a broken watch loop. --json takes
  ...
  program
    ...
    .action((opts: ScopeOpts & { json?: boolean; watch?: string | boolean }) => {
      ...
      // Live view: scope is validated above so a bad value still exits 1 instead
      // of spinning a broken watch loop. --json takes precedence over --watch.
      if (opts.watch !== undefined && !opts.json) {
