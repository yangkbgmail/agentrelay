 ...
 * report on an interval so the "overdue by" spans climb in place while the
 * resume loop is down. Like the other watch loops, `listStatus` re-reads the
 * JSON store each pass (a recovering daemon's writes drain the list
 * automatically) and the `--grace`/`--tool`/`--project`/`--since`/`--until`
 * scope plus `--limit` are re-applied every frame. The window boundaries stay
 * fixed (absolute epoch-ms from when the command started) while live writes
 * still surface. Runs until the process is interrupted (Ctrl-C).
 */
  ...
  program
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # live view while diagnosing a stuck resume loop\n" +
