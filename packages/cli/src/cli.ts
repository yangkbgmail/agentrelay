...
/**
 * Shared live-view loop used by `status`/`upcoming`/`overdue` `--watch`. Paints
 * `frame()`'s output on an interval (clearing the screen each pass) so
 * countdowns tick down in place, and tears down cleanly on Ctrl-C. `frame`
 * re-reads the JSON store every pass, so a running daemon's writes show up
 * automatically. Runs until the process is interrupted.
 */
function runWatchLoop(frame: () => string, intervalMs: number): void {
  const draw = () => {
...
/**
 * Parse a `--watch [seconds]` option value into a poll interval in ms. Bare
 * `--watch` (boolean true) or a non-positive/NaN value defaults to 2s, matching
 * `status --watch`.
 */
function watchIntervalMs(value: string | boolean): number {
...
export function buildCli(): Command {
  ...
  program
    ...
    .action((opts: ScopeOpts & { limit?: string; json?: boolean; watch?: string | boolean }) => {
      ...
      const buildTimeline = (at: number): UpcomingTimeline => {
        const allJobs = listStatus(store);
        const jobs = built.active ? scopeJobs(allJobs, built.scope) : allJobs;
        return buildUpcomingTimeline(jobs, at, limit);
      };

      // Live view: the scope note is frozen (the --since/--until edges are
      // absolute epoch-ms fixed above), but the timeline and countdowns are
      // rebuilt each pass so a running daemon's writes and the ticking clock
      // both show up.
      if (opts.watch !== undefined) {
        const intervalMs = watchIntervalMs(opts.watch);
        runWatchLoop(
          () =>
            renderUpcomingWatchFrame(buildTimeline(Date.now()), store, intervalMs, {
              now: Date.now(),
              scopeNote,
            }),
          intervalMs
  ...
  program
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  agentrelay overdue\n" +
        "  # keep an eye on stuck jobs live\n" +
    ...
    .action((opts: ScopeOpts & { limit?: string; grace?: string; json?: boolean; watch?: string | boolean }) => {
      ...
      const scopeNote = built.active ? built.note : undefined;
      const buildReport = (at: number): OverdueReport => {
        const allJobs = listStatus(store);
        const jobs = built.active ? scopeJobs(allJobs, built.scope) : allJobs;
        return buildOverdueReport(jobs, at, { graceMs, limit });
      };

      // Live view: the report (and its overdue spans) is rebuilt each pass from
      // a fresh clock, so parked jobs' "OVERDUE BY" grows in place and a resume
      // loop recovering clears the list without restarting the command.
      if (opts.watch !== undefined) {
        const intervalMs = watchIntervalMs(opts.watch);
        runWatchLoop(
          () =>
            renderOverdueWatchFrame(buildReport(Date.now()), store, intervalMs, {
              now: Date.now(),
              scopeNote,
            }),
          intervalMs
