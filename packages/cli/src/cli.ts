...
export function buildCli(): Command {
  ...
  program
    ...
    .option("--by-hour", "Also show an hour-of-day activity histogram (0–23h, UTC) — when limits pile up")
    ...
    .action(
      (opts: {
        ...
        const byHour = opts.byHour ? computeHourlyTrend(jobs) : null;

        if (opts.json) {
          console.log(renderStatsJson(stats, store, { scope, trend, byHour }));
          return;
        }
        ...
        // already handles the empty/no-match messaging on its own). --trend and
        // --by-hour can be combined; each prints its own block.
        if (trend !== null && stats.total > 0) {
          console.log("");
          console.log(renderTrend(trend, { color: Boolean(process.stdout.isTTY) }));
        }
        if (byHour !== null && stats.total > 0) {
          console.log("");
          console.log(renderHourlyTrend(byHour, { color: Boolean(process.stdout.isTTY) }));
        }
      }
    );
