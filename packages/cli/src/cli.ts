 ...
 * other watch loops, `listStatus` re-reads the JSON store each pass (so newly
 * queued/resumed jobs surface automatically) and the
 * `--status`/`--tool`/`--project`/`--since`/`--until` scope is re-applied every
 * frame. `summarizeJobs` is rebuilt with a fresh `now` each pass so the countdown
 * stays live; the time-window boundaries stay fixed (absolute epoch-ms from when
