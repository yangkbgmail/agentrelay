 ...
 * `--project`/`--since`/`--until` scope is re-applied every frame. The summary is
 * rebuilt with a fresh `now` each pass so the countdowns stay live; the time-
 * window boundaries stay fixed (absolute epoch-ms from when the command started).
 * Runs until the process is interrupted (Ctrl-C).
 */
 ...
 * Clears the screen and re-renders the per-tool index on an interval so the reset
 * countdowns tick down in place, re-reading the store and re-applying the scope
 * each frame. Runs until interrupted (Ctrl-C).
 */
  ...
  program
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # live view, countdowns refreshing every 2s\n" +
