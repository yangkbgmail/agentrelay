...
export function buildCli(): Command {
  ...
  program
    .command("overdue")
    .description("List jobs whose reset time has passed but are still waiting — the sign of a stopped/stuck daemon")
    .option(
      "--grace <duration>",
      "Only flag a job once it's been past-due this long (absorbs the tick interval; e.g. 30s, 5m)",
      "0s"
    )
    ...
    .addHelpText(
      "after",
      "\nExamples:\n" +
        "  # anything the daemon should have resumed but hasn't?\n" +
        "  agentrelay overdue\n" +
        "  # ignore jobs only a few seconds past due (tick slack)\n" +
        "  agentrelay overdue --grace 1m\n" +
        "  # exit non-zero from a health check if anything is stuck\n" +
