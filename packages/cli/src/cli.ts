 ...
 * watch loops, `listStatus` re-reads the JSON store each pass and the scope
 * plus `--grace`/`--limit` are re-applied every frame — the window boundaries
 * stay fixed (absolute epoch-ms) while live writes still surface. Runs until the
...
/**
 * Live `agentrelay tools --watch`: clears the screen and re-summarizes the
 * per-tool rollup on an interval so the "next reset" countdowns tick down in
 * place. `listStatus` re-reads the JSON store each pass and the scope is
 * re-applied every frame. Runs until the process is interrupted (Ctrl-C).
 */
...
/**
 * Live `agentrelay projects --watch`: clears the screen and re-summarizes the
...
/**
 * Resolve the refresh interval for a `--watch [seconds]` flag: an explicit
 * positive number of seconds (rounded to ms), else the 2s default. Shared by
 * every live watch command so `--watch` and `--watch 5` behave identically.
 */
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the rollup with live reset countdowns (Ctrl-C to exit)")
    .option("--json", "Print the summary as JSON (machine-readable, for scripts/CI)")
    .option("-s, --status <statuses>", "Only count jobs with these comma-separated statuses (e.g. waiting_for_reset)")
    .option("-t, --tool <tools>", `Only count jobs run with these comma-separated tools: ${ALL_TOOLS.join(", ")}`)
    .option("-p, --project <projects>", "Only count jobs from these comma-separated project names (exact match)")
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # live rollup, refreshing every 2s\n" +
    ...
    .action((opts: ScopeOpts & { json?: boolean; watch?: string | boolean }) => {
      ...
      if (opts.watch !== undefined && !opts.json) {
        runProjectsWatch(store, watchIntervalMs(opts.watch), built.scope, built.active ? built.note : undefined);
        return; // setInterval keeps the process alive.
      }
  ...
  program
    ...
    .action((opts: ScopeOpts & { json?: boolean; watch?: string | boolean }) => {
      ...
      // spinning a broken watch loop. --json takes precedence over --watch.
      if (opts.watch !== undefined && !opts.json) {
        runToolsWatch(store, watchIntervalMs(opts.watch), built.scope, built.active ? built.note : undefined);
        return; // setInterval keeps the process alive.
      }
