 ...
 * catch-up countdown on an interval so the "caught up in …" figure ticks down in
 * place. Like the other watch loops, `listStatus` re-reads the JSON store each
 * pass (so a running daemon's writes surface automatically — jobs resuming pull
 * the ETA in, new rate-limits push it out) and `computeQueueEta` is rebuilt with
 * a fresh `now` each frame. `eta` aggregates the whole store, so there is no scope
 * to re-apply. Runs until interrupted (Ctrl-C).
 */
...
export function buildCli(): Command {
  ...
  program
    ...
    .action((opts: { json?: boolean; exitCode?: boolean; watch?: string | boolean }) => {
      ...
      // TTY view). --exit-code is a no-op under --watch (the loop never returns).
