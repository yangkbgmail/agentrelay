 ...
 * running daemon's writes — and jobs newly slipping past due — show up
 * automatically) and the `--tool`/`--project`/`--since`/`--until` scope, the
 * `--grace` window, and `--limit` are re-applied every frame. The scope-window
...
export function buildCli(): Command {
  ...
  program
    ...
    .option(
      "-w, --watch [seconds]",
      "Continuously refresh the report as spans grow and jobs slip past due (Ctrl-C to exit)"
    )
