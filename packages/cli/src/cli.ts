 ...
 * one-glance queue overview on an interval so the next-reset countdown ticks down
 * in place. Like the other watch loops, `listStatus` re-reads the JSON store each
 * pass (so a running daemon's writes and newly-queued jobs surface automatically)
 * and the `--status`/`--tool`/`--project`/`--since`/`--until` scope is re-applied
 * every frame. `summarizeJobs` is rebuilt with a fresh `now` each pass so the
 * countdown stays live; the time-window boundaries stay fixed (absolute epoch-ms
 * from when the command started). Runs until interrupted (Ctrl-C).
 */
  ...
  program
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  agentrelay summary\n" +
        "  # keep it on screen with a live countdown\n" +
    ...
    .action((opts: ScopeOpts & { json?: boolean; watch?: string | boolean }) => {
      ...
      if (opts.json) {
        ...
        console.log(renderSummary(summary, { color: Boolean(process.stdout.isTTY), now, scopeNote }));
      }
    });
