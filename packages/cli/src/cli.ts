import type { AgentTool, ExportFormat, GroupByDimension, JobScope, JobStatus, RelayJob } from "@agentrelay/core";
import {
  ALL_TOOLS,
  computeStats,
  EXPORT_FORMATS,
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("-g, --group-by <dimension>", `Break metrics down per group: ${GROUP_BY_DIMENSIONS.join(" | ")}`)
    ...
    .action(
      (opts: {
        ...
        let groupBy: GroupByDimension | undefined;
        if (opts.groupBy !== undefined) {
          if (!GROUP_BY_DIMENSIONS.includes(opts.groupBy as GroupByDimension)) {
            console.error(`Unknown --group-by: "${opts.groupBy}". Valid: ${GROUP_BY_DIMENSIONS.join(", ")}.`);
            process.exitCode = 1;
            return;
          }
        ...
        // --group-by partitions the (scoped) jobs and reports each group's own
        // metrics side by side, instead of one aggregate block.
