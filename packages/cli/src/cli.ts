...
/**
 * Live `agentrelay overdue --watch`: the diagnostic mirror of `upcoming --watch`.
 * Clears the screen and rebuilds the overdue report on an interval so the spans
 * grow (or the list drains) in place. Like the other watch loops, `listStatus`
 * re-reads the JSON store each pass so a running daemon's writes surface, and the
 * `--tool`/`--project`/`--since`/`--until` scope, `--limit`, and `--grace` all
 * re-apply every frame. The window boundaries stay fixed (absolute epoch-ms from
 * when the command started) while `now` — and therefore each overdue span — is
 * re-evaluated every pass. Runs until the process is interrupted (Ctrl-C).
 */
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("-w, --watch [seconds]", "Continuously refresh the overdue report with growing spans (Ctrl-C to exit)")
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # live monitor: leave open and watch a stuck queue's spans climb\n" +
