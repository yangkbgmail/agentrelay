  ...
  startWatchLoop(intervalMs, () => {
    ...
    if (groupBy !== undefined) {
      ...
      if (heatmap && stats.total > 0) {
        body += `\n\n${renderHeatmap(computeWeekHourHeatmap(jobs), { color: true })}`;
      }
    }
...
export function buildCli(): Command {
  ...
  program
    ...
    .option("--heatmap", "Also show a weekday × hour-of-day activity heatmap (jobs created per UTC weekday+hour)")
    ...
    .addHelpText(
      ...
      "\nExamples:\n" +
        ...
        "  # which hour of which weekday rate-limits cluster in (2-D heatmap)\n" +
    ...
    .action(
      (opts: {
        ...
        const heatmap = opts.heatmap ? computeWeekHourHeatmap(jobs) : null;
