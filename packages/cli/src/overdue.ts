...
// jobs whose reset time has passed but which have not been resumed, most-
// overdue first. Where `upcoming` shows the forward runway (what resumes next)
// and `next` the single most imminent resume, `overdue` answers "what should
// already have run but didn't?" — the clearest sign the resume loop
// (daemon/tick) is down or stuck. Kept as pure functions (separate from the
...
/** Shown when no waiting job is past its reset — the healthy, "nothing behind" case. */
 ...
 * its reset (position, short id, project, how overdue, absolute reset time), a
 * header, and a footer summarizing totals, the worst lateness, and how many
 * are hidden by a `--limit`. Reuses `formatDurationMs` so "1h 3m"/"2d 4h" match
 * the `stats` output. Pure: no ambient clock unless `now` is omitted.
 */
export function renderOverdue(report: OverdueReport, options: { color?: boolean; scopeNote?: string } = {}): string {
  ...
  lines.push(
    d("A healthy daemon drains due jobs within a poll interval — run `agentrelay doctor` if this list grows.")
  );
  return lines.join("\n");
}
 ...
 * entries plus honest totals (`totalOverdue`/`hidden`/`worstOverdueByMs`) and
 * the grace window used.
 */
...
function footer(report: OverdueReport): string {
  ...
  if (report.worstOverdueByMs > 0) parts.push(`worst ${formatDurationMs(report.worstOverdueByMs)} late`);
