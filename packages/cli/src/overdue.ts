 ...
 * colored report. Separated out so the watch loop only has to clear the screen
 * and print this. The overdue spans grow in place because the loop re-reads the
 * store and rebuilds the report against a fresh `now` each pass — so a resume
 * loop that stays down shows the backlog aging live.
 */
