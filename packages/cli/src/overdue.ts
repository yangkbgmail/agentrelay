 ...
 * The "overdue by" spans grow in place because the loop re-reads the store and
 * re-builds the report with a fresh `now` each pass, so a stuck job's clock keeps
 * ticking up until the resume loop clears it.
 */
