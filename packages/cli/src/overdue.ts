...
// supposed to have resumed by now but hasn't: `waiting_for_reset` jobs whose
// reset time is more than the grace window in the past, worst offender first.
// Where `upcoming` shows the runway ahead and `next` the single next move,
// `overdue` surfaces the symptom of a stalled resume loop — a daemon can look
// healthy by its heartbeat and still be failing to resume a specific job.
...
/** Shown when no job is overdue — the healthy, boring case. */
export const NO_OVERDUE_MESSAGE = "No overdue jobs — the relay is keeping up.";

/**
 * Human-friendly table for the overdue report: one row per stuck job
 * (position, short id, project, how long overdue, reset time), a header, and a
 * footer summarizing the count, the worst span, and how many rows a `--limit`
 * hides. Reuses `formatDurationMs` so "2h 4m"/"3d 1h" match the `stats`
 * resolution-time output. Pure: the spans are precomputed in the report, so
 * this touches no clock.
 */
...
function footer(report: OverdueReport): string {
  ...
  const parts = [`${report.totalOverdue} ${jobWord} overdue`, `worst ${formatDurationMs(report.maxOverdueByMs)}`];
  if (report.hidden > 0) parts.push(`${report.hidden} more not shown`);
  parts.push(`grace ${formatDurationMs(report.graceMs)}`);
  return parts.join(" · ");
}
