// Rendering for `agentrelay overdue` — the diagnostic backward-looking view of
// the resume timeline: jobs whose reset time has passed (beyond a grace period)
// yet are still waiting to resume. Where `upcoming` shows the forward runway
// ("what resumes next?"), `overdue` answers "what should already be running but
// isn't?" — the clearest single signal that the resume loop (daemon/tick) is
// absent or stuck. Kept as pure functions (separate from the commander wiring)
// so the output is testable without a TTY, a clock, or a spawned process.

import type { OverdueReport } from "@agentrelay/core";
import { formatDurationMs } from "./stats.js";

const DIM = "\x1b[2m";
const BOLD = "\x1b[1m";
const YELLOW = "\x1b[33m";
const RESET = "\x1b[0m";

/** Shown when no job is overdue — the healthy case (green light). */
export const NO_OVERDUE_MESSAGE = "No overdue jobs — the resume loop is keeping up.";

/**
 * Human-friendly multi-line table for the overdue report: one row per overdue
 * job (position, short id, project, how long it's been overdue, the reset time
 * it blew past), a header, and a footer summarizing totals, the worst lag, and
 * how many are hidden by a `--limit`. When jobs are overdue it also prints a
 * one-line hint pointing at the likely cause (no daemon/tick running). Pure: no
 * ambient clock unless `now` is omitted.
 */
export function renderOverdue(
  report: OverdueReport,
  options: { now?: number; color?: boolean; scopeNote?: string } = {}
): string {
  // `now` is accepted for symmetry with sibling renderers, but the report
  // already carries every duration as an absolute span (overdueByMs), so
  // rendering needs no ambient clock.
  const color = options.color ?? false;
  const b = (s: string): string => (color ? `${BOLD}${s}${RESET}` : s);
  const d = (s: string): string => (color ? `${DIM}${s}${RESET}` : s);
  const warn = (s: string): string => (color ? `${YELLOW}${s}${RESET}` : s);

  if (report.entries.length === 0) {
    const note = options.scopeNote ? ` ${d(`[scope: ${options.scopeNote}]`)}` : "";
    return NO_OVERDUE_MESSAGE + note;
  }

  const projWidth = Math.min(28, Math.max(7, ...report.entries.map((e) => e.job.project.length)));
  const lines: string[] = [];

  if (options.scopeNote) lines.push(d(`[scope: ${options.scopeNote}]`));
  lines.push(b(`${pad("#", 3)}  ${pad("ID", 8)}  ${pad("PROJECT", projWidth)}  ${pad("OVERDUE BY", 12)}  RESET AT`));

  for (const entry of report.entries) {
    const pos = pad(String(entry.position), 3);
    const id = pad(entry.job.id.slice(0, 8), 8);
    const project = pad(truncate(entry.job.project, projWidth), projWidth);
    const overdueBy = pad(formatDurationMs(entry.overdueByMs), 12);
    lines.push(`${pos}  ${b(id)}  ${project}  ${warn(overdueBy)}  ${d(entry.job.resetAt ?? "-")}`);
  }

  lines.push("");
  lines.push(d(footer(report)));
  // A non-empty overdue list almost always means nothing is resuming these
  // jobs; point the user straight at the fix rather than making them infer it.
  lines.push(warn("These jobs are due but not resuming. Is the relay running? Start it with `agentrelay daemon`."));
  return lines.join("\n");
}

/**
 * Machine-readable form for `--json` (scripts/jq). Carries the store path, a
 * generation timestamp, the optional active scope, and the full report —
 * entries plus the honest totals (`totalOverdue`/`hidden`/`worstOverdueMs`) and
 * the `graceMs` threshold that was applied.
 */
export function renderOverdueJson(input: {
  storePath: string;
  generatedAt?: string;
  scope?: Record<string, unknown>;
  report: OverdueReport;
}): string {
  return JSON.stringify(
    {
      storePath: input.storePath,
      generatedAt: input.generatedAt ?? new Date().toISOString(),
      scope: input.scope,
      report: input.report,
    },
    null,
    2
  );
}

function footer(report: OverdueReport): string {
  const jobWord = report.totalOverdue === 1 ? "job" : "jobs";
  const parts = [`${report.totalOverdue} ${jobWord} overdue`];
  parts.push(`worst ${formatDurationMs(report.worstOverdueMs)} past due`);
  if (report.hidden > 0) parts.push(`${report.hidden} more not shown`);
  return parts.join(" · ");
}

function pad(s: string, width: number): string {
  return s.length >= width ? s : s + " ".repeat(width - s.length);
}

function truncate(s: string, width: number): string {
  return s.length <= width ? s : `${s.slice(0, Math.max(1, width - 1))}…`;
}
