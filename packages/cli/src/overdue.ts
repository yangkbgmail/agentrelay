 ...
 * (matching the shape of `status --watch` and `upcoming --watch`) plus the
 * colored overdue table. Separated out so the watch loop only has to clear the
 * screen and print this. The "overdue by" spans grow in place because the loop
 * rebuilds the report with a fresh `now` each pass — a stuck job's parked time
 * ticks *up* (the mirror of `upcoming`, whose countdowns tick down).
 */
