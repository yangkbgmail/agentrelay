...
/**
 * One live frame for `agentrelay overdue --watch`: a bold title + dimmed meta
 * line (matching the shape of `status`/`upcoming --watch`) plus the colored
 * report. Separated out so the watch loop only has to clear the screen and
 * print this. The "overdue by" spans grow in place because the loop re-reads
 * the store and rebuilds the report with a fresh `now` each pass.
 */
