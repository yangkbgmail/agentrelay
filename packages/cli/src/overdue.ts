 ...
 * the shape of `status --watch` and `upcoming --watch`) plus the colored overdue
 * table. Separated out so the watch loop only has to clear the screen and print
 * this. The overdue spans grow in place because the loop re-reads the store and
 * rebuilds the report with a fresh `now` each pass — a stuck job's "overdue by"
 * climbs while you watch, the exact signal you're waiting on.
 */
