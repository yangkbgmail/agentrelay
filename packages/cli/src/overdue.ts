...
// already but hasn't: `waiting_for_reset` jobs whose reset time has passed,
// most-overdue first, each with how long it's been stuck. Where `upcoming`
// shows the forward runway, `overdue` is the mirror — a job-level "why hasn't
// this resumed yet?" probe. A non-empty list usually means the resume loop is
// stale/stopped or the agent binary can't be spawned, the exact silent failure
// the relay exists to catch. Kept as pure functions (separate from the
...
export const NO_OVERDUE_MESSAGE = "No overdue jobs — nothing waiting is past its reset time.";
 ...
 * a header, and a footer summarizing totals plus how many are hidden by a
 * `--limit`. Reuses `formatDurationMs` so the "overdue by" column matches the
 * duration formatting used across `stats`. Pure: no ambient clock unless `now`
 * is omitted.
 */
...
function footer(report: OverdueReport): string {
  ...
  if (report.maxOverdueByMs > 0) parts.push(`oldest ${formatDurationMs(report.maxOverdueByMs)} overdue`);
  if (report.hidden > 0) parts.push(`${report.hidden} more not shown`);
  parts.push("check `agentrelay health` / `agentrelay doctor`");
  return parts.join(" · ");
}
