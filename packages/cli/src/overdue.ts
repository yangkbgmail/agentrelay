 ...
 * (matching the shape of `status`/`upcoming --watch`) plus the colored report.
 * Separated out so the watch loop only has to clear the screen and print this.
 * The overdue durations grow in place because the loop re-reads the store and
 * re-derives the report against a fresh `now` each pass — a stuck job's
 * "OVERDUE BY" ticks upward, making a dead resume loop visible at a glance.
 */
