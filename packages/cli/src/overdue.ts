 ...
 * plus the colored report. Separated out (like `status`'s `renderWatchFrame`)
 * so the watch loop in cli.ts only has to clear the screen and print this. The
 * report is expected to be rebuilt with a fresh `now` each pass so the
 * overdue-by spans grow in place and newly-stuck jobs appear.
 */
