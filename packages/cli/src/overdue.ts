 ...
 * colored overdue table. Pure, so the watch loop in cli.ts only clears the
 * screen and prints this. Mirrors `renderUpcomingWatchFrame`/`renderWatchFrame`
 * so all three live views share the same shape. `now` drives only the timestamp
 * stamp (the overdue spans come precomputed in the report).
 */
