 ...
 * The "overdue by" durations grow in place because the loop re-reads the store
 * and passes a fresh `now` each pass.
 */
export function renderOverdueWatchFrame(
  ...
  const stamp = new Date(_now).toISOString().replace("T", " ").slice(0, 19);
