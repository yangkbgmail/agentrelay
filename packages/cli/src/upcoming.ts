...
/**
 * One frame of the live `upcoming --watch` view: a title/meta block plus the
 * countdown table. Mirrors `status`'s `renderWatchFrame` so the watch loop in
 * cli.ts only has to clear the screen and print this. Always colored (the watch
 * view is inherently a TTY). Pure: no ambient clock unless `now` is omitted.
 */
