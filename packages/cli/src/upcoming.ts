 ...
 * (matching `status --watch`) followed by the colored timeline table so
 * countdowns tick down in place. Separated out so the watch loop in cli.ts only
 * has to clear the screen and print this. Pure: no ambient clock unless `now`
 * is omitted.
 */
