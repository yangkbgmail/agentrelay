 ...
 * colored timeline table. Separated out (pure) so the watch loop in cli.ts only
 * has to clear the screen and print this. Mirrors `renderWatchFrame` in
 * status.ts so the two live views feel the same.
 */
