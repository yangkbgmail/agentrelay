...
/**
 * One frame of the live `upcoming --watch` view: a title/meta header block
 * plus the colored timeline table. Mirrors `status`'s `renderWatchFrame` so the
 * two live views share the same look. The watch loop in cli.ts only has to
 * clear the screen and print this each pass; the timeline itself is rebuilt
 * with a fresh `now` on every tick so the countdowns move.
 */
