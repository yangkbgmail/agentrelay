 ...
 * plus the colored timeline. Mirrors `renderWatchFrame` (status) so the two
 * live views look and behave the same — the watch loop in cli.ts only has to
 * clear the screen and print this. Pure: `now` is injected so the countdowns
 * and the timestamp are deterministic in tests.
 */
