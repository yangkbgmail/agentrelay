 ...
 * plus the colored timeline. Separated out (like `status`'s `renderWatchFrame`)
 * so the watch loop in cli.ts only has to clear the screen and print this. The
 * timeline is expected to be rebuilt with a fresh `now` each pass so the
 * countdowns tick down in place.
 */
