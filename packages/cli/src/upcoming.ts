...
/**
 * One frame of the live `agentrelay upcoming --watch` view: a title/meta block
 * plus the colored timeline table. Mirrors `renderWatchFrame` in status.ts so
 * both live views share the same header shape (bold title with the refresh
 * interval, a dimmed timestamp + store path line). Separated out so the watch
 * loop in cli.ts only has to clear the screen and print this. Pure: no ambient
