...
// built from `job.tool`, the adapter label the whole `--tool` filter ecosystem
// keys off. The exact sibling of `renderProjects`, kept as pure functions here,
// separate from the commander wiring in cli.ts, so the output is unit-testable
// without a store or a clock (the only ambient input, `now`, is injectable for
// the reset countdown).
