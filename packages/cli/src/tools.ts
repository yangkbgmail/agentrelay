...
// only ambient input, `now`, is injectable for the reset countdown).
 ...
 * (with a live countdown). Each row is followed by the distinct binaries
 * (`command[0]`) spawned under that tool — the detail that makes this more than
 * `stats --group-by tool`. Pure: no I/O; the only ambient input is `now`, used
...
export function renderTools(
  ...
  const nameWidth = Math.min(28, Math.max(7, ...summary.tools.map((t) => t.tool.length)));
  lines.push(
    d(
  ...
  for (const t of summary.tools) {
    ...
    if (t.binaries.length > 0) {
      lines.push(d(`  ${" ".repeat(nameWidth)}  binaries: ${t.binaries.join(", ")}`));
    }
  }
