...
// built from `job.tool`, the agent adapter the `--tool` filter ecosystem keys
// off. Symmetric to `agentrelay projects`, but adds a resume-reliability lens
// (per-tool success rate) since a tool maps to one adapter/binary. Kept as pure
 ...
 * and terminal counts, its resolve success rate, and — when jobs are parked —
 * the soonest reset (with a live countdown). Pure: no I/O; the only ambient
 * input is `now`, used solely for the countdown and defaulted when omitted.
...
export function renderTools(
  ...
  const nameWidth = Math.min(20, Math.max(9, ...summary.tools.map((t) => t.tool.length)));
  lines.push(
    d(
      `  ${"TOOL".padEnd(nameWidth)}  ${"TOTAL".padStart(5)}  ${"ACTIVE".padStart(6)}  ${"DONE".padStart(4)}  ${"OK%".padStart(4)}  NEXT RESET`
    )
  );
  ...
  for (const t of summary.tools) {
    ...
    const ok = formatSuccessRate(t.successRate).padStart(4);
    let reset: string;
    if (t.nextResetAt) {
    ...
    lines.push(`  ${name}  ${total}  ${active}  ${done}  ${ok}  ${reset}`);
  }
