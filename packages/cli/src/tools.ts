 ...
 * shape of `upcoming --watch`) plus the colored per-tool table. Separated out so
 * the watch loop only has to clear the screen and print this. The "next reset"
 * countdowns tick down in place because the loop re-reads the store and
 * re-summarizes with a fresh `now` each pass.
 */
export function renderToolsWatchFrame(
  ...
  return [title, meta, "", renderTools(summary, { color: true, now, scopeNote })].join("\n");
}
