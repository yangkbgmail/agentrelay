...
// built from `job.tool`, the label the whole `--tool` filter ecosystem keys
// off. Kept as pure functions here, separate from the commander wiring in
// cli.ts, so the exact output is unit-testable without a store or a clock (the
// only ambient input, `now`, is injectable for the reset countdown). This is
// the tool-dimension mirror of projects.ts.
...
/**
 * Renders the tool index as a table: one row per tool with its total, active,
 * and terminal counts plus, when jobs are parked, the soonest reset (with a
 * live countdown). Pure: no I/O; the only ambient input is `now`, used solely
 * for the countdown and defaulted when omitted. `color` gates ANSI codes (TTY
...
export function renderTools(
  ...
  const nameWidth = Math.min(28, Math.max(4, ...summary.tools.map((t) => t.tool.length)));
  lines.push(
    d(
...
/**
 * Machine-readable form of `agentrelay tools`, mirroring the
 * `renderProjectsJson` / `renderStatsJson` envelope: the resolved store path,
 * when it was generated, the optional active scope, and the full summary. Pure:
