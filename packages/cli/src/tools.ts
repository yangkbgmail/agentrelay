 ...
 * shape of `status --watch`/`upcoming --watch`) plus the colored summary. The
 * per-tool "next reset" countdowns tick down in place because the loop re-reads
 * the store and passes a fresh `now` each pass.
 */
