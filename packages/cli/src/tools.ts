...
// from `job.tool`, the agent-adapter label the whole `--tool` filter ecosystem
// keys off. Kept as pure functions here, separate from the commander wiring in
 ...
 * and terminal counts plus, when jobs are parked, the soonest reset (with a live
 * countdown). Pure: no I/O; the only ambient input is `now`, used solely for the
 * countdown and defaulted when omitted. `color` gates ANSI codes (TTY only); a
...
export function renderTools(
  ...
  const nameWidth = Math.min(14, Math.max(7, ...summary.tools.map((t) => t.tool.length)));
  lines.push(
    d(
