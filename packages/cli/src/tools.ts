...
// from `job.tool`, the label the `--tool` filter keys off across the command
// surface. Kept as pure functions here, separate from the commander wiring in
// cli.ts, so the exact output is unit-testable without a store or a clock (the
// only ambient input, `now`, is injectable for the reset countdown). This is the
// tool-dimension sibling of `renderProjects`.
