 ...
 * shape of `status --watch`/`upcoming --watch`) plus the colored per-tool table.
 * The countdowns tick down in place because the watch loop re-reads the store and
 * passes a fresh `now` each pass.
 */
