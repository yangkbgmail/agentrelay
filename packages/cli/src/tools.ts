...
// built from `job.tool`, the agent-tool label the whole `--tool` filter
 ...
 * active, and terminal counts, the derived success rate, and — when jobs are
 * parked — the soonest reset (with a live countdown). Pure: no I/O; the only
 * ambient input is `now`, used solely for the countdown and defaulted when
 * omitted. `color` gates ANSI codes (TTY only); a `scopeNote` is echoed once at
...
export function renderTools(
  ...
  const nameWidth = Math.min(14, Math.max(4, ...summary.tools.map((t) => t.tool.length)));
  lines.push(
    d(
