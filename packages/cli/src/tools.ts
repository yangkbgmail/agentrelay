...
/**
 * One live frame for `agentrelay tools --watch`: a bold title + dimmed meta line
 * (matching the shape of `status`/`upcoming --watch`) plus the colored summary.
 * Separated out so the watch loop only has to clear the screen and print this.
 * The next-reset countdowns tick down in place because the loop re-reads the
