 ...
 * shape of `status --watch`/`upcoming --watch`) plus the colored tool index. The
 * "next reset" countdowns tick down in place because the watch loop re-reads the
 * store and passes a fresh `now` each pass. Separated out so the loop only has to
