...
describe("generateCompletion — fish", () => {
  ...
  it("starts with the fish install header", () => {
    expect(script.startsWith("# fish completion for agentrelay")).toBe(true);
    expect(script).toContain("~/.config/fish/completions/agentrelay.fish");
  });

  it("offers top-level command names gated by __fish_use_subcommand", () => {
    expect(script).toContain("complete -c agentrelay -f -n '__fish_use_subcommand' -a 'run' -d 'command'");
    expect(script).toContain("complete -c agentrelay -f -n '__fish_use_subcommand' -a 'status' -d 'command'");
    expect(script).toContain("complete -c agentrelay -f -n '__fish_use_subcommand' -a 'config' -d 'command'");
  });
  ...
  it("maps long and short flags to -l/-s once a command is seen", () => {
    ...
    // -r is a short flag → -s r
    expect(script).toContain("complete -c agentrelay -f -n '__fish_seen_subcommand_from status' -s r");
  });

  it("offers subcommand names and their flags for a parent command", () => {
    expect(script).toContain(
      "complete -c agentrelay -f -n '__fish_seen_subcommand_from config' -a 'init' -d 'subcommand'"
    );
    expect(script).toContain(
      "complete -c agentrelay -f -n '__fish_seen_subcommand_from config' -a 'show' -d 'subcommand'"
    );
    // per-subcommand flags gated by the subcommand, not the parent
    expect(script).toContain("complete -c agentrelay -f -n '__fish_seen_subcommand_from init' -l force");
    expect(script).toContain("complete -c agentrelay -f -n '__fish_seen_subcommand_from init' -s f");
    expect(script).toContain("complete -c agentrelay -f -n '__fish_seen_subcommand_from show' -l show-secrets");
  });
  ...
  it("throws on an unsafe flag token rather than emitting it", () => {
    expect(() =>
