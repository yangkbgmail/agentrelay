...
/**
 * Per-tool rollup for `agentrelay tools`. The `--tool` filter used across
 * status/stats/export/cancel/retry/metrics/patterns/errors/projects keys off
 * `job.tool` (the adapter that ran the command: claude-code / codex-cli /
 * generic). But nothing let you *discover* which tools actually have jobs in
 * the store, or see at a glance which adapter has work pending. This is the
 * exact sibling of `summarizeProjects`, grouping by `job.tool` instead of
 * `job.project` — one row per distinct tool, with the counts and timing a
 * maintainer needs to spot which adapter is parked waiting for a reset.
 */
export interface ToolBreakdown {
  /** The agent tool / adapter that ran these jobs. */
  tool: AgentTool;
   ...
   * null when none wait. Lets you spot which adapter resumes soonest.
   */
...
export interface ToolsSummary {
  ...
  total: number;
  /** Number of distinct tools present in the considered jobs. */
  toolCount: number;
 ...
 * min/max.
 *
 * Only tools that actually appear in the input get a row (discovery semantics,
 * like `summarizeProjects` — not a zero-filled fixed enum). Rows are ranked by
 * `active` (desc) so a maintainer sees where work is pending first, then by
 * `total` (desc), then by tool name (asc) for a stable order.
 */
