...
/**
 * Per-tool rollup for `agentrelay tools`. Mirror of `agentrelay projects`, but
 * along the agent-tool dimension: the whole `--tool` filter ecosystem
 * (status/stats/export/cancel/retry/metrics/patterns/errors/upcoming/overdue)
 * keys off `job.tool`, yet nothing let you *discover* which tools actually
 * appear in the store or see at a glance which one has work pending. This fills
 * that gap: one row per distinct `job.tool`, with the counts and timing a
 * maintainer needs to decide where to look next.
 *
 * `tool` is typed `AgentTool` on freshly enqueued jobs, but the store can carry
 * arbitrary strings (imported history), so grouping is by the raw value — no
 * fixed toolset is assumed, matching how `scopeJobs` filters on the raw string.
 */
export interface ToolBreakdown {
  /** The tool label (from `job.tool`; the resolved adapter or an imported value). */
  tool: string;
...
export interface ToolsSummary {
  ...
  total: number;
  /** Number of distinct tool labels. */
  toolCount: number;
