...
/**
 * Per-tool rollup for `agentrelay tools` — the mirror image of `summarizeProjects`,
 * but keyed off `job.tool` (the agent CLI a job runs: claude-code / codex-cli /
 * generic) instead of `job.project`. The whole `--tool` filter ecosystem
 * (status/stats/export/cancel/retry/metrics/patterns/errors/projects) keys off
 * these tool labels, but until now nothing let you *discover* which tools are in
 * play or which one has work parked waiting for a reset. This fills that gap: one
 * row per distinct `job.tool`, with the counts and timing a maintainer needs to
 * see which agent is rate-limited and when it resumes.
 */
 ...
 * identical ranking to `summarizeProjects`.
 */
