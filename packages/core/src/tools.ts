 ...
 * status/stats/export/cancel/retry/metrics/patterns/errors/projects all accept
 * keys off `job.tool`, but nothing let you *discover* which agent tools actually
 * have jobs in the store, or see at a glance which one has work parked waiting
 * for a reset. This is the tool-axis twin of `agentrelay projects`: one row per
 * distinct `job.tool`, with the counts and timing a maintainer needs to decide
 * which agent to look at next.
 *
 * It is deliberately operational, not analytical — `stats --group-by tool`
 * already reports success rate and resolution timing per tool. This answers the
 * different question "where is work pending right now, and when does each tool
 * next resume?".
 */
export interface ToolBreakdown {
  /** The agent tool label (from `job.tool`; e.g. claude-code, codex-cli, generic). */
  tool: AgentTool;
 ...
 * labels present, and the per-tool breakdown ranked so the tool with the most
 * pending work floats to the top.
 */
export interface ToolsSummary {
  /** Total jobs considered (after any scope filter the caller applied). */
  total: number;
  /** Number of distinct tool labels that actually appear in the jobs. */
  toolCount: number;
 ...
 * compared lexically (they sort chronologically), matching how
 ...
 * min/max.
 *
 * Only tools that actually appear in the job list get a row (this is a
 * discovery command — "what's in the store"), unlike `stats.byTool` which
 * zero-fills the full known tool set. Rows are ranked by `active` (desc) so a
 * maintainer sees where work is pending first, then by `total` (desc), then by
 * tool name (asc) for a stable order.
 */
