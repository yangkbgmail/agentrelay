...
/**
 * Per-tool rollup for `agentrelay tools`. The `--tool` filter ecosystem
 * (status/stats/export/cancel/retry/projects/patterns/errors) keys off
 * `job.tool` — the agent adapter a job runs under (claude-code / codex-cli /
 * generic) — but nothing let you *discover* which tools actually appear in the
 * store, or see at a glance which adapter has the most work parked waiting for a
 * reset. This is the tool-axis mirror of the project index (`summarizeProjects`):
 * one row per distinct `job.tool`, with the counts and timing a maintainer needs
 * to decide which agent is hitting rate limits hardest.
 */
export interface ToolBreakdown {
  /** The agent tool/adapter this job ran under. */
  tool: AgentTool;
   ...
   * when none wait. Lets you spot which agent resumes soonest.
   */
 ...
 * and the per-tool breakdown ranked so the adapter with the most pending work
 * floats to the top.
 */
export interface ToolsSummary {
  /** Total jobs considered (after any scope filter the caller applied). */
  total: number;
  /** Number of distinct agent tools present. */
  toolCount: number;
  /** Per-tool rows, ranked by active desc, then total desc, then tool name asc. */
  tools: ToolBreakdown[];
}
 ...
 * min/max.
 *
 * Rows are ranked by `active` (desc) so a maintainer sees which agent has work
 * pending first, then by `total` (desc), then by tool name (asc) for a stable
 * order. Unlike a fixed zero-filled tool set, only tools actually present in the
 * job list get a row — mirroring how `summarizeProjects` only emits observed
 * labels.
 */
