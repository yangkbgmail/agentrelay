...
/**
 * Per-tool rollup for `agentrelay tools`. The `--tool` filter is used across the
 * whole command surface (status/stats/export/cancel/retry/metrics/patterns/
 * errors/projects), but nothing let you *discover* which agent tools actually
 * appear in the store, or see at a glance which one has work pending and when it
 * resumes. This is the tool-dimension sibling of `summarizeProjects`: one row
 * per distinct `job.tool`, with the counts and timing a maintainer needs.
 *
 * Where `stats.byTool` only reports a raw job count per tool (and zero-fills all
 * known tools), this rollup adds the active/waiting/terminal split, the soonest
 * pending reset, and the last-touched timestamp — and only for tools actually
 * present, ranked so the tool with the most pending work floats to the top.
 */
export interface ToolBreakdown {
  /** The agent tool label (from `job.tool`; e.g. "claude-code", "codex-cli"). */
  tool: AgentTool;
 ...
 * `summarizeProjects` already picks the earliest `resetAt`; a missing/empty
