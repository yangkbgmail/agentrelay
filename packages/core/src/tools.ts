 ...
 * `job.tool` — the agent adapter a job was queued under (claude-code / codex-cli
 * / generic) — but until now nothing let you *discover* which tools actually
 * appear in the store, or see at a glance which adapter has work pending. This
 * is the tool-keyed sibling of `agentrelay projects`: one row per distinct
 * `job.tool` present, with the counts and timing a maintainer needs to decide
 * where to look next ("am I relaying Claude Code, Codex, or generic jobs, and
 * which one is stuck waiting for a reset?").
 */
export interface ToolBreakdown {
  /** The agent tool / adapter label (`job.tool`). */
  tool: AgentTool;
 ...
 * min/max. Only tools actually present in `jobs` get a row (this is a discovery
 * view, not a fixed zero-filled table like `stats.byTool`).
 *
