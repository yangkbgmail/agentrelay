 ...
 * (status/stats/export/cancel/retry/metrics/patterns/errors/upcoming) keys off
 * `job.tool`, but until now nothing let you *discover* which agent tools
 * actually have jobs in the store, or see at a glance which one has work pending
 * and when it resumes. This is the tool-axis mirror of `summarizeProjects`: one
 * row per distinct `job.tool`, with the counts and timing a maintainer needs to
 * decide where to look next.
 */
export interface ToolBreakdown {
  /** The agent tool (`claude-code` | `codex-cli` | `generic`). */
  tool: AgentTool;
...
export interface ToolsSummary {
  ...
  total: number;
  /** Number of distinct tools that have at least one job. */
  toolCount: number;
 ...
 * *discovery* command (like `projects`), not a fixed zero-filled shape like
 * `stats.byTool`. Rows are ranked by `active` (desc) so a maintainer sees where
 * work is pending first, then by `total` (desc), then by tool name (asc) for a
 * stable order.
 */
