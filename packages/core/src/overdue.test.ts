...
describe("buildOverdueReport", () => {
  ...
  it("ignores jobs that are not waiting_for_reset even if resetAt has passed", () => {
  ...
  it("computes overdueByMs and maxOverdueByMs from the oldest", () => {
    const report = buildOverdueReport(
      [job({ id: "a", resetAt: "2026-07-30T09:00:00.000Z" }), job({ id: "b", resetAt: "2026-07-30T08:00:00.000Z" })],
      NOW
    );
    expect(report.entries.find((e) => e.job.id === "a")?.overdueByMs).toBe(60 * 60 * 1000);
    expect(report.entries.find((e) => e.job.id === "b")?.overdueByMs).toBe(2 * 60 * 60 * 1000);
  ...
  it("applies a grace period to ignore jobs that only just came due", () => {
    const report = buildOverdueReport(
      [
        job({ id: "fresh", resetAt: "2026-07-30T09:59:00.000Z" }), // 1m overdue
        job({ id: "stuck", resetAt: "2026-07-30T09:30:00.000Z" }), // 30m overdue
      ],
      NOW,
      { graceMs: 5 * 60 * 1000 }
    );
  ...
  it("trims to the limit while keeping totals honest", () => {
    const report = buildOverdueReport(
      [
        ...
        job({ id: "j3", resetAt: "2026-07-30T09:00:00.000Z" }),
      ],
      NOW,
      { limit: 2 }
    );
