...
describe("buildOverdueReport", () => {
  it("returns an empty report when nothing is overdue", () => {
    ...
    expect(report).toEqual({ entries: [], totalOverdue: 0, hidden: 0, minOverdueMs: 0 });
  });
  ...
  it("breaks ties deterministically by createdAt then id, matching upcoming", () => {
    const report = buildOverdueReport(
      [
  ...
  it("applies a grace threshold so momentarily-due jobs are excluded", () => {
    const report = buildOverdueReport(
      [
      ...
      NOW,
      { minOverdueMs: 5 * 60 * 1000 } // 5m grace
    );
    expect(report.entries.map((e) => e.job.id)).toEqual(["stuck"]);
    expect(report.minOverdueMs).toBe(5 * 60 * 1000);
  });

  it("treats a reset exactly at the grace cutoff as overdue", () => {
    const report = buildOverdueReport(
      [job({ id: "edge", resetAt: "2026-07-30T09:55:00.000Z" })],
      NOW,
      { minOverdueMs: 5 * 60 * 1000 } // cutoff = 09:55 exactly
    );
    expect(report.totalOverdue).toBe(1);
  });

  it("clamps a negative or non-finite grace to zero", () => {
    const jobs = [job({ resetAt: "2026-07-30T10:00:00.000Z" })];
    expect(buildOverdueReport(jobs, NOW, { minOverdueMs: -1000 }).minOverdueMs).toBe(0);
    expect(buildOverdueReport(jobs, NOW, { minOverdueMs: Number.NaN }).minOverdueMs).toBe(0);
    expect(buildOverdueReport(jobs, NOW, { minOverdueMs: -1000 }).totalOverdue).toBe(1);
  });
