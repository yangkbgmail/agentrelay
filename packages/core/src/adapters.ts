 ...
 *
 * The epoch is normally 10-digit *seconds*, but different Claude/wrapper
 * versions have been observed emitting 13-digit *milliseconds* (e.g. when the
 * value comes straight from `Date.now()`), so both widths are accepted and
 * disambiguated by digit count. The `\b` boundary keeps ambiguous 11/12-digit
 * values (neither a clean seconds nor a clean ms epoch) from matching — better
 * to fall through than to resume at a wildly wrong time.
 */
