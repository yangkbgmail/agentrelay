...
const PATTERNS: RateLimitPattern[] = [
  ...
  },
  {
    ...
    // "try again in 2 days" / "resets in 1d 4h" / "resets in 1 week" / "try again in
    // 2w 3d" — weeks and days cover the weekly/daily usage windows that agents like
    // Claude Code now enforce ("your weekly limit resets in 3 days"). Seconds are
    // deliberately *not* handled here (see adapters.ts: they are OpenAI/Codex-style
    // wording that the Codex adapter contributes). Units are optional and combine
    // largest-to-smallest, so any subset (e.g. bare "in 1 week") still matches.
