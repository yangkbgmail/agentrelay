 ...
 * can rank the whole resume timeline with the exact ordering `next` uses to
 * pick its single job (its first entry is guaranteed to equal `next`).
 */
...
export function selectNextResume(jobs: RelayJob[], now: number = Date.now()): NextResume | null {
  ...
  const job = waiting.reduce((best, candidate) => (compareResumeOrder(candidate, best) < 0 ? candidate : best));
