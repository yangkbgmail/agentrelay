 ...
 * (the list form of `next`) sorts the whole resume timeline identically.
 */
export function compareNextResume(a: RelayJob, b: RelayJob): number {
...
export function selectNextResume(jobs: RelayJob[], now: number = Date.now()): NextResume | null {
  ...
  const job = waiting.reduce((best, candidate) => (compareNextResume(candidate, best) < 0 ? candidate : best));
