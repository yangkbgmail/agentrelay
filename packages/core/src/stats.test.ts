...
describe("computeHourlyDistribution", () => {
  ...
  it("buckets jobs by their UTC hour of day, aggregated across days", () => {
    const jobs = [
      job({ createdAt: "2026-07-20T09:15:00.000Z" }),
      job({ createdAt: "2026-07-19T09:45:00.000Z" }), // same hour, different day
      job({ createdAt: "2026-07-18T23:59:59.000Z" }),
    ];
  ...
  it("puts midnight and the last hour in the right buckets (0 and 23)", () => {
    const jobs = [job({ createdAt: "2026-07-20T00:00:00.000Z" }), job({ createdAt: "2026-07-20T23:30:00.000Z" })];
    const hourly = computeHourlyDistribution(jobs);
