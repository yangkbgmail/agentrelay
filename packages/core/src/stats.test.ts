...
describe("computeHourlyActivity", () => {
  it("returns exactly 24 zero-filled slots, hour 0 through 23, for an empty store", () => {
  ...
  it("buckets jobs by their UTC hour of day", () => {
    const jobs = [
      job({ createdAt: "2026-07-20T01:15:00.000Z" }),
      job({ createdAt: "2026-07-19T01:59:59.000Z" }), // same hour, different day
      job({ createdAt: "2026-07-18T14:00:00.000Z" }),
    ];
    const hourly = computeHourlyActivity(jobs);
    expect(hourly[1].count).toBe(2);
    expect(hourly[14].count).toBe(1);
    expect(hourly.reduce((sum, h) => sum + h.count, 0)).toBe(3);
  });

  it("uses UTC hours regardless of the offset in the timestamp", () => {
    // 23:30 at +05:00 is 18:30 UTC — must bucket into hour 18, not 23.
    const hourly = computeHourlyActivity([job({ createdAt: "2026-07-20T23:30:00.000+05:00" })]);
    expect(hourly[18].count).toBe(1);
    expect(hourly[23].count).toBe(0);
  });

  it("skips jobs with a missing/unparseable createdAt", () => {
    const jobs = [job({ createdAt: "not-a-date" }), job({ createdAt: "2026-07-20T09:00:00.000Z" })];
    const hourly = computeHourlyActivity(jobs);
    expect(hourly.reduce((sum, h) => sum + h.count, 0)).toBe(1);
    expect(hourly[9].count).toBe(1);
  });
});
