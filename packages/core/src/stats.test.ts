...
describe("computeStats", () => {
  ...
  it("computes quartile spread (p25/p75/iqr) and stdev over resolved jobs", () => {
    ...
    // median 2h; deviations |1-2|,|3-2| = [1h,1h] → median 1h
    expect(stats.timing.madResolutionMs).toBe(3_600_000);
  });

  it("keeps MAD robust to a single extreme outlier the stdev inflates", () => {
    const at = (h: number) => `2026-07-13T${String(h).padStart(2, "0")}:00:00.000Z`;
    const stats = computeStats([
    ...
    // sorted [1h,1h,1h,10h] → median 1h; deviations [0,0,0,9h] → median 0
    expect(stats.timing.madResolutionMs).toBe(0);
    // the lone 10h span drags the stdev far above the MAD's zero
