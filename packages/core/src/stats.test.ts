...
describe("computeStats", () => {
  ...
  it("reports an outlier-robust MAD far below the stdev when one span is extreme", () => {
    const at = (h: number) => `2026-07-13T${String(h).padStart(2, "0")}:00:00.000Z`;
    const stats = computeStats([
    ...
    // sorted spans [1h, 1h, 1h, 21h], median = (1h+1h)/2 = 1h
    expect(stats.timing.medianResolutionMs).toBe(3_600_000);
    // deviations |1-1|,|1-1|,|1-1|,|21-1| = [0,0,0,20h] → median deviation 0
    // MAD ignores the lone outlier entirely; the stdev is dragged up to ~8.66h.
