...
describe("computeHourlyActivity", () => {
  ...
  it("buckets jobs by their UTC creation hour, collapsing all days", () => {
    const jobs = [
      job({ createdAt: "2026-07-20T09:15:00.000Z" }),
      job({ createdAt: "2026-07-13T09:59:59.000Z" }), // different day, same hour
      job({ createdAt: "2026-07-19T23:00:00.000Z" }),
    ];
    const hourly = computeHourlyActivity(jobs);
    expect(hourly[9]).toEqual({ hour: 9, count: 2 });
    expect(hourly[23]).toEqual({ hour: 23, count: 1 });
    expect(hourly.reduce((sum, h) => sum + h.count, 0)).toBe(3);
  });

  it("places midnight and pre-midnight jobs in hours 0 and 23", () => {
