...
export interface AgentRelayConfig {
  ...
  notify?: {
    ...
    webhookAuth?: string;
    /**
     * Comma-separated allowlist of events to deliver (`queued`, `resumed`,
     * `completed`, `failed`) — maps to `AGENTRELAY_NOTIFY_EVENTS`. Empty/unset
     * or listing no known event delivers every event.
     */
    events?: string;
  };
...
export function validateConfig(config: AgentRelayConfig): ConfigIssue[] {
  ...
  if (events !== undefined && events.trim() !== "") {
    const { events: recognized, invalid } = parseNotifyEvents(events);
    if (invalid.length > 0) {
      error(
        "notify.events",
        `has unknown event(s) ${invalid.map((e) => `"${e}"`).join(", ")}; valid events are queued, resumed, completed, failed`
      );
    } else if (recognized.length === 0) {
      warn("notify.events", "lists no known events, so every event will be delivered");
    }
  }
