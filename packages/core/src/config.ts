...
export interface AgentRelayConfig {
  ...
  notify?: {
    ...
    webhookAuth?: string;
    /** Local shell command run per event — maps to `AGENTRELAY_NOTIFY_EXEC`. */
    exec?: string;
  };
...
export const CONFIG_FIELDS: ConfigField[] = [
  ...
  { key: "notify.exec", group: "notify", type: "string" },
...
export function parseConfig(value: unknown, source = "config"): AgentRelayConfig {
  ...
  if (root.notify !== undefined) {
    ...
    if (notify.exec !== undefined) config.notify.exec = asString(notify.exec, `${source}.notify.exec`);
  }
...
export function configToEnv(config: AgentRelayConfig): Record<string, string> {
  ...
  set("AGENTRELAY_NOTIFY_EXEC", config.notify?.exec);
...
export const CONFIG_ENV_KEYS: ConfigEnvKey[] = [
  ...
  { key: "AGENTRELAY_NOTIFY_EXEC", group: "notify" },
