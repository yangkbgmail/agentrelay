...
import { parseNotifyEvents } from "./notify.js";
...
export interface AgentRelayConfig {
  ...
  notify?: {
    ...
    webhookAuth?: string;
    /**
     * Comma-separated lifecycle events to notify on (`queued`, `resumed`,
     * `completed`, `failed`) — maps to `AGENTRELAY_NOTIFY_EVENTS`. Unset means
     * every event is delivered; list a subset (e.g. `"completed,failed"`) to
...
export function sampleConfig(): AgentRelayConfig {
  return {
    ...
    notify: {
      ...
      webhookUrl: "",
      webhookAuth: "",
      events: "queued,resumed,completed,failed",
    },
    retry: {
      maxAttempts: 5,
      baseDelayMs: 1000,
...
export function validateConfig(config: AgentRelayConfig): ConfigIssue[] {
  ...
  if (events !== undefined) {
    ...
    if (parsed.invalid.length > 0) {
      error(
        "notify.events",
        `has unknown event name(s): ${parsed.invalid.join(", ")}. Valid events are queued, resumed, completed, failed`
      );
    } else if (parsed.events.length === 0 && events.trim() !== "") {
      warn("notify.events", "lists no recognised events; every event will be delivered");
    }
  }
