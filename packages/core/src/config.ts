...
export interface AgentRelayConfig {
  ...
  notify?: {
    ...
    webhookAuth?: string;
    /**
     * Comma-separated list of events to notify on (subset of
     * `queued,resumed,completed,failed`) — maps to `AGENTRELAY_NOTIFY_EVENTS`.
     * Empty/unset means notify on every event.
     */
    events?: string;
  };
...
export function validateConfig(config: AgentRelayConfig): ConfigIssue[] {
  ...
  if (events !== undefined && events.trim() !== "") {
    ...
    if (invalid.length > 0) {
      error(
        "notify.events",
        `contains unknown event(s): ${invalid.map((e) => `"${e}"`).join(", ")}. Valid events: ${NOTIFY_EVENTS.join(", ")}`
      );
    } else if (recognized.length === 0) {
      warn("notify.events", "has no valid events, so the filter is ignored and every event notifies");
    }
  }
