...
describe("summarizeTools", () => {
  ...
  it("ranks by active desc, then total desc, then name asc", () => {
    const summary = summarizeTools([
      ...
      // generic: 1 active, 1 total (ties codex-cli on active, loses on total)
      job({ tool: "generic", status: "queued" }),
      // aaa: 1 active, 1 total (ties generic fully → name asc, imported label)
      job({ tool: "aaa" as AgentTool, status: "queued" }),
    ]);
    expect(summary.tools.map((t) => t.tool)).toEqual(["claude-code", "codex-cli", "aaa", "generic"]);
  });

  it("groups by the raw tool string (imported labels outside the known set)", () => {
    const summary = summarizeTools([
      job({ tool: "custom-agent" as AgentTool, status: "queued" }),
      job({ tool: "custom-agent" as AgentTool, status: "completed" }),
    ]);
    expect(summary.toolCount).toBe(1);
    expect(summary.tools[0]).toMatchObject({ tool: "custom-agent", total: 2, active: 1, terminal: 1 });
  });
});
