...
describe("summarizeTools", () => {
  ...
  it("only includes tools that actually appear (no zero-filled enum)", () => {
  ...
  it("ranks by active desc, then total desc, then name asc", () => {
    const summary = summarizeTools([
      ...
      job({ tool: "generic", status: "completed" }),
      job({ tool: "generic", status: "completed" }),
      // claude-code: 2 active, 2 total → top by active
      job({ tool: "claude-code", status: "queued" }),
  ...
  it("does not mutate the input array", () => {
    const jobs = [job({ tool: "generic" }), job({ tool: "claude-code" })];
