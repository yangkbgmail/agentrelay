...
describe("summarizeTools", () => {
  ...
  it("computes success rate from completed/(completed+failed), excluding cancelled", () => {
    const summary = summarizeTools([
      job({ tool: "claude-code", status: "completed" }),
      job({ tool: "claude-code", status: "completed" }),
      job({ tool: "claude-code", status: "completed" }),
      job({ tool: "claude-code", status: "failed" }),
      job({ tool: "claude-code", status: "cancelled" }), // must not drag the rate down
    ]);
    const t = summary.tools[0];
    expect(t.completed).toBe(3);
    expect(t.failed).toBe(1);
    expect(t.successRate).toBeCloseTo(0.75); // 3/(3+1)
  });

  it("leaves success rate null when nothing has resolved for a tool", () => {
    const summary = summarizeTools([
  ...
  it("ranks by active desc, then total desc, then name asc", () => {
    const summary = summarizeTools([
      ...
      job({ tool: "generic", status: "completed" }),
      job({ tool: "generic", status: "completed" }),
      // claude-code: 2 active, 2 total → top (most pending)
      job({ tool: "claude-code", status: "queued" }),
