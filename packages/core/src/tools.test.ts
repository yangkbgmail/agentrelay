...
describe("summarizeTools", () => {
  ...
  it("only lists tools that are actually present (no zero-filled enum)", () => {
    ...
    // "generic" has no jobs, so it must not appear.
    expect(summary.tools.some((t) => t.tool === "generic")).toBe(false);
  });
  ...
  it("leaves nextResetAt null when no job waits (or waiting jobs lack a resetAt)", () => {
    const summary = summarizeTools([
      ...
      job({ tool: "codex-cli", status: "waiting_for_reset", resetAt: null }),
    ]);
  ...
  it("tracks the most recent updatedAt as lastActivityAt", () => {
    const summary = summarizeTools([
      job({ tool: "claude-code", updatedAt: "2026-07-13T01:00:00.000Z" }),
      job({ tool: "claude-code", updatedAt: "2026-07-13T05:00:00.000Z" }),
      job({ tool: "claude-code", updatedAt: "2026-07-13T03:00:00.000Z" }),
    ]);
    expect(summary.tools[0].lastActivityAt).toBe("2026-07-13T05:00:00.000Z");
  });

  it("ranks by active desc, then total desc, then tool asc", () => {
    const summary = summarizeTools([
      // generic: 1 active, 3 total
      job({ tool: "generic", status: "queued" }),
      job({ tool: "generic", status: "completed" }),
      job({ tool: "generic", status: "completed" }),
      // claude-code: 2 active, 2 total → ranks first (most active)
      job({ tool: "claude-code", status: "queued" }),
      job({ tool: "claude-code", status: "resuming" }),
      // codex-cli: 1 active, 1 total (ties generic on active, loses on total;
      // ties nothing fully here, but sits after generic)
      job({ tool: "codex-cli", status: "queued" }),
    ]);
    expect(summary.tools.map((t) => t.tool)).toEqual(["claude-code", "generic", "codex-cli"]);
  });

  it("breaks a full active+total tie by tool name ascending", () => {
    const summary = summarizeTools([
      job({ tool: "generic", status: "queued" }),
      job({ tool: "codex-cli", status: "queued" }),
    ]);
