...
describe("summarizeTools", () => {
  ...
  it("only includes tools that actually appear (no zero-fill)", () => {
  ...
  it("ranks by active desc, then total desc, then name asc", () => {
    const tools: AgentTool[] = ["claude-code", "codex-cli", "generic"];
    const summary = summarizeTools([
      // generic: 1 active, 3 total
      job({ tool: tools[2], status: "queued" }),
      job({ tool: tools[2], status: "completed" }),
      job({ tool: tools[2], status: "completed" }),
      // claude-code: 2 active, 2 total → ranks first
      job({ tool: tools[0], status: "queued" }),
      job({ tool: tools[0], status: "resuming" }),
      // codex-cli: 1 active, 1 total (ties generic on active, loses on total → after generic)
      job({ tool: tools[1], status: "queued" }),
    ]);
    // claude-code (2 active) > generic (1 active/3 total) > codex-cli (1 active/1 total)
