...
describe("summarizeTools", () => {
  ...
  it("ranks by active desc, then total desc, then name asc", () => {
    const summary = summarizeTools([
      ...
      job({ tool: "generic", status: "completed" }),
      job({ tool: "generic", status: "completed" }),
      // claude-code: 2 active, 2 total → floats to top on active
      job({ tool: "claude-code", status: "queued" }),
  ...
  it("breaks a full active+total tie by tool name ascending", () => {
    const summary = summarizeTools([
      // both 1 active, 1 total → decided by name asc: claude-code < generic
      job({ tool: "generic", status: "queued" }),
      job({ tool: "claude-code", status: "queued" }),
    ]);
    expect(summary.tools.map((t) => t.tool)).toEqual(["claude-code", "generic"]);
  });
});
