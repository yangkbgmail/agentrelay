...
export interface TimingStats {
  ...
  /**
   * Median absolute deviation of resolution times (ms): the median of each
   * span's absolute distance from the *median* span, or null when none. Like
   * {@link iqrResolutionMs} it's a robust spread measure — but where the stdev
   * squares deviations (so one far outlier dominates it) the MAD takes their
   * median, so a single pathological long-babysat job barely moves it. Read it
   * against {@link stdevResolutionMs}: a MAD far below the stdev is the
   * fingerprint of a heavy-tailed distribution (most resolutions tightly
   * clustered, a few extreme). A single (or all-equal) span yields 0.
   */
 ...
 * its already-computed median: MAD = median(|xᵢ − median|), rounded to whole ms
 * (via {@link percentile}). Robust to outliers — it takes the median of the
 * deviations rather than squaring them like the stdev, so one far span barely
 * shifts it. A single value (or all-equal values) yields 0. Non-empty input.
 */
