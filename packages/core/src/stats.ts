...
export interface TimingStats {
   ...
   * null when none. The most outlier-robust spread measure here — a single
   * pathological long-babysat job barely moves it (it shifts at most one sample
   * past the median), whereas the same job can dominate {@link stdevResolutionMs}.
   * Unlike the {@link iqrResolutionMs} which reads the middle 50% off two
   * quartiles, MAD measures every resolution's typical distance from the center,
   * so a small MAD next to a large stdev is the clearest "usually consistent,
   * rare wild outliers" signature. Rounded to whole ms.
   */
 ...
 * supplied median, rounded to whole ms. The most outlier-robust spread measure
 * here — one wild span shifts at most a single sample past the center, so the
 * MAD barely moves where the stdev would balloon. Reuses {@link percentile} for
 * the inner median. `median` must be the p50 of `sortedAsc`; a single value
 * yields 0 (no spread). Callers guarantee `sortedAsc.length > 0`.
 */
