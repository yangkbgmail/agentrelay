...
export interface TimingStats {
   ...
   * when none. The most outlier-robust spread measure here — its 50% breakdown
   * point means half the resolutions can be arbitrarily large without moving it,
   * even sturdier than the {@link iqrResolutionMs}. Read alongside
   * {@link stdevResolutionMs}: a MAD far smaller than the stdev is the clearest
   * sign that a few pathological long-babysat jobs are inflating the mean-based
   * spread while the bulk of resolutions stay tightly clustered.
   */
...
/**
 * Median absolute deviation (ms): the median of the absolute deviations from the
 * data's median, `median(|xᵢ − median(x)|)`, rounded to whole ms. Reuses
 * {@link percentile} for both medians (type-7 interpolation) so it agrees with
 * the reported {@link TimingStats.medianResolutionMs}. A single value — or all
 * identical values — yields 0 (no spread). Callers guarantee non-empty input.
 */
