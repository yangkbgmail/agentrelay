...
export interface TimingStats {
   ...
   * null when none. The most outlier-robust spread measure here — where the
   * stdev squares deviations (so one pathological long-babysat job dominates it)
   * and even the IQR still shifts with the quartile positions, the MAD is
   * anchored on the median twice over, so a single wild span barely moves it.
   * A MAD much smaller than the stdev is the clearest signal that the spread is
   * driven by rare outliers rather than broad variability. A single resolved job
   * yields 0. Rounded to whole ms.
   */
 ...
 * whole ms: the median of the absolute deviations `|v − center|` from the list's
 * own median (`center`). A robust dispersion estimator — anchored on the median
 * both for the center and the outer summary, so a single extreme value barely
 * perturbs it, unlike the mean-based stdev. The absolute deviations are sorted
 * fresh (the input's ascending order does not survive the `|·|` fold). A single
 * value yields 0 (no spread). Callers guarantee `sortedAsc.length > 0`.
 */
function medianAbsoluteDeviation(sortedAsc: number[], center: number): number {
  const deviations = sortedAsc.map((v) => Math.abs(v - center)).sort((a, b) => a - b);
