...
export interface TimingStats {
   ...
   * none. The most outlier-robust spread measure here — where the stdev is
   * dragged by a single pathological long-babysat job and even the IQR shifts
   * with the quartiles, the MAD reports the typical distance of a resolution
   * from the median and barely moves for a lone outlier. A MAD far below the
   * stdev is the clearest signal that the spread is driven by a few extremes
   * rather than broad variability. Rounded to whole ms; a single job yields 0.
   */
...
/**
 * Median absolute deviation (ms) over an ascending-sorted, non-empty array,
 * rounded to whole ms. MAD = median(|xᵢ − median|). Reuses {@link percentile}
 * (type-7 median) for both the center and the median of the absolute deviations
 * — the deviations are re-sorted since |x − median| is not monotonic in x.
 * The most outlier-robust spread measure available here. Callers guarantee
 * non-empty input and pass the already-computed median to avoid recomputing it.
 */
