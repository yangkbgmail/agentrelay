...
export interface TimingStats {
   ...
   * outlier-robust spread measure here — where the stdev squares deviations
   * (so one pathological long-babysat job dominates) and even the IQR reads off
   * the quartiles, the MAD is the *typical* deviation from the typical case. A
   * MAD far below the stdev is the clean signature of "usually consistent, a
   * few wild tails". Rounded to whole ms, same "type 7" median as the percentiles.
   */
...
/**
 * Median absolute deviation: the median of each value's absolute distance from
 * `median`, rounded to whole ms. A robust spread measure — unlike the stdev it
 * squares nothing, so a single outlier can't dominate it. Reuses {@link percentile}
 * (same "type 7" interpolation as the reported median) so the two agree. A single
 * value yields 0 (no spread). Callers guarantee `sortedAsc.length > 0`; the
 * pre-sorted `median` is passed in so it isn't recomputed.
 */
