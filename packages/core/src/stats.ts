...
export interface TimingStats {
   ...
   * outlier-robust spread measure here — its breakdown point is 50%, so half
   * the sample can be pathological long-babysat jobs without moving it, where
   * even the IQR shifts and the stdev blows up. Read alongside
   * {@link stdevResolutionMs}: a MAD far below the stdev is the clean signature
   * of a tight core with a few heavy outliers (for a normal distribution stdev
   * ≈ 1.4826 × MAD, so a much larger ratio means fat tails). Rounded to whole ms.
   */
...
/**
 * Median absolute deviation (ms): the median of |x − median| over a non-empty
 * list, rounded to whole ms. `median` is the pre-computed p50 of the same list,
 * passed in so the value matches {@link TimingStats.medianResolutionMs} exactly
 * (percentile interpolation and this share one reference point). Uses the linear
 * `percentile` helper on the sorted deviations, so a single value yields 0 (no
 * spread). Callers guarantee non-empty input.
 */
