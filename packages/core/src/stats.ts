...
export interface TimingStats {
   ...
   * distance from the median resolution, or null when none. Like
   * {@link iqrResolutionMs} it's a robust spread measure, but even more resistant
   * to outliers than the stdev — a single pathological long-babysat job barely
   * moves it, where it can dominate {@link stdevResolutionMs}. A MAD much smaller
   * than the stdev is the signature of a mostly-tight distribution with rare
   * heavy tails. Rounded to whole ms. A single resolved job yields 0 (no spread).
   */
...
/**
 * Median absolute deviation: the median of |value − median| over a non-empty,
 * ascending-sorted list, rounded to whole ms (via {@link percentile}). A robust
 * dispersion measure — unlike the stdev, one extreme outlier barely shifts it.
 * A single value yields 0 (no spread). Callers pass the already-sorted list and
 * its median so the sort/median are computed once.
 */
